...
One may view Proposition \ref{prop322} as a template for the set of requirements any sufficiently local means of defining small extensions of $V$ ought to fulfil. More precisely, should we replace 
\begin{itemize}
    \item ``$\mathfrak{T}$ is a TCI'' with any other means of defining the parameter set, and
    \item ``$\mathcal{M} \models^* \mathfrak{T}$'' with any other means of locally defining the class of possible generators from the parameter set
\end{itemize}
in Proposition \ref{prop322}, we should still expect the said proposition to hold. In any case where the aforementioned replacements behave as expected, we say that the associated means of defining small extensions of $V$ \textit{fulfils the requirements suggested by Proposition \ref{prop322}}.
\end{rem}
