...
Define 
\begin{equation*}
    U(\Vec{y}, \varphi) := \{V[x] : \exists x' \in V \ (x \subset x') \text{ and } \varphi (\Vec{y}, x) \} \text{.}
\end{equation*}
...
Corollary \ref{cor9416} can be improved in different directions with the use of some basic coding.
...
Define 
\begin{align*}
    U'(\Vec{y}, \varphi) := \ & \{W \in \mathbf{M}(V) : (W; \in) \models \exists x \ \varphi (\Vec{y}, x) \} \\
    U''(\Vec{y}, \varphi) := \ & \{W \in \mathbf{M}(V) : \exists x \in W \ (W = V[x] \wedge (W; \in) \models \varphi (\Vec{y}, x))\} \text{.}
\end{align*}
\begin{enumerate}[label=(\arabic*)]
