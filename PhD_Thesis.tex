...
By the modularisability of our forcing framework, one can easily verify that greater support analogues of the forcing notions witnessing the main theorems of Section \ref{ss43} exist. More specifically, the following can be proven.
...
Then there is a stationary-preserving forcing notion $\mathbb{P}$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\alph*)]
    \item for some $\kappa$, $\mathfrak{A}$, $\mathcal{L}$, $C$ and $\{Q_{\lambda} : \lambda \in C\}$ in $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$, 
    \begin{equation*}
        \mathbb{P} = Col(\lambda^{\dagger}, \lambda^{\dagger}) * \mathbb{P}^C_{\lambda^{\dagger}}(\omega_1) \text{,}
    \end{equation*}
    \item $\mathbb{P}$ witnesses $Nb'_1(\lambda^{\dagger})$ (thus also $Nb_1(\lambda^{\dagger})$), and
    \item in $V^{\mathbb{P}}$, there is a $\Diamond$-iteration $$\langle \bar{N}_i = (N_i; \in, I_i, a_i), \sigma_{ij} : i \leq j \leq \omega_1^V \rangle$$ satisfying 
    \begin{enumerate}[label=(\arabic*), leftmargin=40pt]
