...
Let $B \in \mathbf{\Sigma_1}(L_{\alpha}[A])$ for some admissible ordinal $\alpha$. Then $B \leq^P_{\alpha} A$.
\end{lem}
...
\begin{cor}
Let $B \in \mathbf{\Pi_1}(L_{\alpha}[A])$ for some admissible ordinal $\alpha$. Then $B \leq^P_{\alpha} A$.
\end{cor}

\begin{proof}
We can get $B$ from $\alpha \setminus B$ by bit-flipping, and $\alpha \setminus B \in \mathbf{\Sigma_1}(L_{\alpha}[A])$. By Lemma \ref{lem273}, we are done.
\end{proof}
