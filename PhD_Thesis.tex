...
\end{equation*}
Define $\mathrm{Ex}_n(\lambda)$ to be $ran(\Vdash^n_{Col(\omega, \lambda)})$. If there is a definable (with parameters) class function $F$ on $\mathrm{Ex}_n(\lambda)$ such that $ran(F) \subset V^{Col(\omega, \lambda)}$ and 
...
\end{equation*}
Then there is a unique $x$ such that $p_X \Vdash^n_{Col(\omega, \lambda)} F(\psi_X) = \check{x}$, and it must be the case that $x \in X$. We have thus given a definition of a global choice function based on $F$ and $\lambda$. Since $F$ is itself definable with parameters, we are done.
\end{proof}

More specifically, what stands between Corollary \ref{cor9417} and the collapse of the local method hierarchy above $\mathsf{\Sigma^M_1}$, if we were to follow an argument similar to that found in the proof of Theorem \ref{thm9414}, is the next lemma.

\begin{lem}
Let $\lambda$ be an infinite cardinal, $1 \leq n < \omega$, and $\Vdash^n_{Col(\omega, \lambda)}$ be as defined in Lemma \ref{lem9419}.
If for some $k < \omega$, there is a definable (with parameters) class function $F$ on $\mathsf{\Sigma^M_k}$ such that $ran(F) \subset V^{Col(\omega, \lambda)}$ and 
...
\begin{proof}
Assume $F$ exists for some $\lambda$, $n$ and $k$ in accordance with the hypothesis of the lemma. Recall that for any TCI $\mathfrak{T}$, $\text{``} \mathcal{M} \models^* \mathfrak{T} \text{''}$ is expressible as a $\Sigma_1$ sentence in the language of set theory with the parameters $\mathcal{M}$ and $\mathfrak{T}$, so $\Vdash^n_{Col(\omega, \lambda)} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}} \text{''})$ iff $\mathfrak{T}$ is consistent. 
...
\end{equation*}
Then there is a unique $x$ such that $p_X \Vdash^n_{Col(\omega, \lambda)} \text{``} F(\mathfrak{T}_X) \text{ interprets } \ulcorner \check{c} \urcorner \text{ as } \check{x} \text{''}$, and it must be the case that $x \in X$. We are thus done as we were in the proof of Lemma \ref{lem9419}.
\end{proof}
