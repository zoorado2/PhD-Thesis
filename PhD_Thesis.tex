...
Working in the universe $W$ as defined in the proof of Theorem \ref{notion1} and thinning out $C$ if necessary, fix any $\theta$ such that $[Q_{\lambda}]^{< \theta} \subset Q_{\lambda}$ for all $\lambda \in C$. Should we then alter the definition of $\Gamma_{\lambda}$ and $P_{\lambda}$ for each $\lambda \in C \cup \{\lambda^{\dagger}\}$ as follows:
\begin{align*}
...
Let $\theta$ be a cardinal greater than $\omega$ and $C$ be such that $[Q_{\lambda}]^{< \theta} \subset Q_{\lambda}$ for all $\lambda \in C$. Then the most obvious gap in the proof shows up in the semantic interpretation of members of $\Gamma^C_{\lambda}(\theta)$. More specifically, since there are many dense subsets of many of the $\mathbb{P}_{\lambda'}$'s which are definable without parameters and contain only infinite sets, \ref{c8} no longer means
\begin{equation}
    \label{eq47}
...
Thankfully, there is a way out when $\theta = \omega_1$, via a rather straightforward ``quasi-second-order'' augmentation of our language. Let $\mathcal{L}$ be as defined in the proof of Theorem \ref{notion1}. Pick a new distinguished symbol $\dot{G}$, rename $\mathcal{L}$ as $\mathcal{L}^o$, and re-declare
\begin{equation*}
...
We have described above a spiritually countable-support version of the original $\mathbb{P} = \mathbb{P}_{\lambda^{\dagger}}$ constructed in the proof of Theorem \ref{notion1}. To distinguish this new version from the original, call the former $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$. Now, it is a routine exercise in transfinite induction to check that the map $A \mapsto \{\ulcorner A \subset \dot{G} \urcorner\}$ --- under some appropriate quotient --- witnesses $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1) \lessdot \mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$ assuming $[Q_{\lambda}]^{< \theta} \subset Q_{\lambda}$ for all $\lambda \in C$. In fact, as a consequence of the \ref{s9n}'s, the image of any $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$-generic filter $g$ over $W$ under the said map uniquely determines any $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$-generic filter over $W$ extending $g$. This then implies that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$ are forcing equivalent.
...
Let $W$ and $\lambda^{\dagger}$ be as in the proof of Theorem \ref{notion1}. In $W$, can $\theta$, $C$
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}] \text{, and} \\
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto \Gamma_{\lambda}]
\end{gather*}
be chosen such that 
\begin{enumerate}[label=(\arabic*)]
    \item $\theta > \omega_1$ is a cardinal,
    \item $[Q_{\lambda}]^{< \theta} \subset Q_{\lambda}$ for all $\lambda \in C$,
    \item \ref{4101} and \ref{4102} of Lemma \ref{lem26} are fulfilled with $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
...
Let $V$, $W$ and $\lambda^{\dagger}$ be as in the proof of Theorem \ref{notion1}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in $W$, can $\theta$, $C$, 
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}] \text{, and} \\
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto \Gamma_{\lambda}]
\end{gather*}
...
Indeed, one can easily see that the ``quasi-second-order'' augmentation described immediately before Question \ref{oq1} carries over without a hitch in the proofs of Theorems \ref{notion2} and \ref{notion3}, due in no small part to the much-emphasised and -elaborated modularisability of our forcing framework. Moreover, letting $\mathbb{P}^{* < \omega_1}_{\lambda^{\dagger}}$ be the result of the aforementioned augmentation on the version of $\mathbb{P}_{\lambda^{\dagger}}$ constructed in the proof of Theorem \ref{notion2}, one can obtain the forcing equivalence of $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{* < \omega_1}_{\lambda^{\dagger}}$ through essentially the same argument as the one that nets us the forcing equivalence of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$. This specifically motivates the next question.

\begin{ques}\label{oq2}
Let $W$ and $\lambda^{\dagger}$ be as in the proof of Theorem \ref{notion2}. In $W$, can $\theta$, $C$,
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}] \text{, and} \\
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto \Gamma_{\lambda}]
\end{gather*}
...
Let $V$, $W$ and $\lambda^{\dagger}$ be as in the proof of Theorem \ref{notion2}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in  $W$, can $\theta$, $C$, 
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}] \text{, and} \\
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto \Gamma_{\lambda}]
\end{gather*}
