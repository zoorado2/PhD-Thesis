...
Similar in motivation to \ref{def210e} of Definition \ref{lsuitable}, the requirements for $P$ and $\leq_P$ to be $\Sigma_1$-definable in the language associated with $\mathfrak{A}$ are only there so that the proof of Theorem \ref{uni} can go through given its hypothesis.
...
Assume $\mathrm{NS}_{\omega_1}$ is precipitous. Then $Nb'_1(\lambda^{\dagger})$ holds for all regular cardinals $\lambda^{\dagger} > 2^{\omega_1}$. In other words, whenever $\lambda^{\dagger} > 2^{\omega_1}$ is a regular cardinal, there is a stationary-preserving forcing notion $\mathbb{P}'$ such that 
\begin{enumerate}[label=(\arabic*)]
...
If in $V[h]$ there is a stationary-preserving forcing notion $\mathbb{P}$ of size $\leq \lambda^{\dagger}$ fulfilling 
\begin{enumerate}[label=(\arabic*')]
...
\begin{equation*}
    Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}} \text{,}
\end{equation*} 
...
\begin{enumerate}[label=(\arabic*')]
    ...
    \item\label{4104} $\Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} ``2^{< \lambda^{\dagger}} = \lambda^{\dagger}"$
\end{enumerate}
...
In $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$, we are given $|\mathbb{P}| \leq \lambda^{\dagger}$, so $\mathbb{P}$ preserves cofinalities $\geq (\lambda^{\dagger})^+$. But $(\lambda^{\dagger})^+$ in $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$ is exactly $(2^{< \lambda^{\dagger}})^+$ in $V$, by \ref{4103} and \ref{4104}, so $\mathbb{P}'$ preserves cofinalities $\geq (2^{< \lambda^{\dagger}})^+$ in $V$. This implies \ref{cond3} of Theorem  \ref{notion1}.

Now let $\gamma \geq \lambda^{\dagger}$ be regular in $V$. Then $cof^{Col(\lambda^{\dagger}, \lambda^{\dagger})} (\gamma) \geq \lambda^{\dagger}$ because $Col(\lambda^{\dagger}, \lambda^{\dagger})$ is $\lambda^{\dagger}$-closed. That $\mathbb{P}$ both preserves cofinalities $\geq (\lambda^{\dagger})^+$ and fulfils \ref{4102} implies it forces $cof(\gamma) \geq \omega_1$. We have thus shown
\begin{enumerate}[label=(\arabic*')]
    \setcounter{enumi}{4}
    \item\label{4105} $\Vdash_{\mathbb{P}'} ``cof(\gamma) \geq \omega_1"$ for all regular cardinals $\gamma$ satisfying $\lambda^{\dagger} \leq \gamma$.
\end{enumerate}
As \ref{4102}, \ref{4104} and \ref{cond1} of Theorem \ref{notion1} give us $$\Vdash_{\mathbb{P}'} ``cof(\gamma) \leq |\gamma| \leq \omega_1"$$ for all ordinals $\gamma$ satisfying $\lambda^{\dagger} \leq \gamma \leq 2^{< \lambda^{\dagger}}$, \ref{cond2} of Theorem \ref{notion1} must hold.
\end{proof}
...
$|\mathbb{P}| \leq \lambda^{\dagger}$.
\end{lem}

\begin{proof}
This follows immediately from the observation that $$\mathbb{P} \subset [\mathcal{L}]^{<\omega} \subset H(\lambda^{\dagger})$$ and 
\begin{equation*}
    \pushQED{\qed} 
    |H(\lambda^{\dagger})| = \lambda^{\dagger} \text{.} \qedhere
\end{equation*}
\end{proof}
...
\begin{enumerate}[label=(K\arabic*), leftmargin=40pt]
    \item\label{K1} $\mathbb{P} \subset H(\lambda^{\dagger})$, so that $|\mathbb{P}| \leq \lambda^{\dagger}$ by the proof of Lemma \ref{size},
