...
As in the proof of Theorem \ref{thm9419}, we write $\varphi(\Vec{y}, x)$ as $\ulcorner \exists \Vec{z} \ \phi(\Vec{y}, \Vec{z}, x) \urcorner$ for some formula $\phi$ that is $\Delta_0$ in the L\'{e}vy hierarchy, and find in $W$ witnesses $\Vec{b}$ for $\varphi$ so that $(W; \in) \models \phi(\Vec{y}, \Vec{b}, x)$. In $V$, choose a strong limit cardinal $\lambda > \alpha$ and a bijection $g$ from $\lambda$ onto $V_{\lambda}$, such that $\{\Vec{y}, \Vec{b}, x\} \in V_{\lambda}[x]$ and $(V_{\lambda}[x]; \in) \models \phi(\Vec{y}, \Vec{b}, x)$. Over the structure $(V_{\lambda}[x]; \in, g)$, one can define using a formula $\psi$ in two free variables, a canonical bijection $g'$ from $\lambda$ onto $V_{\lambda}[x]$ for which $g'(y) = z \iff \psi(y, z)$. This definition goes through all formulas in a single free variable with parameters in $V_{\lambda} \cup \{g\}$, in a manner similar to how the canonical global well-ordering is coherently defined on every initial segment of $L$.

In $V$, we set $N := H(\lambda^+)$ and expand on the TCI $\mathfrak{T} = (T, \sigma, \dot{\mathcal{U}}, \vartheta)$ defined in the proof of Theorem \ref{thm9419}. To $\sigma$, we add a new constant symbol $\ulcorner g^* \urcorner$ for the interpretation of $g$, so naturally $\vartheta(\ulcorner g^* \urcorner) := (\{g\}, 0)$. For any sentence $\theta$ in the language of set theory with parameters occurring in or among $\Vec{y}$, $\Vec{b}$, $x$, and $g$, define $\theta^*$ to be the unique formula obtained from $\theta$ by a global substitution of
\begin{itemize}
...
\end{itemize}
Next, we add to $T$ the following statements (the reader should again check that they are expressible in our signature with our set constraints).
\begin{enumerate}[label=(\arabic*)]
    ...
    \item A schema involving relativisation of formulas to members of $\lambda$, among other things, that in totality gives us: for every $c \in \lambda$, if $d$ is the unique member of $\pi^{-1}(\{c\})$, then 
    \begin{equation*}
        ((R^*)^{-1}(d); \in^*, x^*) \cong (V_c[x]; \in, x)
    \end{equation*}
    for some $x$, via the Mostowski collapse.
\end{enumerate}
