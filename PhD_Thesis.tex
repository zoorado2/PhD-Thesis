...
\leavevmode
\begin{enumerate}[label=(\Roman*)]
    \item There is a $\Sigma_1$ formula $\psi_{ps}$ in four free variables in the language of set theory, such that $\psi_{ps}(r, \mathfrak{A}, T, \mathrm{Tr}(r, \mathfrak{A}, T))$ defines in $(H(\omega_1); \in)$ a ternary function $$(r, \mathfrak{A}, T) \mapsto \mathrm{Tr}(r, \mathfrak{A}, T)$$ on $H(\omega_1)$, for which whenever 
    \begin{enumerate}[label=(\alph*)]
    ...
    \begin{equation*}
        \{(f_{r^*}" (Diag(\mathfrak{A})), f_{r^*}" T, f_{r^*}" (\mathrm{Tr}(r^*, \mathfrak{A}, T))) : \psi_{ps}(r^*, \mathfrak{A}, T, \mathrm{Tr}(r^*, \mathfrak{A}, T))\}
    \end{equation*}
...
\begin{enumerate}[label=(\Roman*)]
    ...
    \begin{itemize}
        ...
        \item $(f_r" (P(\mathfrak{T}(\mathfrak{A}, T))^{\top}))'$ being uniformly $\Sigma_1$-definable in $(H(\omega_1); \in)$ from $f_r$ and $\mathfrak{T}(\mathfrak{A}, T)$, 
    ...
    \item The proof of this part is basically (implied by) a translation of the \hyperref[6330ip]{proof of (I)} via $f_{r^*}$. The reason the relation in question is not $\Sigma^1_1$ lies in how the derivation of $P(\mathfrak{T}(\mathfrak{A}, T))^{\top}$ from $(\mathfrak{A}, T)$ is represented as a real. This derivation crucially involves some possibly large countable ordinal, and verifying whether a real codes an ordinal is generally a $\Pi^1_1$ process. \qedhere
\end{enumerate}
...
What Corollary \ref{rlcor2nd} (and thus Corollary \ref{rlcor}) offers beyond what Fact \ref{lastfact} does, is a constructive method to uniformly pick out perfect set witnesses for the perfect set property of $\mathrm{Sub}(\mathfrak{A}, T)$ insofar as such witnesses exist, over all possible $\mathfrak{A}$ and $T$ within the confines of the corollary's assumptions. Moreover, said method is essentially $\Delta^1_2$-definable, and is therefore of very low complexity. However, \ref{6330ii} of Corollary \ref{rlcor2nd} should not be mistaken for new knowledge, as it is implied by classical work on hyperarithmetic sets (see e.g. Chapter III of \cite{sacks}). We merely provided an alternative --- and arguably simpler --- proof.

Precisely because Corollary \ref{rlcor} establishes a very strong version of the perfect set property, it makes sense to treat it like a souped-up variant of the Cantor-Bendixson theorem, subject to restrictions on use cases. Instead of being applicable to arbitrary closed subsets of some Polish space, Corollary \ref{rlcor} only applies to sets which contain exactly all the models of some $\Pi_2$ TCI. This treatment comes off as a natural extension of the parallels we drew in Remark \ref{CBrem}. Further extending these parallels and arguably cementing the legitimacy of calling $\mathbb{P}(\mathfrak{T})^{(\alpha)}$ ``the $\alpha$-th-order Cantor-Bendixson derivative of $\mathbb{P}(\mathfrak{T})$'', is the following fact, implicit in the proof of Theorem \ref{lastthm}.

\begin{fact}
Let $(r, f_r)$ witness $(H(\omega); \in)$ is computable and $\mathfrak{T}$ be a code-friendly $\Pi_2$ TCI. Set
\begin{align*}
    \mathrm{CL}(\mathfrak{T}) := \ & \text{the closure of } \{f_r" (\Sigma(\mathfrak{T}, \mathcal{M})) : \mathcal{M} \models^* \mathfrak{T}\} \text{ in Cantor space, and} \\
    \mathrm{Tr}(\mathfrak{T}) := \ & \text{the unique subtree } \mathrm{Tr} \text{ of } 2^{< \omega} \text{ such that } [\mathrm{Tr}] = \mathrm{CL}(\mathfrak{T}) \text{.}
\end{align*}
Then there is a canonical dense embedding $\pi$ from $\mathrm{Tr}(\mathfrak{T})$ into $\mathbb{P}(\mathfrak{T})$ such that for all ordinals $\alpha$, $\pi \restriction \mathrm{Tr}(\mathfrak{T})^{(\alpha)}$ is a dense embedding from $\mathrm{Tr}(\mathfrak{T})^{(\alpha)}$ into $\mathbb{P}(\mathfrak{T})^{(\alpha)}$, where
\begin{equation*}
    \mathrm{Tr}(\mathfrak{T})^{(\alpha)} := \text{the unique subtree } \mathrm{Tr} \text{ of } 2^{< \omega} \text{ such that } [\mathrm{Tr}] = \mathrm{CL}(\mathfrak{T})^{(\alpha)} \text{.}
\end{equation*}
\end{fact}
