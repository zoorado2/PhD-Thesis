...
Towards this end, we want to come up with a formula $\psi_{cert}$ in two free variables: $\psi_{cert}$ is intended to help us define a uniform reduction of the problem of constructing models of TCIs. We shall exhibit $\psi_{cert}$ in the proof of the next lemma, the latter of which statement also highlights some key properties of $\psi_{cert}$.

\begin{lem}\label{mcequiv}
The formula $\psi_{cert}$ can be defined such that in any model of $\mathsf{ZFC}$, 
\begin{enumerate}[label=(\alph*)]
