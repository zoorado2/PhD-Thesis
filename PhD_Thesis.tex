...
Let us now turn our attention to the relation $\leq^M$ and the local method hierarchy. For each of the remaining questions in this section, one can also ask a variant thereof in which we swop out every (explicit or otherwise) instance of the relation $\leq^M$ for an instance of the relation $\leq^N$.
...
Note that the $\leq^N$-variant of Question \ref{q843} has a negative answer. Indeed, $\{\mathfrak{T}\} \leq^N \mathsf{Fg}$ for every TCI $\mathfrak{T}$ because of Lemma \ref{conalt}.
