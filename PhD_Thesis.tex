...
\begin{align*}
    ...
    & \text{of } \mathrm{Cd}(\mathfrak{T}) \text{ as a Polish subspace of Cantor space) in Cantor space, and} \\
    \mathrm{Tr}^{(\alpha)}(\mathfrak{T}) := \ & \text{the unique subtree } \mathrm{Tr} \text{ of } 2^{< \omega} \text{ such that } [\mathrm{Tr}] = \mathrm{Cl}^{(\alpha)}(\mathfrak{T}) \text{.}    
\end{align*}
\end{defi}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item for all ordinals $\alpha$, $(f_r^{-1} \circ \pi \circ f_r) \restriction \mathrm{Tr}^{(\alpha)}(\mathfrak{T})$ is a dense embedding from $\mathrm{Tr}^{(\alpha)}(\mathfrak{T})$ into $\mathbb{P}(\mathfrak{T})^{(\alpha)}$,
\end{enumerate}
we have $\Xi^{\langle \langle e, n \rangle, x \rangle} = \langle e', x \oplus f_r" (\Gamma_{\mathfrak{T}}) \rangle$.
\end{cor}
