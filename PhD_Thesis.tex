...
\makeatother
\author{DESMOND LAU}
\title{A FEW NOTIONS OF GENERALISED COMPUTATION} 
\date{\textit{Within the Set-Theoretic Universe and Beyond}}
...
\begin{flushleft}
\textit{A Few Notions of Generalised Computation Within the Set-Theoretic Universe and Beyond} 
...
\thispagestyle{simple}
I am very grateful to have my supervisor, Dilip Raghavan, dedicate an hour a week out of his hectic schedule to my project. Your ideas for further research and advice on graduate studies are very valuable to me. 
\par 
I would like to thank Yang Yue, who taught me in my first set theory course, Kenneth Kunen, for writing such a great introductory textbook, Saharon Shelah, for publishing his wealth of ideas which culminated in this project.
\par 
I cannot thank my parents enough, for being understanding of my unusual working habits, tolerant of my absence from home, and for offering to cook for me ever so often.
\par 
To my friends -- Sudarshan, thank you for all the tea-drinking sessions; Davin and Yikjiun, our discussions about pop music and our weekly karaoke sessions were such great respites from work; Caitlin, thanks for opening my eyes to so many cool things; Ziyang and the rest of the 'Dealz' clique, you probably didn't know I was working on this, but your presence and the usual pointless banter we had never failed to brighten my day.
...
\chapter{Overview}
\thispagestyle{simple}
\emph{Just what is a computation?} This thesis seeks to give the most general answers. We first make sense of the notion of generalised computation on arbitrary sets within our conventional mathematical universe (also known as the \emph{set-theoretic universe}), before using the results thereof to justify a version of computation beyond this universe. 
...
We first analyse Gurevich's original conception of abstract algorithms through the lens of set theory. It must be said that his formal definition of a sequential algorithm is among the most general yet. However, our analysis suggests it might be \emph{too} general an abstraction of computation. More precisely, we discover that it is possible for a sequential algorithm \textit{a la} Gurevich to be \emph{inconstructible} in the sense of being outside G\"{o}del's universe $L$ (Example \ref{ex21}). This seems to go against the common intuition that computations are intrinsically constructive. We are thus morally justified to impose restrictions on what constitutes a sequential algorithm.  

The restrictions we impose are natural and well-motivated; the reader can hopefully verify this by following our thought process as detailed in Section \ref{sect82}. As a result, we obtain two classes of abstract algorithms for set computation, namely \textit{restricted sequential algorithms} (\textit{RSeqAs}, Defintion \ref{def224}) and \textit{restricted sequential algorithms with parameters} (\textit{RSeqAPs}, Definition \ref{def836}). It is straightforward to derive from each class, certain relative computability relations on sets, which are analogous to the Turing reducibility relation on reals. We focus on two such relations derived from RSeqAPs, denoted $\leq^P$ (Definition \ref{def839}) and $\leq^{P, s}$ (Definition \ref{def8329}). These relations are shown to possess nice properties not found in more popular notions of relative computability (follows from Table \ref{table3}). By leveraging on Koepke's analysis of $\alpha$-machines, we then prove that $\leq^P$ is equivalent to the relative constructibility relation in set theory (Theorem \ref{thm275}).

\subsection*{Beyond the Set-theoretic Universe}

\subsection*{Our Main Tools}

For better compartmentalisation and modularisation of knowledge and techniques, we extract the key ideas employed in the proofs of theorems about our particular model of generalised computation beyond $V$, and present them in a more general context. These ideas are packaged as technical theorems about TCIs in Chapter \ref{TCIsec}, to be applied rather straightforwardly in Chapter \ref{chap7}, when we seek answers to questions regarding the aforementioned model of computation. While technical, these theorems are also interesting in their own right, for they open up intriguing avenues for investigation within the theory of TCIs, some of which we shall briefly explore. As such, our choice of presentation allows us to kill two birds with one stone. 
...
We can, in fact, strengthen Corollary \ref{lastcor} so that when a countable structure is presented in a certain manner, the perfect set property holds for its $T$-substructures given any sufficiently simple $T$. Let us first recall some basic definitions in effective descriptive set theory.

\begin{defi}
For any $X \in 2^{\omega}$, a $\Pi^0_1(X)$ \emph{class} is the set of branches through some $X$-computable (under a canonical encoding of $2^{<\omega}$) infinite subtree of $2^{<\omega}$.
\end{defi}
...
\begin{enumerate}[label=(\arabic*)]
    \item $A$ is a $\Pi^0_1(X)$ class, 
    \item $f_r^{-1}(\Phi^X)$ is an infinite subtree of $2^{<\omega}$, and
    \item $A$ the set of branches through $f_r^{-1}(\Phi^X)$.
\end{enumerate} 
\end{defi}

\begin{fact}\label{2ndlf}
There is an oracle machine $\Theta$ such that whenever $X \in 2^{\omega}$, $\Theta^{X' \oplus X}$ is a uniformly splitting perfect tree $T$ on $2^{<\omega}$, all of which branches are $X$-$1$-generic.
\end{fact}

\begin{cor}\label{rlcor}
Let $(r, f_r)$ witness $(H(\omega); \in)$ is computable. Then there is an oracle machine $\Phi$ for which, whenever $\mathfrak{T}$ is a $\Pi_2$ TCI, one of the following must hold:
\begin{enumerate}[label=(\arabic*)]
    \item $|\{\mathcal{M} : \mathcal{M} \models^* \mathfrak{T}\}| \leq \aleph_0$.
    \item\label{rlcor2} There is a perfect subset $A$ of $2^{\omega}$ such that $$A \subset \{f_r" \mathcal{M} : \mathcal{M} \models^* \mathfrak{T}\}$$ and $\Phi$ witnesses $A$ is a $\Pi^0_1(X)$ class for some $X \in 2^{\omega}$.
\end{enumerate}
\end{cor}

\begin{proof}
Follows from a straightforward combination of Theorem \ref{lastthm} and Fact \ref{2ndlf}. Note that we can always choose $X$ in \ref{rlcor2} to be $((f_r" (P(\mathfrak{T})^{\top}))' \oplus f_r" (P(\mathfrak{T})^{\top}))$.
\end{proof}

Corollary \ref{rlcor} trivially implies the next corollary.

\begin{cor}\label{rlcor2nd}
Let $(r, f_r)$ witness $(H(\omega); \in)$ is computable. Then there is an oracle machine $\Phi$ for which, whenever 
\begin{enumerate}[label=(\alph*)]
    ...
    \item generating $\mathfrak{T}$ from $\mathfrak{A}$ and $T$ via the procedure outlined in Example \ref{2ndex} results in $\mathfrak{T}$ being code-friendly,
\end{enumerate} 
one of the following must hold:
\begin{enumerate}[label=(\arabic*)]
    \item $|\mathrm{Sub}(\mathfrak{A}, T)| \leq \aleph_0$.
    \item There is a perfect subset $A$ of $2^{\omega}$ such that $$A \subset \{f_r" \mathcal{M} : \mathcal{M} \in \mathrm{Sub}(\mathfrak{A}, T)\}$$ and $\Phi$ witnesses $A$ is a $\Pi^0_1(X)$ class for some $X \in 2^{\omega}$.
\end{enumerate}
\end{cor}
...
What Corollary \ref{rlcor2nd} (and thus Corollary \ref{rlcor}) offers beyond the results of classical descriptive set theory --- in particular, beyond Fact \ref{lastfact} --- is a constructive method to uniformly pick out perfect set witnesses for the perfect set property of $\mathrm{Sub}(\mathfrak{A}, T)$ insofar as such witnesses exist, over all possible $\mathfrak{A}$ and $T$ within the confines of the corollary's assumptions. Moreover, said method is uniformly $\Pi^0_1$ in a single real parameter, so its uniform component is of very low complexity.
...
In this chapter, we investigate the promise of Gurevich's \textit{sequential algorithms} in infinitary computations. We make well-meaning restrictions to their definition along the way, so that they are better endowed with our intuition of computation born out of classical computing. 

\section{Generalising Computability}\label{subsec22}
...
Unlike the previous models of generalised computation, Gurevich's sequential algorithms were not designed with any form of relative computability in mind. In fact, in the name of generality, his definition of an algorithm does not even specify the existence of canonical inputs or outputs, which are typically integral to associating a relative computability relation with a computational model. Further, preliminary investigations into the limits of their computational power quickly uncover something philosophically unsatisfying, if not unsettling. We seek to correct this with our restrictions to the definition of a sequential algorithm.

The way restrictions are added to the notion of sequential algorithms results in a new model of generalised computation called \textit{restricted sequential algorithms with parameters} (abbreviated as \textit{ReqSAPs}). The main draw of RSeqAPs is that they possess more concrete guardrails than the original sequential algorithms without compromising too much of the latter's intuitiveness. Coupled with an input/output paradigm, RSeqAPs naturally induce a relative computability relation, $\leq^P$, on all sets of ordinals. It turns out that $\leq^P$ coincides with both $\preceq_A$ as well as the \textit{relative constructibility} relation. Unlike $\preceq_A$ though, $\leq^P$ tends very much to remain transitive under computational constraints. This makes for interesting comparisons between $\leq^P$ and say, the relations in Table \ref{table1}, when size limitations are imposed.

\section{Restricting Sequential Algorithms}\label{sect82}

In this section, we progressively chip away at the very useful, but very inclusive, definition of a sequential algorithm, to arrive at a more restrictive notion of algorithms which also better captures our intuition of generalised computation. 

\subsection{Sequential Algorithms}

Gurevich introduced the \textit{ASM thesis} in \cite{gurevich} as a justification of correctness for his definition of (a most general conceptualisation of) a sequential algorithm. He followed it up with a number of collaborations and papers (e.g. \cite{gurevichblass}) on various other classes of algorithms. If we follow the convention of using ``algorithm'' and ``computer program'' interchangeably, then sequential algorithms are natural candidates for capturing a general notion of computability. However, the claim that Gurevich's notion of sequential algorithms is natural seems to have undergone insufficient scrutiny on an infinitary scale. We shall start this section by refuting this claim through a set-theoretic argument. 

Briefly, a \emph{sequential algorithm} --- henceforth \textit{SeqA}, to distinguish the formal concept from the colloquial --- $\mathfrak{M}$ comprises a class of states $S(\mathfrak{M})$, a class of initial states $I(\mathfrak{M})$, and a class transition function $\tau_{\mathfrak{M}} : S(\mathfrak{M}) \longrightarrow S(\mathfrak{M})$ such that
...
For brevity's sake, some details are withheld here. First, Gurevich originally require states to have only function and constant symbols in their signatures. However, this restriction is purely cosmetic because every relation in a structure can be represented as a boolean function. Second, $\tau$ is supposed to satisfy the \emph{bounded exploration postulate}, which vaguely means that $\tau$ must perform the same updates to all states with the same interpretations of ground terms in a particular finite set. We shall keep this requirement (or at least its spirit) in mind for this subsection, even if we need not formally verify it. Finally, our definition above can be generalised so that $\tau_{\mathfrak{M}}$ need only be a binary relation -- resulting in algorithms which might not be sequential -- but that is unnecessary for the point we seek to put across.
...
We argue against SeqAs as a good notion of generalised algorithms. Consider the example below.

\begin{ex}\label{ex21}
Let $\kappa$ be an infinite cardinal. Expand the signature of $(\kappa; \in)$ to include a unary relation symbol $\ulcorner A \urcorner$ and a constant symbol $\ulcorner c \urcorner$. Now if $A_1$ and $A_2$ are subsets of $\kappa$, then we can define a SeqA $\mathfrak{M}(\kappa, A_1, A_2)$ as follows:
\begin{gather*}
    s_1 := (\kappa; \in, A_1, 0) \\
    s_2 := (\kappa; \in, A_2, 1) \\
...
For much of the rest of this section, we will make educated modifications to the definition of a SeqA, in an attempt to formulate a more philosophically justified notion of algorithms, at least from a set-theoretic viewpoint.
...
Before adding more requirements to the transition function of a SeqA, let us enforce that every state in a SeqA $\mathfrak{M}$ must have the same base set $b(\mathfrak{M})$. This can be done without loss of generality in the original definition of a SeqA, because we can extend the base set of each state in $\mathfrak{M}$ to be the union of the base sets of states in $\mathfrak{M}$, while arbitrarily extending the interpretation of function and relation symbols in the signature by each state.
...
As TCIs impose natural constraints on first-order theories, they can be easily adapted to constrain the possible states of a SeqA. 
...
A \emph{restricted sequential algorithm} --- henceforth \textit{RSeqA} --- $\mathfrak{M}$ is fully specified by a limit ordinal $\kappa(\mathfrak{M})$, a finite signature $\sigma(\mathfrak{M})$, and sentences $\phi^{\tau}_{\mathfrak{M}}$ and $\phi^d_{\mathfrak{M}}$ such that
...
Fix any limit ordinal $\kappa$. Notice that there are only countably many RSeqAs $\mathfrak{M}$ satisfying $\kappa(\mathfrak{M}) = \kappa$. Indeed, this countable set of RSeqAs specifications is absolute for transitive models of $\mathsf{ZFC}$ containing $\kappa$.
\end{rem}

\begin{defi}
If $\mathfrak{M} = (\kappa(\mathfrak{M}), \sigma(\mathfrak{M}), \phi^{\tau}_{\mathfrak{M}}, \phi^d_{\mathfrak{M}})$ is an RSeqA, define 
\begin{enumerate}[label=(\arabic*)]
...
The next proposition highlights the important properties of RSeqAs, based on what we have discussed so far.

\begin{prop}\label{prop226}
Let $\mathfrak{M}$ be an RSeqA. Then the following are absolute for transitive models of $\mathsf{ZFC}$ containing $\mathfrak{T}(\mathfrak{M})$:
\begin{enumerate}[label=(\arabic*)]
...
Now that RSeqAs have been officially defined, it is imperative to revisit Example \ref{ex212}, for any notion of generalised algorithm that cannot represent an arbitrary Turing cannot be too useful.

\begin{ex}[\ref{ex212} revisited]\label{ex228}
Let us describe changes to the $\mathfrak{M}$ defined in Example \ref{ex212} that would qualify it as an RSeqA. First, an observation: there are formulas $\phi_P$ and $\phi_S$ over $\ulcorner \in \urcorner$, each with two free variables $x$ and $y$, such that for all $s \in S(\mathfrak{M})$,
\begin{equation*}
...
A rotuine check would reveal that the aforementioned details are sufficient to specify $\mathfrak{M}$ as an RSeqA, and that they allow $\mathfrak{M}$ to accurately simulate the Turing machine $M$. 
\end{ex}

From the next subsection onwards, the construction of RSeqAs used in proofs and arguments can be too cumbersome to formally spell out. In such cases, only high-level details will be given. The reader should always try to convince themselves that those details can be properly realised by RSeqAs.

\subsection{Transfinite Runs}

Consider a finite terminating run $\Vec{s} = (s_1, \dots, s_n)$ of an RSeqA $\mathfrak{M}$. Then by combining witnesses to $\phi^{\tau}_{\mathfrak{M}}$ being bounded for $\sigma(\mathfrak{M})$, we can conjure witnesses to some sentence $\phi$ being bounded for $\sigma(\mathfrak{M})$, such that using $\phi$ in place of $\phi^{\tau}_{\mathfrak{M}}$ in the definition of $\mathfrak{M}$ gives us an RSeqA $\mathfrak{M}'$ with $\tau_{\mathfrak{M}'}(s_1) = s_n$. Therefore, as far as computability is concerned, allowing finite steps of arbitrary sizes in computations is no more powerful than allowing only one-step computations. This goes to show that
RSeqAs are in fact built for runs of infinite lengths.

Given an infinite ordinal $\delta$, what does it mean to have an RSeqA computation of length $\delta$? Naturally, every state of the computation must depend on the states prior. For successor ordinals $\alpha < \delta$, the $\alpha$-th state of the computation depends solely on its unique previous state, according to the transition function of the RSeqA. But what about limit ordinals less than $\delta$? How do we describe the states associated with these ordinals in terms of states occurring in the computation before them? This is equivalent to asking how we are going to process the limit of a sequence of states.

Fix an RSeqA $\mathfrak{M}$, a limit ordinal $\delta$, and let $Sq$ be a sequence of states of $\mathfrak{M}$. We want to have a uniform definition of $lim(Sq) \in S(\mathfrak{M})$. One natural way to do so is to take ordinal limits point-wise (according to the order topology on $\kappa(\mathfrak{M})$ induced by $\in$), while treating relations as boolean functions. The obvious hindrance to this method is, the limit of a sequence of ordinals does not always exist. However, for various purposes, there are suitable ersatzes.
...
\begin{defi}
Let $\mathfrak{M}$ be an RSeqA and $Sq$ be a function from some ordinal into $S(\mathfrak{M})$. Given $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ an $n$-ary function symbol, denote by $\ulcorner R^{\ulcorner X \urcorner} \urcorner$ a distinguished $(n+2)$-ary relation symbol and define the relation 
\begin{equation*}
...
\begin{defi}
Let $\mathfrak{M}$ be an RSeqA. Given $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ an $n$-ary function symbol, we say $\phi$ is a \emph{simple limit formula for} $(\ulcorner X \urcorner, \mathfrak{M})$ iff $\phi$ is a formula over $\{\ulcorner \in \urcorner, \ulcorner R^{\ulcorner X \urcorner} \urcorner\}$ with free variables $x_1, \dots, x_{n+1}$.
\end{defi}

\begin{defi}\label{def234c}
Let $\mathfrak{M}$ be an RSeqA. Given $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ an $n$-ary function symbol and $\phi$ a \emph{simple limit formula for} $(\ulcorner X \urcorner, \mathfrak{M})$, we say $\phi$ \emph{preserves ordinal limits} iff interpreting $\ulcorner \in \urcorner$ as $\in$ and $\ulcorner R^{\ulcorner X \urcorner} \urcorner$ as $R_{Sq}^{\ulcorner X \urcorner}$, 
\begin{equation*}
...
\begin{proof}
By the definition of an RSeqA and the observation that 
\begin{equation*}
    lim \ inf (T) = lim (T)
\end{equation*}
...
Colloquially, Proposition \ref{prop233} tells us that every RSeqA is \emph{strongly closed under limit inferiors}. The following definitions generalise this concept.

\begin{defi}\label{def237}
Let $\mathfrak{M}$ be an RSeqA. Then $\mathfrak{M}$ is \emph{strongly closed under} a set $Y$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$, such that 
\begin{equation*}
...
Let $\mathfrak{M}$ be an RSeqA. Then $\mathfrak{M}$ is \emph{practically closed under} a set $Y$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$, such that 
\begin{equation*}
    Y = \{\phi_{lim}^{\ulcorner X \urcorner} : \ulcorner X \urcorner \in \sigma(\mathfrak{M})\}
\end{equation*}
...
Clearly, for any RSeqA $\mathfrak{M}$ and any set $Y$,
\begin{align*}
    \mathfrak{M} \text{ is strongly closed under } Y \implies \mathfrak{M} \text{ is practically closed under } Y \text{.}
\end{align*}

We are ready to define transfinite terminating runs of RSeqAs.

\begin{defi}
Let $\mathfrak{M}$ be an RSeqA that is practically closed under $Y$. A \emph{short terminating run of} $(\mathfrak{M}, Y)$ \emph{with input} $A$ \emph{and output} $B$ is a function $Sq$ from some successor ordinal $\delta < \kappa(\mathfrak{M})$ into $S(\mathfrak{M})$ such that 
\begin{enumerate}[label=(\alph*)]
...
Suppose $\mathfrak{M}$ is an RSeqA practically closed under $Y$. Then for some signature $\sigma'$, as long as $\mathfrak{M}'$ is an RSeqA satisfying 
\begin{itemize}
    \item $\sigma(\mathfrak{M}') = \sigma'$ and
    \item $\kappa(\mathfrak{M}') = \kappa(\mathfrak{M})$,
\end{itemize}
and $Z$ is a set of simple limit formulas preserving ordinal limits with $\mathfrak{M}'$ strongly closed under $Z$, there is an RSeqA $\mathfrak{M}^*$ such that
\begin{enumerate}[label=(\alph*)]
...
\begin{align*}
    \mathrm{SRuns}_{\kappa}(A, B) := \{(\mathfrak{M}, Sq) : \ & \mathfrak{M} \text{ is an RSeqA with } \kappa(\mathfrak{M}) = \kappa \text{ and } Sq \text{ is a short} \\
    & \text{terminating run of } \mathfrak{M} \text{ with input } A \text{ and output } B\}
\end{align*}
and
\begin{align*}
    \mathrm{Runs}_{\kappa}(A, B) := \{(\mathfrak{M}, Sq) : \ & \mathfrak{M} \text{ is an RSeqA with } \kappa(\mathfrak{M}) = \kappa \text{ and } Sq \text{ is a} \\
    & \text{terminating run of } \mathfrak{M} \text{ with input } A \text{ and output } B\}
\end{align*}
as defined are absolute for transitive models of $\mathsf{ZFC}$ containing $A, B$ and $\kappa$. 

\begin{rem}\label{rem243}
For a limit ordinal $\kappa$ and two sets of ordinals $A$ and $B$, that $\mathrm{Runs}_{\kappa}(A, B)$ is a set and not a proper class stems from the fact that by our definition of terminating runs, $Sq$ must be an injection into $S(\mathfrak{M})$ whenever $(\mathfrak{M}, Sq) \in \mathrm{Runs}_{\kappa}(A, B)$. The aforementioned fact also means that all terminating runs of an RSeqA $\mathfrak{M}$ must be of length $< (2^{\kappa(\mathfrak{M})})^+$.
\end{rem}

\section{Computability and Comparisons}

In this section, we derive from our restricted sequential algorithms, various notions of generalised (relative) computability. We then compare these notions with existing ones.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item for some RSeqA $\mathfrak{M}$ with $\kappa(\mathfrak{M}) = \kappa$, there is a terminating run of $\mathfrak{M}$ with input $A$ and output $B$. 
\end{enumerate}
...
It makes sense that a machine is more powerful when endowed with more space. This might not quite be the case for RSeqAs, but we can get close to this sort of monotonicity with a mild assumption.
...
\end{equation*}
for all limit ordinals $\kappa$ and $B \subset \kappa$. An ideal relative computability relation uniformly definable in $\kappa$ should have its predecessor sets grow along with $\kappa$. Towards such a relation, we shall introduce an augmentation to RSeqAs.

\begin{defi}\label{def836}
An \emph{RSeqA with parameters} (henceforth \emph{RSeqAP}) 
\begin{equation*}
    \mathfrak{M} = (\kappa(\mathfrak{M}), \sigma(\mathfrak{M}), \phi^{\tau}_{\mathfrak{M}}, \phi^d_{\mathfrak{M}}, \vartheta_{\mathfrak{M}})
\end{equation*} 
is defined like an RSeqA, except constants in $\sigma(\mathfrak{M})$ are allowed to evaluate to $(\{\alpha\}, 1)$ under $\vartheta_{\mathfrak{M}}$ for any $\alpha \in \kappa(\mathfrak{M})$. In this case, we call $\alpha$ a \emph{parameter} of $\mathfrak{M}$.
\end{defi}

\begin{rem}\label{rem248}
Fix any limit ordinal $\kappa$. There are $|\kappa|$ many RSeqAPs $\mathfrak{M}$ satisfying $\kappa(\mathfrak{M}) = \kappa$. Also, this set of RSeqAP specifications is absolute for transitive models of $\mathsf{ZFC}$ containing $\kappa$ (cf. Remark \ref{rem225}).
\end{rem}

The definitions and properties so far associated with RSeqAs adapt perfectly to RSeqAPs. We will use the adapted definitions in the context of RSeqAPs without further clarifications.
...
\begin{enumerate}[label=(\alph*)]
    \item $A, B \subset \kappa$, and
    \item for some RSeqAP $\mathfrak{M}$ with $\kappa(\mathfrak{M}) = \kappa$, there is a terminating run of $\mathfrak{M}$ with input $A$ and output $B$. 
\end{enumerate}
...
Let $\mathfrak{M}$ be an RSeqAP and $Sq$ be a function from a limit ordinal into $S(\mathfrak{M})$. A state $s \in S(\mathfrak{M})$ is a \emph{simple limit of} $Sq$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$ such that, interpreting $\ulcorner \in \urcorner$ as $\in$ and $\ulcorner R^{\ulcorner X \urcorner} \urcorner$ as $R_{Sq}^{\ulcorner X \urcorner}$, 
\begin{equation*}
...
Let $\mathfrak{M}$ be an RSeqAP. Then $\mathfrak{M}$ is \emph{strongly closed under} a set $Y$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$, such that 
\begin{equation*}
    Y = \{\phi_{lim}^{\ulcorner X \urcorner} : \ulcorner X \urcorner \in \sigma(\mathfrak{M})\}
\end{equation*}
...
Let $\mathfrak{M}$ be an RSeqAP. Then $\mathfrak{M}$ is \emph{practically closed under} a set $Y$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$, such that 
\begin{equation*}
    Y = \{\phi_{lim}^{\ulcorner X \urcorner} : \ulcorner X \urcorner \in \sigma(\mathfrak{M})\}
\end{equation*}
...
Note that Definitions \ref{def257p} and \ref{def258} are almost word-for-word the same as Definitions \ref{def237} and \ref{def238}. This only changes made are \begin{itemize}
    \item substituting ``RSeqAP'' for ``RSeqA'' and 
    \item removing the requirement for $\delta$ to be $< \kappa(\mathfrak{M})$.
\end{itemize} 

\begin{prop}\label{prop261c}
Proposition \ref{prop233} holds with ``RSeqAP'' substituted for ``RSeqA'' and the requirement for $\delta$ to be $< \kappa(\mathfrak{M})$ removed.
\end{prop}

\begin{proof}
Any formula that works for all RSeqAs $\mathfrak{M}$ with $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ in the context of proving Proposition \ref{prop233} (such a formula exists by the last sentence of the proof of Proposition \ref{prop233}) also works here.
\end{proof}

\begin{prop}\label{prop261}
Suppose $\mathfrak{M}$ is an RSeqAP practically closed under $Y$. Then for some signature $\sigma'$, as long as $\mathfrak{M}'$ is an RSeqAP satisfying
\begin{itemize}
    \item $\sigma(\mathfrak{M}') = \sigma'$ and
    \item $\kappa(\mathfrak{M}') = \kappa(\mathfrak{M})$,
\end{itemize}
and $Z$ is a set of simple limit formulas preserving ordinal limits with $\mathfrak{M}'$ strongly closed under $Z$, there is an RSeqAP $\mathfrak{M}^*$ such that
\begin{enumerate}[label=(\alph*)]
...
\begin{itemize}
    \item Remark \ref{rem243} remains true with ``$\mathrm{RunsP}_{\kappa}(A, B)$'' substituted for ``$\mathrm{Runs}_{\kappa}(A, B)$'' and ``RSeqAP'' substituted for ``RSeqA'', and 
...
Let $\kappa$ be a limit ordinal. We say $B$ is $\kappa$\emph{-computable with parameters from} $A$ \emph{with short runs}, or $B \leq^{P, s}_{\kappa} A$, if there is an RSeqAP $\mathfrak{M}$ witnessing $B \leq^P_{\kappa} A$ and a short terminating run of $\mathfrak{M}$ with input $A$ and output $B$. Here, $\mathfrak{M}$ is said to witness $B \leq^{P, s}_{\kappa} A$.
\end{defi}
...
Let $\alpha$ be a limit ordinal. An $\alpha$-machine $M$ can be simulated by an RSeqAP $\mathfrak{M}$ in the sense that for any $A, B \subset \alpha$, $M$ halts on input $A$ with output $B$ iff $\mathfrak{M}$ has a short terminating run with input $A$ and output $B$.
\end{prop}

\begin{proof}
It is not difficult to check that modifying Example \ref{ex228} by substituting $\alpha$ for $\omega$, and adding oracle reads based on how the oracle is coded into the input (through e.g. adding a third tape), works out just fine. The finitely many ordinal parameters of an $\alpha$-machine can be ported wholesale to an RSeqAP as the latter's ordinal parameters; the way an RSeqAP determines the limit states of a computation also perfectly mimicks how limit configurations of an $\alpha$-machine are defined. Moreover, an $\alpha$-machine halting on input $A$ with output $B$ is equivalent to the RSeqAP simulating it having a short terminating run with input $A$ and output $B$.
\end{proof}
...
\begin{proof}
Let $\mathfrak{M}$ be an RSeqAP with parameter set $K$ witnessing $B \leq^{P, s}_{\kappa} A$, and $Sq$ be a short terminating run of $\mathfrak{M}$ with input $A$ and output $B$. Suppose we want to find out whether some $\beta < \kappa$ is a member of $B$. To do so, we draw up a dependency tree of terms of which interpretations among members of $ran(Sq)$ are needed to decide if $\beta \in B$. This tree is finitely branching by the bounded exploration postulate, and it has number of levels no more than $dom(Sq)$. By the regularity of $\kappa$, it has size $< \kappa$. 
...
\begin{proof}
By Fact \ref{fact272}, $B$ is $\kappa$-computably enumerable in $A$, as witnessed by some $\kappa$-machine $M$. Let $\mathfrak{M}$ be an RSeqAP simulating $M$ as per Proposition \ref{prop270}. We define an RSeqAP $\mathfrak{M}'$ that, when given input $A$, dovetails and runs $M$ through $\mathfrak{M}$ on each input in
\begin{equation*}
...
Lemmas \ref{lem269} and \ref{lem273} reflect the difference in computability between considering all terminating runs of RSeqAPs and considering only short terminating runs, for regular ordinal tape lengths. In tandem, the lemmas also give us a nice characterisation of the reducibility relation $\leq^P$.
...
\begin{enumerate}[label=(GCT\arabic*), leftmargin=50pt]
    ...
    \item a ``high-level'' notion of abstract algorithms such as RSeqAPs, and
...
Just as practical implementations of finitary SeqAs do not aim to model algorithmic intuition beyond the limits of Church-Turing thesis, RSeqAPs should not allow for algorithms that are non-constructible. We can view RSeqAPs as a paradigm for easy high-level descriptions of algorithms on arbitrary sets, one that gives the user enough structure to organise their ideas, and serves as guardrails to rule out clearly non-algorithmic descriptions.
...
\begin{proof}
For all forcing notions $\mathbb{P}$, the TCI $\mathfrak{T}(\mathbb{P})$ is always a member of $\mathsf{\Pi^M_2}$ by the proof of Theorem \ref{revgenmodels} (and thus of Proposition \ref{prop323}). That $\mathfrak{T}(\cdot)$ is definable in $V$ makes it a witness to $\mathsf{Fg} \leq^M \mathsf{\Pi^M_2}$. Lemma \ref{prop252'} then implies $\mathsf{Fg} \leq^M \mathsf{\Sigma^M_1}$.
\end{proof}
...
\end{equation*}
By Lemma \ref{prop252'} and Proposition \ref{prop343}, we can conclude the following:
\begin{enumerate}[label=(\arabic*)]
    \item $\mathcal{C} \subsetneq \{\mathfrak{T} : \mathfrak{T} \text{ is a } \Pi_2 \text{ TCI}\}$, and
