...
\section{Extending the Namba Extension Problem}
...
For the set $C$ defined in the proof of Theorem \ref{notion1}, the sequence $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ will play a crucial role in our solution to a higher analogue of the Namba extension problem.
...
\begin{align*}
    ...
    & P \subset H(\lambda), \\
    & \mathbb{P} \text{ adds no } \omega \text{-sequences of ordinals,} \\
...
\begin{proof}
We leave it to the reader to check that, under our hypotheses, the entire argument after Lemma \ref{lem26} in the proof of Theorem \ref{notion1} goes through without a hitch should we employ the ``natural'' substitutions of
\begin{enumerate}[label=(\arabic*)]
...
\begin{itemize}
    ...
    \item $\dot{f}$ be a $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$-name of function from $\omega$ into $ORD$, 
...
\end{equation*}
Such an $r_n$ can be found because of \ref{7445}. Assign $p_{n+1}$ to be $r_n$ and $B_{n+1}$ to be any set extending $B_n$ that satisfies \ref{744iii}. From this construction, and in particular, from its implication
\begin{align*}
    \forall n < \omega \ \exists \beta \ \exists \delta \ \exists \lambda \ \exists D \ (& B_n(n) = (\delta, \lambda, D) \text{, } p_{n+1} \Vdash_{\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)} \dot{f}(n) = \beta \text{ and} \\
    & [p_{n+1}]^{< \omega_1} \cap X_{\delta, \lambda}(p_{n+1}) \cap D \neq \emptyset)
\end{align*}
as well as the (predefined target) properties \ref{744i} to \ref{744iv}, we observe that 
\begin{equation*}
    p^* := \bigcup \{p_n : n < \omega\}
\end{equation*}
is a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ extending $p$ that forces $\dot{f}$ to take on a value in $V$. As $p$ is an arbitrary condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$, the usual density argument tells us that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ adds no $\omega$-sequences of ordinals.
\end{proof}

\begin{thm}
Let $\kappa$, $\lambda^{\dagger}$ be regular cardinals such that $\omega_1 < \kappa < \lambda^{\dagger}$ and $\kappa$ is measurable. Then there is a forcing notion of size $2^{< \lambda}$ witnessing $GNb_1'(\kappa, \lambda^{\dagger})$.
\end{thm}
...
\end{enumerate}
As a result, the hypotheses of Theorem \ref{thm744} are fulfilled in all $Col(\lambda^{\dagger}, \lambda^{\dagger})$-generic extensions $V[h]$ of $V$. Furthermore, for every such $V[h]$, Lemma \ref{lem26} holds after we replace
\begin{itemize}
...
Recall our augmentation of the definition of $\{\mathbb{P}_{\lambda} : \lambda \in C \cup \lambda^{\dagger}\}$ in the proof of Theorem \ref{notion1} to obtain the sequence $\{\mathbb{P}^{< \omega_1}_{\lambda} : \lambda \in C \cup \lambda^{\dagger}\}$. What prevents us from adapting this augmentation process to ``supports'' of greater cardinalities? For one, \ref{cs2} no longer holds if we replace the sole instance of $\omega_1$ therein with a larger cardinal. Whether this is a fundamental obstacle to our cause can be formalised as the following question.
