...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item\label{9258} for all TCIs $\mathfrak{T}$, $\mathfrak{T}$ is consistent iff $$\Vdash_{Col(\omega, \lambda)} \exists x \ \psi'(\mathfrak{T}, x) \text{,}$$ where $\lambda$ is any cardinal $\geq |H(|trcl(\mathfrak{T})|^+)|$.
\end{enumerate}
\end{prop}
...
In practice, where forcing is used predominantly within the context of consistency proofs, Theorem \ref{thm9415} indicates that there is no better tool than forcing to define a class of small extensions of $V$ through their generators, if we want this definition to be sufficiently local. 
