...
\begin{enumerate}[label=(\arabic*)]
    ...
    \end{gather*}
\end{enumerate}
Obviously, $\leq^M_w \subset \leq^N_w$ and $\leq^M \subset \leq^N$ as partial orders on local method definitions. In fact, it can be easily verified that the identity map is both a dense embedding of $\leq^M_w$ into $\leq^N_w$ and a dense embedding of $\leq^M$ into $\leq^N$. As the results in this and the next sections are positive with respect to the relation $\leq^M$, they also hold should we substitute $\leq^N$ for $\leq^M$ when stating them.
\end{rem}
