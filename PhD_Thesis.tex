...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item\label{9258} for all TCIs $\mathfrak{T}$, $\mathfrak{T}$ is consistent iff $$\Vdash_{Col(\omega, \lambda)} \exists x \ \psi'(\mathfrak{T}, x) \text{,}$$ where $\lambda \geq |H(|trcl(\mathfrak{T})|^+)|$.
\end{enumerate}
\end{prop}
