...
Also natural is the question of whether we can swop out the precipitous ideal on $\omega_1$ for a measurable cardinal and still derive a close analogue of Theorem \ref{notion1}. If we were to allow the collapse of all cardinals strictly between $\omega$ and a measurable cardinal $\kappa$, it would be much more economical to reduce the hypotheses to that of Theorem \ref{notion1} by by first collapsing $\kappa$ to $\omega_1$ and obtaining that $\mathrm{NS}_{\omega_1}$ is precipitous. Preserving both $\omega_1$ and $\kappa$ would require substantial innovation, because although
\begin{enumerate}[label=(\alph*)]
    \item the Asper\'{o}-Schindler side condition method is the only known non-trivial way to ensure the preservation of $\omega_1$ that plays well with the general machinery of forcing with language fragments, and
    \item employing the Asper\'{o}-Schindler side condition method to preserve $\omega_1$ requires the existence of ample non-trivial definable elementary embeddings of $V$ into definable inner models, all with critical point $\omega_1^V$, from the perspective of some outer model of $V$, 
\end{enumerate}
yet
\begin{enumerate}[label=(\alph*)]
    \setcounter{enumi}{2}
    \item the existence of even a single outer model of $V$ in which there is a definable non-trivial elementary embedding of $V$ with critical point $\omega_1^V$ into a definable inner model, is not known to follow from the existence of a measurable cardinal. 
\end{enumerate} 
