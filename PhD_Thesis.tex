...
\chapter{Overview}
\thispagestyle{simple}
(Describe the overview of this thesis following the points outlined in Figure \ref{overview}.)

For better compartmentalisation and modularisation of knowledge and techniques, we extract the ideas used in our proofs about generalised computation beyond $V$, and present them in a more general context. These ideas are packaged as technical theorems about TCIs, to be applied in the later part of the thesis, when we seek to answer certain questions regarding the aforementioned model of computation. While technical, these theorems are also interesting in their own right, for they open up intriguing avenues for investigation within the theory of TCIs, some of which we briefly touch upon. As such, our choice of presentation allows us to kill two birds with one stone.
...
Chapters \ref{sect15} to \ref{chap4} lay the technical foundation for the rest of the paper. Potential philosophical and meta-theoretic concerns are addressed, prerequisite knowledge highlighted, and background readings recommended. Important definitions and conventions are made explicit, especially those that are more niched, and those that lack community consensus. Consequentially, the topics related to forcing and generic iterations --- found in Chapters \ref{subs24} and \ref{chap4} --- are given more careful and comprehensive treatments than the others. All chapters subsequent to Chapters \ref{sect15} and \ref{subs24} depend on them, whereas the material in Chapter \ref{chap4} is only referenced in Section \ref{ss43}.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item A natural number is a member of $\omega$. In other words, we view $\omega$ as the set of all natural numbers.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item To follow up on \ref{ctc}, every binary string $\sigma \in 2^{< \omega}$ represents a (unique) natural number, and we write $\langle \sigma \rangle$ to mean the natural number $\sigma$ represents.
...
\begin{equation*}
    \Phi^A = \{\langle \sigma \rangle : \sigma \in X\} \text{.}
\end{equation*}
\end{defi}
...
\begin{defi}
For $X \subset 2^{< \omega}$, we say $X$ is a \emph{tree} iff $X$ is closed under predecessors with respect to the subset relation. More formally, $X$ is a tree iff whenever $\sigma \in X$ and $\tau \in 2^{< \omega}$, 
\begin{equation*}
    \tau \subset \sigma \implies \tau \in X \text{.}
\end{equation*}
\end{defi}

\begin{defi}
Let $X \subset 2^{< \omega}$ be a tree. Then $\sigma$ is a splitting node in $X$ iff $\sigma^\frown (0) \in X$ and $\sigma^\frown (1) \in X$.
\end{defi}

\begin{defi}
A set $X \subset 2^{< \omega}$ is a \emph{perfect tree} iff for every $\sigma \in X$ there is $\tau$ such that $\sigma \subset \tau$ and $\tau$ is a splitting node in $X$.

A set $X \subset 2^{< \omega}$ is a \emph{uniformly splitting tree} iff for every splitting node $\sigma$ in $X$ and every $\tau \in X$, 
\begin{equation*}
    |\sigma| = |\tau| \implies \tau \text{ is a splitting node in } X \text{.}
\end{equation*}
\end{defi}
...
\begin{enumerate}[label=(\arabic*)]
    \item For some $n < \omega$, $A \restriction n \in X_{\phi}$, where $$X_{\phi} := \{\sigma \in 2^{< \omega} : \mathbb{N} \models \phi[x \mapsto \langle \sigma \rangle\} \text{.}$$
    \item For some $n < \omega$, whenever $A \restriction n \subset \sigma \in 2^{< \omega}$, $\sigma \not\in X_{\phi}$.
\end{enumerate}
\end{defi}
...
\section{Miscellaneous Properties of Forcing Notions}
...
\section{Restricting Abstract State Machines}
...
As TCIs impose natural constraints on first-order theories, they can be easily adapted to constrain the possible states of an ASM. 
...
\begin{enumerate}[label=(C\arabic*)]
    ...
    \item\label{c4pr} Points \ref{c2pr} and \ref{c3pr} guarantee that whenever $\mathfrak{M}$ is an ASM with $\sigma(\mathfrak{M}) = \sigma$, the statement
    \begin{equation*}
...
Within the context of Turing computability, a countable set $A$ is computable iff there is a computable subset of $\omega$ coding an isomorphic copy of $A$. By Lemma \ref{setcode}, every set can be similarly coded as a subset of a large enough limit ordinal. It makes sense then to simplify things by requiring the uniform base set of an ASM $\mathfrak{M}$ to be a limit ordinal $\kappa(\mathfrak{M})$. To successfully decode a set of ordinals using the G\"{o}del pairing function, we also need the true membership relation to be a part of each state of $S(\mathfrak{M})$. In other words, $\ulcorner \in \urcorner \in \sigma(\mathfrak{M})$, and whenever $s \in S(\mathfrak{M})$, $s$ interprets $\ulcorner \in \urcorner$ as the true membership relation so that $s$ an expansion of the structure $(\kappa(\mathfrak{M}); \in)$.
...
Choose a limit ordinal $\kappa$ and $A \subset \kappa$. Let $\mathfrak{M}$ be an ASM with 
\begin{gather*}
...
According to what is shown in Chapter \ref{chap6}, there is an intuitive notion of generalised computation, definable within the set-theoretic universe $V$, that canonically partitions sets into their degrees of constructibility. This lends credence to the belief that generative power over a model of set theory is a surrogate measure of computational power. When dealing with degrees of constructibility, the relevant model of set theory is G\"{o}del's constructible universe $L$, an inner model of $V$. Switching out $L$ for larger inner models makes sense for coarser degree structures.
...
Just like how a formula in the language of arithmetic can be used to pick out a Turing degree, we want to have a TCI in $V$ pick out a degree of small extension of $V$. However, it would be absurd to expect any such TCI to isolate a single such degree, due to the non-constructive nature of outer models. As a result, there is no avoiding fuzziness when we evaluate TCIs to obtain degrees. This is the intuition behind $\mathrm{Eval}^V$ naturally taking TCIs to sets, instead of taking TCIs to points.
