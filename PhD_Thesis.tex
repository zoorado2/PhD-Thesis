...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item for all ordinals $\alpha$, $(f_r^{-1} \circ \pi^*(e, n, x) \circ f_r) \restriction \mathrm{Tr}^{(\alpha)}(A^{\phi}_x)$ is a dense embedding from $\mathrm{Tr}^{(\alpha)}(A^{\phi}_x)$ into $\mathbb{P}(\mathfrak{T}^*(e, n, x))^{(\alpha)}$,
