...
\begin{enumerate}[label=(\alph*)]
    \item\label{82161} $\phi^{\ulcorner X \urcorner}$ is a $\Delta_0$ formula in the L\'{e}vy hierarchy (see Definition \ref{def27}) over $\{\ulcorner Y_0 \urcorner : \ulcorner Y \urcorner \in \sigma\}$ ($\subset 2 \cdot \sigma$), each of which atomic formulas contains at most one function symbol,
...
\begin{enumerate}[label=(C\arabic*)]
    ...
    \item In \ref{82161} of Definition \ref{def220}, we require that each atomic formula occurring in $\phi^{\ulcorner X \urcorner}$ contains at most one function symbol to prevent possible unbounded ``searches'' for existential witnesses, since e.g.
    \begin{align*}
        \ulcorner \forall x \ (X_1(x) = Y_1(x) \iff & \ \exists y \ (\phi^{\ulcorner X \urcorner}(x, y) \wedge \phi^{\ulcorner Y \urcorner}(x, y))) \urcorner \text{ and} \\
        \ulcorner \forall x \ (X_1(Y_1(x)) = y \iff & \ \exists z \ (\phi^{\ulcorner X \urcorner}(z, y) \wedge \phi^{\ulcorner Y \urcorner}(x, z))) \urcorner
    \end{align*}
    are both logically implied by the conjunction of $\psi^{\ulcorner X \urcorner}$ and $\psi^{\ulcorner Y \urcorner}$ (see \ref{82163} of Definition \ref{def220}), for any two unary function symbols $\ulcorner X \urcorner, \ulcorner Y \urcorner \in \sigma$.
...
\begin{prop}
For any GSeqA $\mathfrak{M}$, $\phi^{\tau}_{\mathfrak{M}}$ can be rewritten as a $\Delta_0$ formula over $\{\ulcorner \in_0 \urcorner\}$.
\end{prop}

\begin{proof}
By \ref{8222d5} to \ref{8222d8} of Definition \ref{def224}, as well as the definitions of being bounded and being simple for $\sigma(\mathfrak{M})$. In particular, notice how \ref{simpa} in the definition of being simple for $\sigma(\mathfrak{M})$ combines with \ref{82161} of Definition \ref{def220}.
\end{proof}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item $c_0$ first returns to $0$ in a state other than an initial state after exactly $\alpha + 1$ many steps, at which point $M$ would have been simulated on every input in 
    \begin{equation*}
        \{\langle \{\beta\}, I \rangle : \beta < \alpha\}
    \end{equation*}
    for up to arbitrarily many steps $< \alpha$, with all eventual haltings recorded in $R$.
\end{enumerate}
...
\begin{proof}
That $B \in \mathbf{\Pi_1}(L_{\alpha}[A])$ implies $\alpha \setminus B \in \mathbf{\Sigma_1}(L_{\alpha}[A])$, so let $\mathfrak{M}$ be a GSeqAP witnessing $\alpha \setminus B \leq^P_{\alpha} A$ constructed \textit{a la} the proof of Lemma \ref{lem273}. By modifying the code of $\mathfrak{M}$ such that in \hyperref[8340c3]{Case 3}, we bit-flip the contents of $R$ before copying them to the output tape, we obtain a GSeqAP witnessing $B \leq^P_{\alpha} A$.
\end{proof}
