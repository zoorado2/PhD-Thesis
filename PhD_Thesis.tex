...
\begin{enumerate}[label=(\alph*)]
    ...
    \item\label{def210e} $\mathcal{L}$ is $\Pi_1$-definable in the language associated with $\mathfrak{A}$.
\end{enumerate}
\end{defi}
...
\end{itemize}
Similar in motivation to \ref{def210e} of Definition \ref{lsuitable}, the requirement for $\mathbb{P}$ to be $\Sigma_1$-definable in the language associated with $\mathfrak{A}$ is only there so that the proof of Theorem \ref{uni} can go through given its hypothesis.
