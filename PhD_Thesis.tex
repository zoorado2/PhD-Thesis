...
\begin{enumerate}[label=(CS\arabic*), leftmargin=40pt]
    \item $P'_{\lambda}$ is dense in $\mathbb{P}_{\lambda}$ for all $\lambda \in C$, and
...
We have described above what is spiritually a countable-support version of the original $\mathbb{P} = \mathbb{P}_{\lambda^{\dagger}}$ constructed in the proof of Theorem \ref{notion1}. To distinguish this new version from the original, call the former $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$. Now, it is a routine exercise in transfinite induction to check that the map $A \mapsto \{\ulcorner A \subset \dot{G} \urcorner\}$ --- under some appropriate quotient --- witnesses $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1) \lessdot \mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$. In fact, as a consequence of the \ref{s9n}'s, the image of any $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$-generic filter $g$ over $W$ under the aforementioned map uniquely determines any $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$-generic filter over $W$ extending $g$. We thus have that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{< \omega_1}_{\lambda^{\dagger}}$ are forcing equivalent.
