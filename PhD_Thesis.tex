...
\end{equation*}
as specified in \ref{6330i}\ref{6330i2} of Corollary \ref{rlcor2nd}, is always a $G_{\delta}$ subset of $2^{\omega}$. On the other hand, for every $G_{\delta}$ subset $X$ of $2^{\omega}$, there are $r$, $\mathfrak{A}$, $T$, $\Phi$ such that
\begin{itemize}
    \item $r$, $\mathfrak{A}$ and $T$ satisfy conditions \ref{6330ia} to \ref{6330ic} of \ref{6330i} of Corollary \ref{rlcor2nd},
    \item $\Phi$ is an oracle machine, and
    \item $\{\Phi^x : x \in \{f_r" (\Sigma(\mathfrak{T}(\mathfrak{A}, T), \mathcal{M})) : \mathcal{M} \in \mathrm{Sub}(\mathfrak{A}, T)\}\} = X$.
\end{itemize}
Thus \ref{6330ii} of Corollary \ref{rlcor2nd} essentially provides a $\Delta^1_2$-definable way of selecting perfect set witnesses for uncountable $G_{\delta}$ subsets of $2^{\omega}$, given particular (sufficiently simple) real codes of the said subsets. By analysing the aforementioned $\Delta^1_2$ definition, we can conclude something stronger.
...
It is standard knowledge in effective descriptive set theory (see e.g. Chapter III of \cite{sacks}) that for any real $A$, a $\Sigma^1_1(A)$-definable set of reals contains a perfect set iff it contains a $\Delta^1_2(A)$-definable perfect set. However, it is non-trivial to derive from this knowledge a $\Delta^1_2$ definition for a perfect set witness (whenever it exists) that uniformly applies to real codes of even a relatively simple class of sets of reals. Corollary \ref{cor6335} provides one such definition uniform with respect to any $\Sigma^1_1$ real coding of $G_{\delta}$ subsets of $2^{\omega}$. 
