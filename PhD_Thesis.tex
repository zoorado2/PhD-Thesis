...
One may view Proposition \ref{prop322} as a template for the set of requirements any sufficiently local means of defining small extensions of $V$ should fulfil. More precisely, should we replace 
\begin{itemize}
    \item ``$\mathfrak{T}$ is a TCI'' with any other description $\varphi(x)$ of a parameter set $x$, and
    \item ``$\mathcal{M} \models^* \mathfrak{T}$'' with any other sufficiently local means of defining the class of generators of small extensions of $V$ from a parameter set $x$ satisfying $\varphi(x)$
\end{itemize}
...
In any case where the aforementioned replacements behave as expected, we say that the associated means of defining small extensions of $V$ \textit{fulfils the requirements suggested by Proposition \ref{prop322}}.
\end{rem}
...
By carefully analysing the proof of Theorem \ref{thm9414}, we can come to a stronger conclusion. Indeed, by an argument virtually identical to the one in the said proof, any means of defining small extensions of $V$ that fulfils the requirements suggested by Proposition \ref{prop322} (see Remark \ref{rem926}) can be refined by forcing with some forcing notion. In practice, where forcing is used predominantly within the context of consistency proofs, this indicates that there is no better tool than forcing to generate small extensions of $V$ in a sufficiently local manner. 
