...
\begin{enumerate}[label=(C\arabic*)]
    ...
    \end{align*}
    are logically implied by the conjunction of $\psi^{\ulcorner X \urcorner}$ and $\psi^{\ulcorner Y \urcorner}$ (see \ref{82163} of Definition \ref{def220}) for any two unary function symbols $\ulcorner X \urcorner, \ulcorner Y \urcorner \in \sigma$, and the unbounded existential quantifiers occurring in them signal the necessity of unbounded exploration in obtaining the truth values of $\ulcorner X_1(x) = Y_1(x) \urcorner$ and $\ulcorner X_1(Y_1(x)) = y \urcorner$. 
...
\end{equation*}
For each $\ulcorner X \urcorner \in \sigma(\mathfrak{M}_1)$, define a formula $\varphi^{\ulcorner X \urcorner}$ over $\{\ulcorner Y_0 \urcorner : \ulcorner Y \urcorner \in \sigma(\mathfrak{M}_1)\}$ with one free variable as follows:
\begin{enumerate}[label=(\alph*)]
    \item if $\ulcorner X \urcorner$ is an $n$-ary function symbol, then 
    \begin{align*}
    ...
    \item if $\ulcorner X \urcorner$ is an $n$-ary relation symbol, then 
    \begin{align*}
        \varphi^{\ulcorner X \urcorner}(x) := \ulcorner & \forall y_1 \in_0 x \dots \forall y_n \in_0 x \ (X_0(y_1, \dots, y_n) \\
        & \iff \phi^{\ulcorner X \urcorner}(y_1, \dots, y_n)) \urcorner \text{, and}
    \end{align*}
    \item if $\ulcorner X \urcorner$ is a constant symbol, then 
    \begin{align*}
        \varphi^{\ulcorner X \urcorner}(x) := \ulcorner \forall y \in_0 x \ (X_0 = y \iff \phi^{\ulcorner X \urcorner}(y)) \urcorner \text{.}
    \end{align*}
\end{enumerate}
Now define 
\begin{equation*}
...
\begin{align*}
    ...
    & \text{(by \ref{834ci} and \ref{834ciii})} \\
