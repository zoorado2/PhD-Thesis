...
It is standard knowledge in effective descriptive set theory (see e.g. Chapter III of \cite{sacks}) that for any real $A$, a $\Sigma^1_1(A)$-definable set of reals contains a perfect set iff it contains a $\Delta^1_2(A)$-definable perfect set. However, it is non-trivial to derive from this knowledge a $\Delta^1_2$ definition for a perfect set witness that uniformly applies to all real codes of even a simple class of sets of reals. Corollary \ref{rlcor2nd} provides one such definition --- an easily verifiable one at that --- uniform on all $G_{\delta}$ sets of reals. 
