...
\begin{rem}
Instead of specifying $\mathrm{Eval}^V(\mathfrak{T}') \subset \mathrm{Eval}^V(\mathfrak{T})$ and $\mathrm{Eval}^V(F(\mathfrak{T})) \subset \mathrm{Eval}^V(\mathfrak{T})$ in \ref{92101} and \ref{92102} of Definition \ref{def9210} respectively, we can respectively have a subset of $\mathrm{Eval}^V(\mathfrak{T})$ being reducible to $\mathrm{Eval}^V(\mathfrak{T}')$ and $\mathrm{Eval}^V(F(\mathfrak{T}))$. The latter approach takes into account the ``minimal computational power'' of local method definitions when talking about their complexity. To wit, let
\begin{enumerate}[label=(\arabic*)]
