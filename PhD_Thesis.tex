...
\begin{enumerate}[label=(\alph*)]
    ...
    \end{equation*}
\end{enumerate} 
The only category of obvious alternatives to the Asper\'{o}-Schindler side condition method we have encountered so far appears to be the class of methods that leverage on the ``countable support'' the sequences of forcing notions $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ are inherently endowed with. In this subsection, let us briefly experiment with such sequences. 
...
What is left for us to prove is that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ adds no $\omega$-sequences of ordinals. We want to show that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ exhibits some sort of strategic closure property. Towards this end, let 
\begin{itemize}
...
Our line of reasoning --- as intuitive as it is --- juxtaposed with the tiny gap we encountered along the way, points to the following:
\begin{quote}
    if a a witness to Assumption \ref{thm744} can be uniformly constructed from $\kappa$ and $\lambda^{\dagger}$, then either $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ is also a witness for some uniform definition of $C$, or radically new technology is needed in the construction.
\end{quote}
