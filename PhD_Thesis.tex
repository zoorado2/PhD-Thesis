...
\end{enumerate}
we have $\Xi^{\langle \langle e, n \rangle, x \rangle} = \langle e', f_r" (\Gamma_{\mathfrak{T}}) \rangle$. 
\end{cor}

Intuitively, Corollary \ref{cor6338} tells us we can derive from each $G_{\delta}$ subset an ``equivalent'' code-friendly $\Pi_2$ TCI, in a manner that is uniformly computable across all $\Pi^0_2$ real codings of $G_{\delta}$ subsets of $2^{\omega}$.
