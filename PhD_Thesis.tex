...
\end{equation*}
Define $\mathrm{Ex}(\lambda)$ to be $ran(\Vdash^n_{Col(\omega, \lambda)})$. If there is a definable (with parameters) class function $F$ on $\mathrm{Ex}(\lambda)$ such that $ran(F) \subset V^{Col(\omega, \lambda)}$ and 
\begin{equation}\label{eq943}
    \forall \psi \in \mathrm{Ex}(\lambda) \ \forall \phi \ (((\Vdash^n_{Col(\omega, \lambda)} \psi) \wedge \psi = \ulcorner \exists x \ \phi(x) \urcorner) \implies \Vdash^n_{Col(\omega, \lambda)} \phi(F(\psi))) \text{,}
\end{equation}
then there is a definable (with parameters) global choice function.
\end{lem}

\begin{proof}
Assume $F$ exists for some $\lambda$ and $n$ in accordance with the hypothesis of the lemma. Note that the forcing notion $Col(\omega, \lambda)$ and the proper classes $V^{Col(\omega, \lambda)}$, $\Vdash^n_{Col(\omega, \lambda)}$ and $\mathrm{Ex}(\lambda)$ are all definable from the parameter $\lambda$, which means (\ref{eq943}) is expressible as a single sentence (with parameters) in the language of set theory. 

Fix any definable well-ordering $<_{\lambda}$ of (the conditions of) $Col(\omega, \lambda)$. For example, the length-lexicographical ordering works as $<_{\lambda}$. For any non-empty set $X$, set $\psi_X := \ulcorner \exists x \ ( x \in \check{X}) \urcorner \in \mathrm{Ex}(\lambda)$, where $\check{X}$ is the canonical $Col(\omega, \lambda)$-name for $X$. Obviously, $\Vdash^n_{Col(\omega, \lambda)} \psi_X$, so by (\ref{eq943}), $\Vdash^n_{Col(\omega, \lambda)} F(\psi_X) \in \check{X}$. Let $p_X$ be the $<_{\lambda}$-least member of the (necessarily non-empty) set 
\begin{equation*}
