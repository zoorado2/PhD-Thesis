...
\end{equation*}
for any admissible ordinal $\alpha$, and the inclusions cannot be reversed in general. By Proposition \ref{prop270}, Fact \ref{fact272} and Lemma \ref{lem273}, we have
\begin{equation}\label{eq2}
    \leq_{\alpha} \ \subset \ \leq_{w \alpha} \ \subset \ \preceq_{\alpha} \ \subset \ \leq^{P, s}_{\alpha} \ \subset \ \leq^P_{\alpha}
\end{equation}
for admissible ordinals $\alpha$. The penultimate inclusion cannot be reversed in general because $\leq^{P, s}_{\alpha}$ is transitive for all admissible $\alpha$ but $\preceq_{\alpha}$ is not. Incidentally, this means there exists some (necessarily singular) admissible $\alpha$ and $A, B \subset \alpha$ such that $B \leq^{P, s}_{\alpha} A$ and $B \not\in \mathbf{\Delta_1}(L_{\alpha}[A])$. That the last inclusion cannot be reversed is due to Lemmas \ref{lem269} and \ref{lem273}. 
