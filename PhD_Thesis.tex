...
\begin{enumerate}[label=(P\arabic*), leftmargin=40pt]
    ...
    \item\label{p2} $P_{\lambda} = \bigcup \{P_{\lambda'} : \lambda' \in C \cap \lambda\}$ whenever $\lambda \in C \cup \{\lambda^{\dagger}\}$ and $sup(\lambda \cap C) = \lambda$.
\end{enumerate}
...
This section provides a cursory investigation into natural analogues of the constructions that transpired in the proofs of Theorems \ref{notion1} and \ref{notion2}. The said investigation is carried out roughly along two paths.

\subsection{Towards Greater Supports}

Work in the universe $W$ as defined in the proof of Theorem \ref{notion1} and let $\lambda^{\dagger}$, $\kappa$, $C$ and $\{Q_{\lambda} : \lambda \in C\}$ be as in the same proof. Fix any infinite cardinal $\theta > \omega$. Should we then alter the definition of $\Gamma_{\lambda}$ and $P_{\lambda}$ for each $\lambda \in C \cup \{\lambda^{\dagger}\}$ as follows:
\begin{align*}
...
\begin{quote}
    ``$sup(X_{\delta, \lambda'} \cap \delta) = \delta$ whenever $X_{\delta, \lambda'}$ is non-empty'',
\end{quote}
...
It so happens that recent results by De Bondt and Veli\v{c}kovi\'{c} (\cite{bendb}, as a part of De Bondt's PhD dissertation) resolved \ref{newq2} in the affirmative. In fact, the class of forcing notions constructed by De Bondt and Veli\v{c}kovi\'{c} assuming only $\mathsf{ZFC}$, bears witness to a theorem significantly stronger than 
\begin{equation*}
