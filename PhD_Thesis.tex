...
At this point, there should be an inkling that $p^*$ being a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ is not obvious. In fact, there is an inability to guarantee its consistency with the fulfilment of just one of the 8 modified specifications among \ref{c1} to \ref{c8}: the reader should be able to convince themselves without too much hassle that $p^*$ can be extended to some $\Sigma$ satisfying the modified versions of all these specifications except for \ref{c5}. Even if we weaken the modified \ref{c5} to instead say
\begin{quote}
    ``$sup(X_{\delta, \lambda'} \cap \delta) = \delta$ whenever $X_{\delta, \lambda'}$ is non-empty'',
\end{quote}
it appears hard to argue that such a $\Sigma$ exists in some outer model of $V$. 
