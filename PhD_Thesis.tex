...
We first analyse Gurevich's original conception of abstract state machines through the lens of set theory. Abstract state machines are among the most general representations of algorithms and computations. However, our analysis suggests they might be \emph{too} general a representation. More precisely, we discover that it is possible for a computation on some such machine to be \emph{inconstructible} in the sense of being outside G\"{o}del's universe $L$ (Example \ref{ex21}). This seems to go against the common intuition that computations are intrinsically constructive. We are thus morally justified to impose restrictions on what constitutes an abstract state machine.  

The restrictions we impose are natural and well-motivated; the reader can hopefully verify this by following our thought process as detailed in Section \ref{sect82}. As a result, we obtain two classes of machine models for generalised computation, namely \textit{restricted abstract state machines} (\textit{RASMs}, Defintion \ref{def224}) and \textit{restricted abstract state machines with parameters} (\textit{RASMPs}, Definition \ref{def836}). It is straightforward to derive from each class, certain relative computability relations on sets, which are analogous to the Turing reducibility relation on reals. We focus on two such relations derived from RASMPs, denoted $\leq^P$ (Definition \ref{def839}) and $\leq^{P, s}$ (Definition \ref{def8329}). These relations are shown to possess nice properties not found in more popular notions of relative computability (follows from Table \ref{table3}). By leveraging on Koepke's analysis of $\alpha$-machines, we then prove that $\leq^P$ is equivalent to the relative constructibility relation in set theory (Theorem \ref{thm275}).
...
In this chapter, we investigate the promise of abstract state machines in infinitary computations. We make well-meaning restrictions to their definition along the way, so that they are better endowed with our intuition of computation born out of classical computing. 
...
\section{Restricting Abstract State Machines}\label{sect82}
