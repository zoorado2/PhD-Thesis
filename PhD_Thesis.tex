...
\AtBeginDocument{\addtocontents{toc}{\protect\thispagestyle{simple}} \addtocontents{toc}{\protect{\pdfbookmark[0]{\contentsname}{toc}}}}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item\label{224} The variable symbols occurring in any first-order formula must come from a fixed countably infinite set $\mathrm{Var} \subset H(\omega)$. We will assume that $\mathrm{Var}$ is disjoint from both the set of first-order logical symbols and the class of signature-related symbols.
    \item A string over a vocabulary set $\Sigma$ is a member of $\Sigma^{< \omega}$. When a string is mentioned without specifying its vocabulary set, we may assume its vocabulary set is the entire universe $V$. We present the contents of a string between the delimiters $\ulcorner$ and $\urcorner$.
...
\begin{defi}
For each $n < \omega$, fix a different unary relation symbol $A_n \in H(\omega)$. Define
\begin{align*}
    \mathrm{2Var} := \ & \{A_n : n < \omega\}  \text{ and} \\
    \mathrm{Var^+} := \ & \mathrm{Var} \cup \mathrm{2Var} \text{,}
\end{align*}
so that $\mathrm{Var^+} \subset H(\omega)$. A \emph{monadic second-order formula} over a signature $\sigma$ is a first-order formula over the signature $\sigma \cup \mathrm{2Var}$ with its variable symbols coming from the enlarged variable set $\mathrm{Var^+}$. 
\end{defi}
...
\begin{defi}
Fix bijections $\mathrm{Pr1}$ and $\mathrm{Pr2}$ from $\omega \times \omega$ into $\omega$ and from $\omega \times 2^{\omega}$ into $2^{\omega}$ respectively, such that their projection functions are all computable. In other words, there are $e_i$ for $i < 4$, such that
\begin{enumerate}[label=(\alph*)]
    \item $\Phi^{\emptyset}_{e_0}(\mathrm{Pr1}(m, n)) = m$ for all $m, n < \omega$, 
    \item $\Phi^{\emptyset}_{e_1}(\mathrm{Pr1}(m, n)) = n$ for all $m, n < \omega$, 
    \item $\Phi^{\mathrm{Pr2}(m, x)}_{e_2}(n) = m$ for all $m, n < \omega$ and $x \in 2^{\omega}$, and
    \item $\Phi^{\mathrm{Pr2}(m, x)}_{e_3} = x$ for all $m < \omega$ and $x \in 2^{\omega}$.
\end{enumerate}
Given any $(m, n) \in \omega \times \omega$, we use $\langle m, n \rangle$ to denote $\mathrm{Pr1}(m, n)$. Given any $(m, x) \in \omega \times 2^{\omega}$, we use $\langle m, x \rangle$ to denote $\mathrm{Pr2}(m, x)$.
\end{defi}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The set 
    \begin{equation*}
        \{n \in \omega : \exists x, y \in H(\omega) \ (x \in y \text{ and } n = \langle f(x), f(y) \rangle)\}
    \end{equation*}
    is a computable subset of $\omega$.
    \item The graph of $f$, i.e. the set 
    \begin{equation*}
        \{n \in \omega : \exists x \in H(\omega) \ (n = \langle x, f(x) \rangle )\} \text{,}
    \end{equation*}
...
The idea of forcing models of a theory has been studied by model theorists since the 1970s, under the label \emph{model-theoretic forcing}. Robinson's original approach to this type of forcing involves forcing notions of which conditions are finite consistent fragments of a Henkin theory $T$. One can then argue that a sufficiently generic filter over such a forcing notion gives us an elementary diagram of an existentially closed model of $T$. Thanks to the many model theorists who built upon Robinson's pioneering work, among whom Barwise is probably the most notable, more intricate relationships between the properties of language fragments in forcing notions and the properties of resultant generic theories were uncovered (see e.g. \cite{keisler}).

On the other hand, amalgamating language fragments into a theory in infinitary sentential logic through set-theoretic forcing was also explored, though perhaps somewhat independently from their model-theoretic counterpart, in the 1970s. This led to Bukovsk\'{y}'s landmark theorem \cite{bukovsky1}, which provides a characterisation of set generic extensions in terms of the existence of global coverings. The same sort of technique --- forcing with quotients of Lindenbaum algebras ---  was later used by Woodin for his extender algebra (see e.g. \cite{farah}), albeit with more sophisticated combinatorial interactions made possible by large cardinal assumptions. Indeed, as Schindler put forth in \cite{schindler-hod}, even the famous Vope\v{n}ka algebra \cite{vopenka} defined in the early 1970s can be argued to have been cut from a similar cloth.

More recently, set theorists have leveraged on results in model-theoretic forcing to generate conditions of the forcing notions used in various relative consistency proofs. Jensen initiated the use of Barwise theory in arguments regarding combinatorial properties of forcing notions, in his remarkably original theory on $\mathcal{L}$-forcing (see \cite{lforcing}). Further works and applications in this direction include \cite{doebler}. However, in these cases, the languages (usually denoted $\mathcal{L}$) specified in the construction of forcing notions seem very much specialised for their use cases, and it is not immediately clear how much of the analysis of one forcing notion can be recycled in the analysis of another. 

Asper\'{o} and Schindler are likely the first to present, in their seminal work \cite{schindler}, a construction that not only outright stipulates its forcing conditions to be exclusively language fragments, but also allows for such fine control of the said fragments that one is able to build complicated Henkin term models from the generic theories they produce. Here, unlike in previous works, 
\begin{itemize}
    \item there is no introduction of any proof system, so the forcing notion in question is not defined via provability and proof-theoretic consistency;
    \item instead, things are reasoned about semantically with the aid of the forcing relation in the ground model, and further,
    \item all model-theoretic arguments and constructions are deferred to generic theories in generic extensions.
\end{itemize}  
It quickly became clear that this approach is spiritually similar to, yet spiritually expands on, that of model-theoretic forcing. This realisation might lead one to wonder, \textit{what are some general theorems about sufficiently generic theories produced through this kind of low-level set-theoretic forcing with language fragments?}
...
More precisely, this chapter details a framework for constructing forcing notions with fragments of a language $\mathcal{L}$ as conditions, based on specifications of a theory over $\mathcal{L}$. Our goal is to ensure that the generic theories of each forcing notion constructed in this way always satisfy the given specifications. To state these specifications, a \textit{meta-language} dependent on $\mathcal{L}$ is required. By choosing the right meta-language, we are able to exert suitably fine control over the generic theories our forcing notions produce. This enables certain fine-tuning of our forcing notions so that their generic theories meet the specifications we start out with. We will make the relevant technical terms and concepts precise as we build our framework over the subsequent three sections.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item\label{def210e} $\mathcal{L}$ is $\Pi_1$-definable in the language associated with $\mathfrak{A}$, as witnessed by some $\Pi_1$ formula $\varphi_{\mathcal{L}}$ with a single free variable.
\end{enumerate}
Where appropriate, we shall use $``x \in \mathcal{L}"$ to represent $\varphi_{\mathcal{L}}(x)$.
\end{defi}
...
A set $\Sigma$ is $\mathcal{L}$-\emph{nice} iff $\Sigma \subset \mathcal{L}$ and for all $\phi \in \mathcal{L}$, exactly one of $\phi$ and $\neg \phi$ is a member of $\Sigma$.
\end{defi}
...
\begin{defi}
Given a $\mathcal{L}^{*}_{\mathfrak{A}}$ formula $\phi$, a $\phi$\emph{-max peeling} is a member of $\mathrm{QA}(\phi)$ that is not a subformula of any other member of $\mathrm{QA}(\phi)$.
\end{defi}
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \begin{enumerate}[label=Subcase (\alph*):, leftmargin=70pt]
...
\begin{itemize}
    \item $P$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$, as witnessed by some $\Sigma_1$ formula $\varphi_{P}$ with a single free variable,
    \item $\leq_P$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$, as witnessed by some $\Sigma_1$ formula $\varphi_{\leq_P}$ with two free variables, and
    \item $(\mathfrak{A}, \mathbb{P})$ is good for $\mathcal{L}$.
\end{itemize}
Where appropriate, we shall use $``x \in P"$ and $``x \leq_P y"$ to represent $\varphi_{P}(x)$ and $\varphi_{\leq_P}(x, y)$ respectively.

Similar in motivation to \ref{def210e} of Definition \ref{lsuitable}, the requirements for $P$ and $\leq_P$ to be $\Sigma_1$-definable in the language associated with $\mathfrak{A}$ are only there so that the proof of Theorem \ref{uni} can go through given its hypothesis. For brevity, we say $\mathbb{P}$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$ exactly when both $P$ and $\leq_P$ are as such.
...
This framework both generalises and is inspired by the forcing construction Asper\'{o} and Schindler carried out in the proof of the main theorem (Lemma 2.14) of \cite{schindler}.
...
Let $|trcl(\mathfrak{A})| \leq \lambda$ and $p \subset \mathcal{L}$. Assume there is $\Sigma$ in an outer model $W$ of $V$ such that $\Sigma$ $\Gamma(\mathcal{L}, \mathfrak{A})$-certifies $p$. Then $$\Vdash_{Col(\omega, \lambda)} \exists \Sigma \ (``\Sigma \ \Gamma(\mathcal{L}, \mathfrak{A}) \text{-certifies } p").$$
\end{lem}
...
\begin{itemize}
    ...
    \item $g \in W$ is a $\mathbb{P}$-$\Sigma_1$-generic filter over $\mathfrak{A}$.
\end{itemize}
If there is $\Sigma$ in an outer model $W'$ of $V$ such that $\Sigma$ $\Gamma(\mathcal{L}, \mathfrak{A})$-certifies $\emptyset$, then $\bigcup g$ $\Gamma(\mathcal{L}, \mathfrak{A})$-certifies $\emptyset$.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item\label{3397} $W$ is an outer model of $V$,
...
The definition of $\mathbb{P}$ from parameters $B$, $\mathcal{L}$, $\mathfrak{A}$, $\Gamma$ in Proposition \ref{certgood}, where $\lambda$ is additionally specified to be $|trcl(\mathfrak{A})|$, is absolute for transitive models of $\mathsf{ZFC}$ sharing the same ordinals.
\end{lem}
...
\end{equation}
is absolute for transitive models of $\mathsf{ZFC}$ sharing the same ordinals. 
...
Towards this end, we want to come up with a formula $\psi_{cert}$ in two free variables: $\psi_{cert}$ is intended to help us define a uniform reduction of the problem of constructing models of TCIs to the problem of forcing with language fragments. We shall exhibit $\psi_{cert}$ in the proof of the next lemma, the latter of which statement also highlights some key properties of $\psi_{cert}$.

\begin{lem}\label{mcequiv}
The formula $\psi_{cert}$ can be chosen such that in any model of $\mathsf{ZFC}$, 
\begin{enumerate}[label=(\alph*)]
...
Fix $\psi_{cert}$ to be as in the proof of Lemma \ref{mcequiv}. We are then justified in our next definition.
...
By Lemmas \ref{Pisabs} and \ref{mcequiv}, the definition of $\mathbb{P}(\mathfrak{T})$ from $\mathfrak{T}$ is absolute for transitive models of $\mathsf{ZFC}$ sharing the same ordinals.

At this juncture, it is customary for us to remark on possible reasons to consider $\mathbb{P}(\mathfrak{T})$ in our process of studying $\mathfrak{T}$. To this end, it is helpful to preview the main forcing construction of next chapter.

\begin{rem}\label{rem520}
Consider the sequence of forcing notions $$\{\mathbb{P}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$$ constructed within $W$ in the proof of Theorem \ref{notion1}. The adaptation of this inductive construction to the language of TCIs is straightforward: given $\lambda \in C \cup \{\lambda^{\dagger}\}$ and $\Vec{\mathbb{P}}_{\lambda} := \{\mathbb{P}_{\theta} : \theta \in \lambda \cap C\}$, define $\sigma$ to contain
\begin{enumerate}[label=(\Alph*)]
    \item\label{5201} the variable names of all subsets of $H(\lambda^{\dagger})$ germane to the definition of $\mathbb{P}_{\lambda}$ in the original proof (this set includes $\Vec{\mathbb{P}}_{\lambda}$), along with
    \item\label{5202} two other ternary relation symbols, $\dot{F}$ and $\dot{X}$,
\end{enumerate}
and nothing else. If $\dot{Z} \in \sigma$ is of type \ref{5201}, then we interpret $\dot{Z}$ invariably as whichever set it is defined to be in the original proof; for example, when $\dot{Z} = \ulcorner C \urcorner$, we set $\vartheta (\dot{Z}) = (C, 1)$. On the other hand, if $\dot{Z} \in \sigma$ is of type \ref{5202}, then we interpret $\dot{Z}$ according to either \ref{l1lamb} or \ref{l2lamb} based on the identity of $\dot{Z}$; for example, when $\dot{Z} = \dot{F}$,  we set $$\vartheta (\dot{Z}) = (\{(i, n, \alpha) : i \in R \cap \lambda, n < \omega \text{ and } \alpha < i\}, 0).$$ Also set $\vartheta (\dot{\mathcal{U}}) = (H(\lambda^{\dagger}), 1)$. 
...
As we will see, based on Remark \ref{rem520} and what transpires in the next chapter, that our procedure of associating a partial order with each TCI can be used to generate forcing notions as complex as the ones specifically constructed to solve a difficult problem in set theory. More formal declarations of the power of this procedure will appear --- and be proven --- in the later parts of this thesis. But before that, let us return to the setting of the ground.
...
$$\Vdash_{Col(\omega, \lambda)} \exists \mathcal{M} \ (``\mathcal{M} \models^* \mathfrak{T}") \text{,}$$ where $\lambda$ is any cardinal $\geq |H(|trcl(\mathfrak{T})|^+)|$.
\end{lem}
...
\begin{itemize}
    \item Lemma \ref{inout}, and
    \item the fact that every forcing extension of $V$ is an outer model of $V$,
\end{itemize}   
tells us that $\mathfrak{T}$ is consistent iff $$\Vdash_{Col(\omega, \lambda)} \exists \Sigma \ (``\Sigma \ \Gamma_{\mathfrak{T}} (\mathcal{L}_{\mathfrak{T}}, \mathfrak{A}_{\mathfrak{T}})\text{-certifies } \emptyset") \text{,}$$ where $\lambda$ is any cardinal $\geq |H(|trcl(\mathfrak{T})|^+)|$. 
...
\begin{defi}
Let $X \subset H(\omega)$. We say $(r, f)$ \emph{witnesses} $(X; \in)$ \emph{is computable} iff there are $e$ and $g$ such that
\begin{enumerate}[label=(\alph*)]
    \item $e < \omega$,
    \item $g$ is a function from $\omega$ into $\omega$,
    \item $f$ is a bijection from $X$ into $\omega$,
    \item for all $x, y \in X$, $x \in y$ implies $f(x) < g(f(y))$,
    \item $\Phi^{\emptyset}_e = \{m \in \omega : \exists n < \omega \ (m = \langle n, g(n) \rangle)\}$,
    \item $r = \langle e, \{n \in \omega : \exists x, y \in H(\omega) \ (x \in y \text{ and } n = \langle f(x), f(y) \rangle)\} \rangle$, and
...
\end{enumerate}
The verity of \ref{ss53a} to \ref{ss53c} implies that the proof of Lemma \ref{ctblegeneric2} essentially functions as a proof of a more general statement, which we formally present as a theorem below.
...
What Corollary \ref{rlcor2nd} (and thus Corollary \ref{rlcor}) offers beyond what Fact \ref{lastfact} does, is a constructive method to uniformly pick out perfect set witnesses for the perfect set property of $\mathrm{Sub}(\mathfrak{A}, T)$ insofar as such witnesses exist, over all possible $\mathfrak{A}$ and $T$ within the confines of the corollary's assumptions. Moreover, said method is of very low complexity in the grand scheme of things. To offer another perspective, it is standard knowledge in effective descriptive set theory (see e.g. Chapter III of \cite{sacks}) that for any real $A$, a $\Sigma^1_1(A)$-definable set of reals contains a perfect set iff it contains a $\Delta^1_2(A)$-definable perfect set. However, it is not immediately obvious how one can derive from this knowledge a $\Delta^1_2$ definition for a perfect set witness (whenever it exists) that uniformly applies to real codes of even a relatively simple class of sets of reals. Corollary \ref{rlcor2nd} seems to break new ground in this respect through a wholly different approach. But is it really something novel?
...
Now, Corollary \ref{cor6335} is easily derivable from classical results. Indeed, iteratively applying the Cantor-Bendixson derivative on any real under some fixed $\Sigma^1_1$ real coding of $G_{\delta}$ subsets of $2^{\omega}$ can be done in a uniform $\Delta^1_2$ manner. We have thus come full circle from the parallels we drew in Remark \ref{CBrem}. At this point, the next corollary, implicit in the proof of Theorem \ref{lastthm}, hopefully becomes clear.

\begin{defi}\label{def6336}
Let $X$ be a subset of $2^{\omega}$. Define
\begin{align*}
    \mathrm{Cl}^{(\alpha)}(X) := \ & \text{the closure of } X^{(\alpha)} \text{ (the } \alpha \text{-th-order Cantor-Bendixson derivative} \\
    & \text{of } X \text{ as a subspace of Cantor space) in Cantor space, and} \\
    \mathrm{Tr}^{(\alpha)}(X) := \ & \text{the unique subtree } \mathrm{Tr} \text{ of } 2^{< \omega} \text{ such that } [\mathrm{Tr}] = \mathrm{Cl}^{(\alpha)}(X) \text{.}    
\end{align*}
\end{defi}

We can adapt Definition \ref{def6336} to the context of TCIs by associating each TCI with a $G_{\delta}$ subset of $2^{\omega}$, as we have been doing so far.

\begin{defi}
Let $r$ be a nicely computable code of $H(\omega)$ and $\mathfrak{T}$ be a code-friendly $\Pi_2$ TCI. Define
\begin{align*}
    \mathrm{Cd}(\mathfrak{T}) := \ & \{f_r" (\Sigma(\mathfrak{T}, \mathcal{M})) : \mathcal{M} \models^* \mathfrak{T}\} \text{,} \\
    \mathrm{Cl}^{(\alpha)}(\mathfrak{T}) := \ & \mathrm{Cl}^{(\alpha)}(\mathrm{Cd}(\mathfrak{T})) \text{, and} \\
    \mathrm{Tr}^{(\alpha)}(\mathfrak{T}) := \ & \mathrm{Tr}^{(\alpha)}(\mathrm{Cd}(\mathfrak{T})) \text{.}    
\end{align*}
\end{defi}

\begin{cor}\label{cor6337}
Let $r$ be a nicely computable code of $H(\omega)$ and $\mathfrak{T}$ be a code-friendly $\Pi_2$ TCI. Then there is a canonical dense embedding $\pi$ from $\mathrm{Tr}^{(0)}(\mathfrak{T})$ into $\mathbb{P}(\mathfrak{T})$ such that for all ordinals $\alpha$, $\pi \restriction \mathrm{Tr}^{(\alpha)}(\mathfrak{T})$ is a dense embedding from $\mathrm{Tr}^{(\alpha)}(\mathfrak{T})$ into $\mathbb{P}(\mathfrak{T})^{(\alpha)}$.
\end{cor}

Corollary \ref{cor6337} arguably cements the legitimacy of calling $\mathbb{P}(\mathfrak{T})^{(\alpha)}$ ``the $\alpha$-th-order Cantor-Bendixson derivative of $\mathbb{P}(\mathfrak{T})$''. In fact, based on the meandering journey we took to obtain Corollary \ref{cor6335}, a far greater amount of uniformity can be uncovered from the correspondence --- the ``Cantor-Bendixson connection'' if you will --- on display in Corollary \ref{cor6337}. This is exhibited in the final corollary of this section.

\begin{cor}\label{cor6338}
There are an oracle machine $\Xi$, a set $\mathcal{L}$ and ternary functions
\begin{equation*}
    \mathfrak{T}^*, e^*, \pi^* : \omega \times \omega \times 2^{\omega} \longrightarrow H(\omega_1)
\end{equation*}
wherewith whenever $e$, $n$, $r$, $\phi$, $x$ are such that
\begin{enumerate}[label=(\alph*)]
    \item\label{6339a} $e, n < \omega$,
    \item $r$ is a nicely computable code of $H(\omega)$,
    \item $\Phi^{\emptyset}_e = r$,
    \item $\phi$ is a $\Pi^0_2$ real coding of $G_{\delta}$ subsets of $2^{\omega}$,
    \item $f_r(\phi) = n$, and
    \item\label{6339f} $x \in 2^{\omega}$,
\end{enumerate}
we have
\begin{enumerate}[label=(\arabic*)]
    \item $\mathfrak{T}^*(e, n, x)$ is a code-friendly $\Pi_2$ TCI,
    \item $f_r" (\mathcal{L}_{\mathfrak{T}^*(e, n, x)})$ is computable,
    \item $\mathcal{L}_{\mathfrak{T}^*(e, n, x)} = \mathcal{L}$,
    \item $\pi^*(e, n, x)$ is an injective function from $f_r" (2^{< \omega})$ into $\omega$,
    \item $\Phi^{x}_{e^*(e, n, x)} = \{n \in \omega : \exists m \in dom (\pi^*(e, n, x)) \ (n = \langle m, \pi^*(e, n, x)(m) \rangle)\}$,
    \item $(f_r^{-1} \circ \pi^*(e, n, x) \circ f_r) \restriction \mathrm{Tr}(A^{\phi}_x)$ is a dense embedding from $\mathrm{Tr}(A^{\phi}_x)$ into \\ $\mathbb{P}(\mathfrak{T}^*(e, n, x))$,
    \item $\Xi^{\langle \langle e, n \rangle, x \rangle} = \langle e^*(e, n, x), f_r" (\mathcal{L}_{\mathfrak{T}^*(e, n, x)}) \oplus f_r" (\Gamma_{\mathfrak{T}^*(e, n, x)}) \rangle$, and
    \item for all ordinals $\alpha$ and all $e'$, $n'$, $r'$, $\phi'$, $x'$ satisfying \ref{6339a} to \ref{6339f} in place of $e$, $n$, $r$, $\phi$, $x$ respectively, 
    \begin{enumerate}[label=(\roman*)]
        \item\label{63398i} if $(A^{\phi}_x)^{(\alpha)} = A^{\phi'}_{x'}$, then $\mathbb{P}(\mathfrak{T}^*(e, n, x))^{(\alpha)} = \mathbb{P}(\mathfrak{T}^*(e', n', x'))$, and
        \item if in addition to \ref{63398i}, $e = e'$, then $e^*(e, n, x) = e^*(e', n', x')$,
    \end{enumerate}
    so that in particular, $(f_r^{-1} \circ \pi^*(e, n, x) \circ f_r) \restriction \mathrm{Tr}^{(\alpha)}(A^{\phi}_x)$ is a dense embedding from $\mathrm{Tr}^{(\alpha)}(A^{\phi}_x)$ into $\mathbb{P}(\mathfrak{T}^*(e, n, x))^{(\alpha)}$ for all ordinals $\alpha$.
\end{enumerate} 
\end{cor}

Intuitively, Corollary \ref{cor6338} tells us we can choose for each $G_{\delta}$ subset of $2^{\omega}$ an ``equivalent'' code-friendly $\Pi_2$ TCI, in a manner that is uniformly computable across all $\Pi^0_2$ real codings of $G_{\delta}$ subsets of $2^{\omega}$. Moreover, this choice can be made such that it commutes with Cantor-Bendixson derivatives of all orders and of any given $G_{\delta}$ subset of $2^{\omega}$.
...
The main results in this chapter are Theorems \ref{notion1} and \ref{notion2}. We shall see that the proof of Theorem \ref{notion1} can be intuitively extended to a proof of Theorem \ref{notion2}, thanks to the modularisability of our framework.
...
Assume $\mathrm{NS}_{\omega_1}$ is precipitous. Then $Nb'_1(\lambda^{\dagger})$ holds for each regular cardinal $\lambda^{\dagger} > 2^{\omega_1}$, and is witnessed by a forcing notion of size $2^{< \lambda^{\dagger}}$. In other words, whenever $\lambda^{\dagger} > 2^{\omega_1}$ is a regular cardinal, there is a stationary-preserving forcing notion $\mathbb{P}'$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\arabic*)]
    \item\label{cond1} $\Vdash_{\mathbb{P}'} ``cof(\alpha) = \omega"$ for all regular cardinals $\alpha$ satisfying $\omega_2 \leq \alpha < \lambda^{\dagger}$,
    \item\label{cond2} $\Vdash_{\mathbb{P}'} ``cof(\gamma) = \omega_1"$ for all regular cardinals $\gamma$ satisfying $\lambda^{\dagger} \leq \gamma \leq 2^{< \lambda^{\dagger}}$, and
    \item\label{cond3} $\Vdash_{\mathbb{P}'} ``cof(\beta) = \beta"$ for all regular cardinals $\beta$ satisfying $2^{< \lambda^{\dagger}} < \beta$.
\end{enumerate}
\end{thm}

\begin{proof}
Assume $\mathrm{NS}_{\omega_1}$ is precipitous, and fix a regular cardinal $\lambda^{\dagger} > 2^{\omega_1}$.  

Let $h$ be a generic filter on $Col(\lambda^{\dagger}, \lambda^{\dagger})$, so that in $V[h]$, 
\begin{itemize}
    \item $cof(\alpha) = cof^{V}(\alpha)$ for all $ \alpha \leq \lambda^{\dagger}$,
    \item $\mathrm{NS}_{\omega_1}$ is still precipitous,
    \item $|H(\lambda^{\dagger})| = \lambda^{\dagger}$, and
    \item there is a a $\Diamond_{\lambda^{\dagger}}$-sequence $(\bar{A}_{\lambda} : \lambda < \lambda^{\dagger})$.
\end{itemize}

\begin{lem}\label{lem26}
If in $V[h]$ there is a stationary-preserving forcing notion $\mathbb{P} = (P, \leq_P)$ fulfilling 
\begin{enumerate}[label=(\arabic*')]
    \item\label{4100} $P \subset H(\lambda^{\dagger})$, 
    \item\label{4101} $\Vdash_{\mathbb{P}} ``cof(\alpha) = \omega"$ for all regular cardinals $\alpha$ satisfying $\omega_2 \leq \alpha < \lambda^{\dagger}$, and
    \item\label{4102} $\Vdash_{\mathbb{P}} ``cof(\lambda^{\dagger}) = \omega_1"$,
\end{enumerate}
then for some $Col(\lambda^{\dagger}, \lambda^{\dagger})$-name $\dot{\mathbb{P}}$ for $\mathbb{P}$,
\begin{equation*}
    Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}}
\end{equation*} 
is a stationary-preserving forcing notion that contains a dense subset of size $2^{< \lambda^{\dagger}}$ and fulfils \ref{cond1} to \ref{cond3} of Theorem \ref{notion1} in $V$.
\end{lem}

\begin{proof}
Working in $V$, we set $$\mathbb{P}' := Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}} \text{,}$$ and note the following facts:
\begin{enumerate}[label=(\arabic*')]
    \setcounter{enumi}{3}
    \item\label{4103} $|Col(\lambda^{\dagger}, \lambda^{\dagger})| = 2^{< \lambda^{\dagger}}$,
    \item\label{4104} $\Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} ``H(\lambda^{\dagger}) = H(\lambda^{\dagger})^V"$.
\end{enumerate}
$\mathbb{P}'$ is a stationary-preserving forcing notion fulfilling \ref{cond1} of Theorem \ref{notion1} because 
\begin{itemize}
    \item $Col(\lambda^{\dagger}, \lambda^{\dagger})$ is a stationary-preserving forcing notion forcing $\dot{\mathbb{P}}$ to be stationary-preserving, 
    \item $Col(\lambda^{\dagger}, \lambda^{\dagger})$ forces $cof(\alpha) = cof^{V}(\alpha)$ for all $ \alpha < \lambda^{\dagger}$, and
    \item \ref{4101} holds.
\end{itemize}

By \ref{4100}, \ref{4103}, \ref{4104} and the fact that $|H(\lambda^{\dagger})| = 2^{< \lambda^{\dagger}}$, $\mathbb{P}'$ has a dense subset of size $2^{< \lambda^{\dagger}}$ in $V$. This implies $\mathbb{P}'$ preserves cofinalities $\geq (2^{< \lambda^{\dagger}})^+$ in $V$, and thus \ref{cond3} of Theorem  \ref{notion1} holds.

Now let $\gamma \geq \lambda^{\dagger}$ be regular in $V$. Then $cof^{Col(\lambda^{\dagger}, \lambda^{\dagger})} (\gamma) \geq \lambda^{\dagger}$ because $Col(\lambda^{\dagger}, \lambda^{\dagger})$ is $\lambda^{\dagger}$-closed. Since $|H(\lambda^{\dagger})| = \lambda^{\dagger}$ in $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$, \ref{4100} tells us that $\mathbb{P}$ preserves cofinalities $\geq (\lambda^{\dagger})^+$. This, together with \ref{4102}, ensures $cof(\gamma) \geq \omega_1$. We have thus shown
\begin{enumerate}[label=(\arabic*')]
    \setcounter{enumi}{5}
    \item\label{4105} $\Vdash_{\mathbb{P}'} ``cof(\gamma) \geq \omega_1"$ for all regular cardinals $\gamma$ satisfying $\lambda^{\dagger} \leq \gamma$.
\end{enumerate}
As \ref{4102}, \ref{4104} and \ref{cond1} of Theorem \ref{notion1} give us $$\Vdash_{\mathbb{P}'} ``cof(\gamma) \leq |\gamma| \leq \omega_1"$$ for all ordinals $\gamma$ satisfying $\lambda^{\dagger} \leq \gamma \leq 2^{< \lambda^{\dagger}}$, \ref{cond2} of Theorem \ref{notion1} must hold in view of \ref{4105}.
\end{proof}
...
\begin{align*}
    \kappa & := (2^{\lambda^{\dagger}})^+  \\
    \mathfrak{A} & := (H(\kappa); \in) \\
    R & := \{i < \lambda^{\dagger} : \omega_2 \leq i \text{ and } i \text{ is regular}\} \text{.}
\end{align*}

As $|H(\lambda^{\dagger})| = \lambda^{\dagger}$, we can fix a bijection $c : \lambda^{\dagger} \longrightarrow H(\lambda^{\dagger})$, and define
\begin{align*}
    Q_{\lambda} := \ & c" \lambda \\
    A_{\lambda} := \ & c" (\bar{A}_{\lambda}) \text{.}
\end{align*}
for each $\lambda < \lambda^{\dagger}$.

Making use of straightforward closure arguments, we inductively define $C$ such that $C$ is a club in $\lambda^{\dagger}$ and for all $\lambda \in C$, 
\begin{itemize}
    \item $Q_{\lambda}$ is transitive, and
    \item $(Q_{\lambda}; \in, c \cap Q_{\lambda}) \prec (H(\lambda^{\dagger}); \in, c)$.
\end{itemize}
We will let $Q_{\lambda^{\dagger}} = H(\lambda^{\dagger})$.

Now, given any $P, B \subset H(\lambda^{\dagger})$, the set
\begin{align*}
    \{\lambda \in C : (Q_{\lambda}; \in, P, B) \prec (H(\lambda^{\dagger}); \in, P, B)\}
\end{align*}
is a club in $\lambda^{\dagger}$. Moreover, we can derive from $(\bar{A}_{\lambda} : \lambda < \lambda^{\dagger})$ being a $\Diamond_{\lambda^{\dagger}}$-sequence, that the set
\begin{align*}
    \{\lambda \in C : B \cap Q_{\lambda} = A_{\lambda}\}
\end{align*}
is stationary in $\lambda^{\dagger}$. We thus obtain
\begin{itemize}[label=($\diamond$)]
    \item for all $P, B \subset H(\lambda^{\dagger})$, the set
    \begin{align*}
        \{\lambda \in C : (Q_{\lambda}; \in, P, A_{\lambda}) \prec (H(\lambda^{\dagger}); \in, P, B)\}
    \end{align*}
    is stationary in $\lambda^{\dagger}$.
\end{itemize}

We want to define $\mathbb{P}$ as a forcing notion comprising finite fragments of some language $\mathcal{L} \subset H(\lambda^{\dagger})$, such that $\mathbb{P}$ satisfies the hypothesis of Lemma \ref{lem26}.
...
$\mathfrak{A}$ is a transitive model of $\mathsf{ZFC - Powerset}$, and moreover $\mathcal{L} \subset H(\lambda^{\dagger}) \in H(\kappa)$, we conclude that $\mathfrak{A}$ is $\mathcal{L}$-suitable.
...
\end{enumerate}
with their domains restricted to $H(\lambda^{\dagger})$. Note that under this domain restriction, $\chi_1$, $\chi_2$, $\chi_3$ and $\chi_4$ are all members of $H(\kappa)$; in fact, they are all $\Delta_0$-definable functions in $(H(\lambda^{\dagger}); \in)$ (and thus in $\mathfrak{A}$). For brevity's and clarity's sake, we will abuse notation and suppress mention of these functions, whenever it is clear that we are using variables as placeholders for parameters in our construction of $\mathcal{L}^{*}_{\mathfrak{A}}$ formulas involving the symbol $\ulcorner E \urcorner$ --- see e.g. Definition \ref{defc}. Since 
\begin{itemize}
...
For any $\lambda \in C \cup \{\lambda^{\dagger}\}$, an object of the form $$\langle \langle F_i : i \in Z \rangle, \langle X_{\delta, \lambda} : \delta < \omega_1, \lambda \in C \rangle \rangle$$ \emph{interprets} $\mathcal{L} \cap Q_{\lambda}$ iff 
\begin{enumerate}[label=(\alph*)]
...
Given $\lambda \in C \cup \{\lambda^{\dagger}\}$, $$\mathfrak{C} := \langle \langle F_i : i \in Z \rangle, \langle X_{\delta, \lambda} : \delta < \omega_1, \lambda \in C \rangle \rangle$$
...
\end{enumerate}
It is clear that $\Sigma(\mathfrak{C}, \mathcal{L}) \cap Q_{\lambda} = \Sigma(\mathfrak{C}, \mathcal{L} \cap Q_{\lambda})$ is $\mathcal{L} \cap Q_{\lambda}$-nice for all $\lambda \in C \cup \{\lambda^{\dagger}\}$.
\end{defi}

\begin{rem}\label{rem416}
If 
\begin{itemize}
    \item $\lambda \in C \cup \{\lambda^{\dagger}\}$, 
...
We will define $\{\mathbb{P}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ by induction on $\lambda$. Assume that $\mathbb{P}_{\lambda'} = (P_{\lambda'}, \leq_{\lambda'})$ has been defined for all $\lambda' \in \lambda \cap C$. Also, for $\lambda' \in \lambda \cap C$, allow 
\begin{enumerate}[label=(\Alph*)]
...
The functions 
\begin{align*}
    Ef_0^{\lambda} : \ & \lambda \cap C \longrightarrow H(\lambda^{\dagger}) \ [\lambda' \mapsto Ef_0(\lambda')] \text{,} \\
    Ef_1^{\lambda} : \ & \lambda \cap C \longrightarrow H(\lambda^{\dagger}) \ [\lambda' \mapsto Ef_1(\lambda')] \text{, and}  \\
    Df^{\lambda} : \ & \lambda \cap C \longrightarrow H(\lambda^{\dagger}) \ [\lambda' \mapsto Df(\lambda')]
\end{align*}
...
\begin{enumerate}[label=(S\arabic*)$_{\lambda}$, leftmargin=40pt]
    ...
    \!
    $\begin{aligned}[t]
        \ulcorner & \forall \delta \ \forall \lambda' \ \forall \phi \ \forall \bar{s} \ \forall x \ \exists p \in P_{\lambda'} \\
        & ((E(\ulcorner x \in \dot{X}_{\delta, \lambda'} \urcorner) \ \wedge \\
...
We let $\mathbb{P}$ denote $\mathbb{P}_{\lambda^{\dagger}}$. 

\begin{lem}\label{size}
$P_{\lambda^{\dagger}} \subset H(\lambda^{\dagger})$, i.e. $\mathbb{P}$ fulfils \ref{4100} of Lemma \ref{lem26}.
\end{lem}

\begin{proof}
This follows immediately from the observation that
\begin{equation*}
    P_{\lambda^{\dagger}} \subset [\mathcal{L}]^{<\omega} \subset H(\lambda^{\dagger}) \text{.} \qedhere
\end{equation*}
\end{proof}

By Proposition \ref{certgood} and the lemma below, $(\mathfrak{A}, \mathbb{P})$ is good for $\mathcal{L}$. Obviously, $\mathbb{P}$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$ because $\mathbb{P} \in H(\kappa)$.

\begin{lem}\label{nonemp}
For all $\lambda \in C \cup \{\lambda^{\dagger}\}$, $\emptyset \in P_{\lambda}$.
\end{lem}

\begin{proof}
Let $g$ be $Col(\omega, |H(\kappa)|)$-generic over $W$. In $W[g]$, for every $i \in R$, choose a cofinal map from $\omega$ into $i$ and call it $F_i$. For every $\delta < \omega_1$ and every $\lambda \in C$, let $X_{\delta, \lambda}$ be the empty set. Then $$\mathfrak{C} := \langle \langle F_i : i \in R \rangle, \langle X_{\delta, \lambda} : \delta < \omega_1, \lambda \in C \rangle \rangle$$ interprets $\mathcal{L}$ and $\Sigma(\mathfrak{C}, \mathcal{L} \cap Q_{\lambda})$ $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$ for all $\lambda \in C \cup \{\lambda^{\dagger}\}$.  
\end{proof}
...
If 
\begin{itemize}
    \item $\lambda_0, \lambda_1 \in C \cup \{\lambda^{\dagger}\}$, 
...
\begin{enumerate}[label=(P\arabic*), leftmargin=40pt]
    \item\label{p1} $P_{\lambda_0} = P_{\lambda_1} \cap Q_{\lambda_0}$ whenever $\lambda_0, \lambda_1 \in C \cup \{\lambda^{\dagger}\}$ and $\lambda_0 \leq \lambda_1$, and
    \item\label{p2} $P_{\lambda} = \bigcup \{P_{\lambda'} : \lambda' \in C \cap \lambda\}$ whenever $\lambda \in C \cup \{\lambda^{\dagger}\}$ and $\mathrm{sup}(\lambda \cap C) = \lambda$.
\end{enumerate}

\begin{lem}\label{modeldone}
Let $\lambda' \in C \cup \{\lambda^{\dagger}\}$ and $g$ be a $\mathbb{P}_{\lambda'}$-$\Sigma_1$-generic filter over $W$. Then $\bigcup g \ \Gamma_{\lambda'} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } \emptyset$.
\end{lem}
...
\begin{itemize}
    ...
    \item $\mathbb{P}_{\lambda'}$ being non-empty (per Lemma \ref{nonemp}) implies there is $\Sigma$ in some outer model $W'$ of $W$ such that $\Sigma$ $\Gamma_{\lambda'} (\mathcal{L} \cap Q_{\lambda'}, \mathfrak{A})$-certifies $\emptyset$,
\end{itemize}
...
\begin{proof}
For any $\mathbb{P}_{\lambda'}$-generic filter $g$ over $W$, $\bigcup g \models^*_{\mathfrak{A}} \Gamma_{\lambda^{\dagger}}$ by Lemma \ref{modeldone}. In particular, $\bigcup g \models^*_{\mathfrak{A}} \text{(S2)}_{\lambda^{\dagger}}$. That $\mathbb{P}$ fulfils \ref{4101} of Lemma \ref{lem26} follows immediately.
\end{proof}
...
\begin{enumerate}[label=(\Alph*)]
    ...
    \item\label{def423} $U(C, \lambda^{\dagger})$ to denote the set of all subsets of $C$ unbounded in $\lambda^{\dagger}$.
\end{enumerate}
Check that both $S(\omega_1)$ and $U(C, \lambda^{\dagger})$ are members of $H(\kappa)$.

\begin{lem}\label{sideuni}
The $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentence 
\begin{align}\label{c9}
    \ulcorner \forall x \in H(\lambda^{\dagger}) \ \forall S \in S(\omega_1) \ \forall U \in U(C, \lambda^{\dagger}) \ \exists \mu \in S \ \exists \nu \in U \ (E(\ulcorner x \in \dot{X}_{\mu, \nu} \urcorner)) \urcorner
\end{align} 
...
\begin{itemize}
    \item $p \in \mathbb{P}$,
    \item $x \in H(\lambda^{\dagger})$,
    \item $S \in S(\omega_1)$, and
    \item $U \in U(C, \lambda^{\dagger})$.
\end{itemize}
...
Let $e$ be $Col(\omega, j(\lambda^{\dagger}))$-generic over $W[g \times f]$, and set
\begin{align*}
...
\begin{enumerate}[label=<$k$> $\eq$ \arabic* :, leftmargin=70pt]
    ...
    \begin{enumerate}[label=(K7.\arabic*), leftmargin=50pt]
        ...
        \item\label{k72} $(\phi, \bar{s}) \in j(Ef_1^{\lambda^{\dagger}})(\lambda)$, and
    ...
    \end{enumerate}
    so $(\phi, j^{-1}(\bar{s})) \in Ef_1^{\lambda^{\dagger}}(\lambda')$. Henceforth, there are two possible cases. We will analyse them with reference to the way $\Sigma^*$ is constructed. 
    \begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \begin{enumerate}[label=(K8.\arabic*), leftmargin=50pt]
        \item $\ulcorner x \in \dot{X}_{\delta, \lambda} \urcorner \in \Sigma^*$,
        \item\label{k82} $(\phi, \bar{s}) \in j(Df^{\lambda^{\dagger}})(\lambda)$, and
    ...
    \end{enumerate}
    so $(\phi, j^{-1}(\bar{s})) \in Df^{\lambda^{\dagger}}(\lambda')$. Henceforth, there are two possible cases. We will analyse them with reference to the way $\Sigma^*$ is constructed. 
    \begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
...
\begin{itemize}
    \item $\Sigma^* \in W^*$,
    \item $\Sigma^*$ $j(\Gamma_{\lambda^{\dagger}})(j(\mathcal{L}), j(\mathfrak{A}))$-certifies $q^*$,
    \item $W^*$ is an outer model of $M$, and
...
\begin{align*}
    U := \{\lambda \in C : (Q_{\lambda}; \in, \mathbb{P}, A_{\lambda}) \prec (H(\lambda^{\dagger}); \in, \mathbb{P}, D)\}
\end{align*}
is stationary in $\lambda^{\dagger}$, so $U \in U(C, \lambda^{\dagger})$.

In $W[g]$, there are $\mu \in S$ and $\nu \in U$ such that 
\begin{align*}
    \emptyset \neq (X_{\mu, \nu}(\bigcup g); \in, \mathbb{P}, A_{\nu}) \prec (Q_{\nu}; \in, \mathbb{P}, A_{\nu}) \prec (H(\lambda^{\dagger}); \in, \mathbb{P}, D),
\end{align*}
since $$\bigcup g \models^*_{\mathfrak{A}} \Gamma_{\lambda^{\dagger}} \text{ (in particular } \bigcup g \models^*_{\mathfrak{A}} \text{(S7)}_{\lambda^{\dagger}} \text{) and } \bigcup g \models^*_{\mathfrak{A}} \text{(}\ref{c9}\text{)}$$ by Lemmas \ref{uni}, \ref{modeldone} and \ref{sideuni}. Now, noting \ref{p1}, we have 
...
The set $$E_{\zeta} := \{q \in \mathbb{P} : \exists \eta > \zeta \ ((q, \eta) \in D)\}$$ is dense in $\mathbb{P}$, so (\ref{eseq}) tells us $$E_{\zeta} \cap Q_{\nu} = \{q \in \mathbb{P}_{\nu} : \exists \eta > \zeta \ ((q, \eta) \in A_{\nu})\}$$ is dense in $\mathbb{P}_{\nu}$. As $\bigcup g \models^*_{\mathfrak{A}} \text{(S5)}_{\lambda^{\dagger}}$, we know $\zeta \in X_{\mu, \nu}(\bigcup g)$. Having $\bigcup g \models^*_{\mathfrak{A}} \text{(S8)}_{\lambda^{\dagger}}$ then bestows us the existence of some $$q \in [\bigcup g]^{< \omega} \cap X_{\mu, \nu}(\bigcup g) \cap E_{\zeta} \cap Q_{\nu} \neq \emptyset.$$ That $q \in X_{\mu, \nu}(\bigcup g)$ and (\ref{eseq}) holds means $$(X_{\mu, \nu}(\bigcup g); \in, \mathbb{P}, D) \models \exists \eta > \zeta \ ((q, \eta) \in D).$$ Invoking the fact that $\bigcup g \models^*_{\mathfrak{A}} \text{(S6)}_{\lambda^{\dagger}}$ gives us some $\eta$ such that $\zeta < \eta < \mu$ and $(q, \eta) \in D$. Recalling the definition of $D$, we conclude $\eta \in \dot{C}[g]$ because $q \in [\bigcup g]^{< \omega} = g$, 
\end{proof}

\begin{lem}\label{2done}
$\Vdash_{\mathbb{P}} ``cof(\lambda^{\dagger}) = \omega_1"$. That is, $\mathbb{P}$ fulfils \ref{4102} of Lemma \ref{lem26}.
\end{lem}

\begin{proof}
Let $g$ be $\mathbb{P}$-generic over $W$. By Lemma \ref{modeldone}, $\bigcup g \models^*_{\mathfrak{A}} \Gamma_{\lambda^{\dagger}}$, so that in $W[g]$, $$K := \{(\delta, \lambda) : \exists x \ (\ulcorner x \in \dot{X}_{\delta, \lambda} \urcorner \in g)\}$$ is a strictly increasing function with domain contained in $\omega_1^W$ and range contained in $\lambda^{\dagger}$. Lemma \ref{sideuni} tells us that $dom(K)$ is cofinal in $\omega_1^W$ and $ran(K)$ is cofinal in $\lambda^{\dagger}$, hence $$cof^{W[g]}(\lambda^{\dagger}) = cof^{W[g]}(\omega_1^W).$$ By Lemma \ref{statdone}, we have $\omega_1^W = \omega_1^{W[g]}$, and consequently, 
\begin{equation*}
    cof^{W[g]}(\lambda^{\dagger}) = \omega_1^{W[g]}.
    \qedhere
\end{equation*}
\end{proof}
...
\subsection{Intuition Behind the Forcing Definition}\label{intuit}
...
Let us use the proof of Theorem \ref{notion1} to illustrate this. The key components of the proof are the objects
\begin{equation*}
    \lambda^{\dagger}, \mathfrak{A}, \mathcal{L}, C, \{(Q_{\lambda}, A_{\lambda}) : \lambda \in C\}, \Gamma_{base}, \Gamma_{side1}(\Vec{P}), \Gamma_{side2}(P) \text{,}
\end{equation*}
in $W$, where 
\begin{itemize}
    \item $\Gamma_{base}$ is the set containing just \ref{c1} to \ref{c3},
    \item $\Gamma_{side1}(\Vec{P})$ is the set containing just \ref{c4} to \ref{c8}, and
...
\begin{enumerate}[label=(\alph*)]
    \item\label{intuita} $trcl(\mathcal{L})$ is sufficiently small relative to $\mathfrak{A}$.
    ...
    \item\label{intuitd} $\Gamma_{side2}(P)$ is a set of $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentences parametrised by a forcing notion $P$ and possibly having members of $\mathrm{Fd}$ as fixed parameters.
\end{enumerate}
...
\begin{enumerate}[leftmargin=50pt, label=Step \arabic*:]
    \item\label{remstep1} Inductively construct forcing notions along $C$ such that for every $\lambda \in C \cup \{\lambda^{\dagger}\}$, 
    \begin{align*}
        P_{\lambda} = \ & \mathrm{AF}(\mathfrak{A}, \mathcal{L} \cap Q_{\lambda}, \Gamma_{base} \cup \Gamma_{side1}(\Vec{\mathbb{P}}_{\lambda})) \\
        \leq_{\lambda} \ = & \subset \cap \ P_{\lambda} \\
        \mathbb{P}_{\lambda} = \ & (P_{\lambda}, \leq_{\lambda}) \text{,}
    \end{align*}
    where $$\Vec{\mathbb{P}}_{\lambda} := \{\mathbb{P}_{\lambda'} : \lambda' \in C \cap \lambda\} \text{.}$$
    \item Setting $\mathbb{P} := \mathbb{P}_{\lambda^{\dagger}}$, show that $\Gamma_{side2}(\mathbb{P})$ is $(\mathfrak{A}, \mathbb{P}, \mathcal{L})$-universal.
\end{enumerate}
...
\subsection{Key Definitions and a Fact}
...
\begin{itemize}
    ...
    \item $\lambda^{\dagger} > \omega_2$ be a regular cardinal.
\end{itemize}
Then there is a stationary-preserving forcing notion $\mathbb{P}$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\alph*)]
    \item $\mathbb{P}$ witnesses $Nb'_1(\lambda^{\dagger})$, and
...
Move to $W$ as in the proof of Theorem \ref{notion1} via forcing with $Col(\lambda^{\dagger}, \lambda^{\dagger})$, so that 
\begin{itemize}[label=($\diamond$)]
    \item for all $P, B \subset H(\lambda^{\dagger})$, the set
    \begin{align*}
        \{\lambda \in C : (Q_{\lambda}; \in, P, A_{\lambda}) \prec (H(\lambda^{\dagger}); \in, P, B)\}
...
\begin{enumerate}[label=(\Alph*)]
    ...
    \item $\mathcal{L} \cap Q_{\lambda} = \mathcal{L}^s \sqcup (\mathcal{L}^o \cap Q_{\lambda})$ for every $\lambda \in C \cup \{\lambda^{\dagger}\}$.
\end{enumerate}
...
The theorem holds if in $W$, we can define a forcing notion $\mathbb{P} = (P, \leq_P)$ with the following properties: 
\begin{enumerate}[label=(K\arabic*), leftmargin=40pt]
    \item\label{K1} $P \subset H(\lambda^{\dagger})$,
    ...
    \end{enumerate} 
    \item\label{K4} $\Vdash_{\mathbb{P}} ``cof(\alpha) = \omega"$ for all regular cardinals $\alpha$ satisfying $\omega_2 \leq \alpha < \lambda^{\dagger}$,
    \item\label{K5} $\Vdash_{\mathbb{P}} ``cof(\lambda^{\dagger}) = \omega_1"$,
\end{enumerate}
\end{fact}
...
Let $\lambda \in C \cup \{\lambda^{\dagger}\}$. A $\lambda$\emph{-certificate} is a tuple 
\begin{equation*}
...
One can easily verify that being a $\lambda$-certificate, for any $\lambda \in C \cup \{\lambda^{\dagger}\}$, is absolute for weak outer models of $W$. If a $\lambda$-certificate shows up in some context without reference to the universe it inhabits, we may assume said universe to be any weak outer model of $W$.
...
\begin{itemize}
    ...
    satisfying 
    \begin{itemize}[label=$\circ$]
        \item \ref{cd0} and \ref{cdlast} of Definition \ref{def437} for some $\lambda \in C \cup \{\lambda^{\dagger}\}$, 
...
It is clear that $\Sigma'(\mathfrak{D}, \mathcal{L}) \cap Q_{\lambda} = \Sigma'(\mathfrak{D}, \mathcal{L} \cap Q_{\lambda})$ is $\mathcal{L} \cap Q_{\lambda}$-nice for all $\lambda \in C \cup \{\lambda^{\dagger}\}$.

\begin{defi}
Let $\Gamma'$ be the following set
\begin{align*}
    \{\phi : \ & \phi \text{ is a } (\mathcal{L}^s)^*_{\mathfrak{A}}\text{-}\Pi_2 \text{ sentence and} \\
    & \Vdash_{Col(\omega, \lambda^{\dagger})} \forall \mathfrak{D} \ \forall \lambda \in C \cup \{\lambda^{\dagger}\} \\
...
Notice whenever $\phi$ is a $(\mathcal{L}^s)^*_{\mathfrak{A}}$-$\Pi_2$ sentence, it must be the case that in any $Col(\omega, \lambda^{\dagger})$-generic extension of $V$, 
\begin{equation*}
    \varphi(\phi) := \forall \mathfrak{D} \ \forall \lambda \in C \cup \{\lambda^{\dagger}\} \ (``\mathfrak{D} \text{ is a } \lambda \text{-certificate} \implies \Sigma'(\mathfrak{D}, \mathcal{L} \cap Q_{\lambda}) \models^*_{\mathfrak{A}} \phi")
\end{equation*}
is equivalent to a $\mathbf{\Pi^1_2}$ sentence. We can thus employ an argument akin to that which proved Lemma \ref{inout}, to obtain the fact below.
...
\begin{enumerate}[label=(\arabic*)$_{\phi}$]
    \item\label{4491} $\Vdash_{Col(\omega, \lambda^{\dagger})} \varphi(\phi)$.
    \item\label{4492} $\mspace{-3mu}\not \mspace{3mu}\Vdash_{Col(\omega, \lambda^{\dagger})} \neg \varphi(\phi)$.
    \item $W \models \varphi(\phi)$ for every outer model $W$ of $V$.
\end{enumerate}
\end{fact}

\begin{rem}
That statements \ref{4491} and \ref{4492} of Fact \ref{fact449} are equivalent can also be derived directly from the homogeneity of $Col(\omega, \lambda^{\dagger})$.
\end{rem}
...
\leavevmode
\begin{enumerate}[label=(\arabic*)]
    \item\label{443'1} If $\lambda \in C \cup \{\lambda^{\dagger}\}$ and
    \begin{equation*}
        \mathfrak{D} = \langle \langle \bar{M}_i, \pi_{ij},  \bar{N}_i, \sigma_{ij} : i \leq j \leq \omega_1^{W} \rangle, S, \langle e_i : i < \omega_1^W \rangle, \mathfrak{C} \rangle
    \end{equation*}
    ...
    \end{equation*}
    \item\label{443'2} In part due to \ref{443'1}, as long as $\lambda \in C \cup \{\lambda^{\dagger}\}$ and $\mathfrak{D}$ is a $\lambda$-certificate, it must be that $\Sigma'(\mathfrak{D}, \mathcal{L} \cap Q_{\lambda})$ $\Gamma'_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$.
    \item\label{443'3} If $\lambda \in C \cup \{\lambda^{\dagger}\}$ and $\Sigma \ \Gamma'_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } \emptyset$, then also
    \begin{equation*}
...
\end{align*}
As before, let $\mathbb{P}$ denote $\mathbb{P}_{\lambda^{\dagger}}$. 
...
\begin{proof}
Fix $\lambda \in C \cup \{\lambda^{\dagger}\}$ and let 
\begin{itemize}
...
Now let $h'$ be $Col(\omega, \pi(\lambda^{\dagger}))$-generic over $W[h]$, so that in $W[h][h']$, there is 
\begin{equation*}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item $(\Sigma')^{\pi, M}(\mathfrak{D}, \pi(\mathcal{L} \cap Q_{\lambda}))$ is a set found in some outer model of $M$, and
...
Let $\lambda \in C \cup \{\lambda^{\dagger}\}$, and $g$ be a $\mathbb{P}_{\lambda}$-$\Sigma_1$-generic filter over $W$. Then $\bigcup g \ \Gamma'_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } \emptyset$.
\end{lem}
...
\begin{equation*}
    \mathfrak{D}(g) = \langle \langle \bar{M}_i, \pi_{ij}, \bar{N}_i, \sigma_{ij} : i \leq j \leq \omega_1^{W} \rangle, S, \langle e_i : i < \omega_1^W \rangle, \mathfrak{C} \rangle
\end{equation*}
from $g$, checking that $\mathfrak{D}(g)$ fulfils the conditions of Definition \ref{def437} as we go along. The reader ought to check for themselves that 
\begin{enumerate}[label=(\Roman*)]
...
\begin{split}
    \ulcorner & \forall x \in H(\lambda^{\dagger}) \ \forall U \in U(C, \lambda^{\dagger}) \ \forall i < \omega_1^W \ \forall m < \omega \ \exists \delta < \omega_1^W \ \exists n < \omega \ \exists \nu \in U \\ 
...
\end{split}
\end{align}
where $U(C, \lambda^{\dagger})$ is as in Definition \ref{def423}, is $(\mathfrak{A}, \mathbb{P}, \mathcal{L})$-universal.
\end{lem}
...
\end{enumerate} 
where $\lambda := \lambda^{\dagger}$. Indeed, let us stipulate that $\mathfrak{C}'$ be constructed the same way $\mathfrak{C}$ is in the proof of Lemma \ref{sideuni}. By \ref{4344}, \ref{4346b}, \ref{4346}, \ref{451b'} and the elementarity of $j$, 
\begin{equation*}
...
\begin{enumerate}[label=(\alph*)]
    \setcounter{enumi}{4}
    \item\label{451c} $\Sigma'(\mathfrak{D}', j(\mathcal{L}))$ is a set found in some outer model of $M$,
...
Move to $W[g]$. There, due to Lemma \ref{lem446}, $\mathfrak{D}(g)$ is a $\lambda^{\dagger}$-certificate and
\begin{equation*}
    \Sigma'(\mathfrak{D}(g), \mathcal{L}) = \bigcup g \text{.}
\end{equation*}
...
\begin{proof}
Let $g$ be $\mathbb{P}$-generic over $W$, have $\lambda$ denote $\lambda^{\dagger}$, and work in $W[g]$. Consider the $\lambda$-certificate
\begin{equation*}
...
\begin{itemize}
    ...
    \item $\lambda$ denote $\lambda^{\dagger}$, 
\end{itemize} 
...
Then there is a stationary-preserving forcing notion $\mathbb{P}$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\alph*)]
    \item $\mathbb{P}$ witnesses $Nb'_1(\lambda^{\dagger})$ (thus also $Nb_1(\lambda^{\dagger})$), and
...
\section{Discussion: Obstacles to Natural Analogues}\label{sect74}

This section provides a cursory investigation into some of the constructions that transpired in the proofs of Theorems \ref{notion1} and \ref{notion2}, and natural analogues thereof. The said investigation is carried out in four different directions, each of which is allocated its own subsection.

\subsection{Towards a Two-cofinality Generalisation}\label{2cof}

Let us first recapitulate a part of Subsection \ref{intuit}, namely the high-level description of the forcing construction integral to the proof of Theorem \ref{notion1}. For the rest of this subsection and the next one, we adopt the notation of Subsection \ref{intuit}, so that the major components of the proof of Theorem \ref{notion1} are labelled 
\begin{equation*}
    \lambda^{\dagger}, \mathfrak{A}, \mathcal{L}, C, \{(Q_{\lambda}, A_{\lambda}) : \lambda \in C\}, \Gamma_{base}, \Gamma_{side1}(\Vec{P}) \text{ and } \Gamma_{side2}(P)
\end{equation*}
accordingly, from the perspective of the universe $W$ as defined in the proof of Theorem \ref{notion1}. We shall also work in $W$ for the rest of this subsection and the next. 

Now, expand $\mathfrak{A}$, $\mathcal{L}$, $\Gamma_{base}$, $\Gamma_{side1}(\Vec{P})$ and $\Gamma_{side2}(P)$ such that 
\begin{itemize}
    \item $\mathfrak{A} = (A; \in, \Vec{R})$,
    \item $(A; \in)$ is a transitive model of $\mathrm{ZFC - Powerset}$, and
    \item \ref{intuita} to \ref{intuitd} in Subsection \ref{intuit} are satisfied with $\mathrm{Fd}$ denoting the set 
    \begin{equation*}
        \{\mathcal{L}, C, \{(Q_{\lambda}, A_{\lambda}) : \lambda \in C\}\} \text{,}
    \end{equation*}
\end{itemize}
before inductively constructing $P_{\lambda}$, $\leq_{\lambda}$, $\mathbb{P}_{\lambda}$ and $\Vec{\mathbb{P}}_{\lambda}$ for $\lambda \in C \cup \{\lambda^{\dagger}\}$ following \hyperref[remstep1]{Step 1} of Subsection \ref{intuit}, where $\mathrm{AF}(\mathfrak{A}', \mathcal{L}', \Gamma')$ denote
\begin{equation*}
    \{p \in [\mathcal{L}']^{< \omega} : \ \Vdash_{Col(\omega, |trcl(\mathfrak{A}')|)} \exists \Sigma \ (``\Sigma \ \Gamma'(\mathcal{L}', \mathfrak{A}') \text{-certifies } p")\}
\end{equation*}
whenever $\mathfrak{A}'$, $\mathcal{L}'$ and $\Gamma'$ respectively fulfil the properties of $\mathfrak{A}$, $\mathcal{L}$ and $\Gamma$ in Section \ref{forframe}. Finally, set $\mathbb{P} := \mathbb{P}_{\lambda^{\dagger}}$.

When a forcing notion $\mathbb{P}$ is obtained through the process highlighted in the previous paragraph, we say $\mathbb{P}$ is \textit{additively defined from the proof of Theorem \ref{notion1}} (in short, \textit{additively defined}), precisely because the said process is a form of additive construction built upon the skeleton that is the proof of Theorem \ref{notion1}. Note that this is actually a shorthand because the definition of such a $\mathbb{P}$ is contingent on the definitions of a slew of other objects --- so it is really the tuple $(\mathfrak{A}, \mathcal{L}, \Gamma_{base}, \Gamma_{side1}(\Vec{P}), \Gamma_{side2}(P))$ that is additively defined. As prefaced (or rather, preempted) in Subsection \ref{intuit}, the main theorems of Section \ref{ss43} are crucially dependent on forcing notions which are additively defined. In this subsection, we briefly explore the use of such additive definitions in proving a possible two-cofinalty generalisation of the Namba extension problem. 

\begin{defi}
Define 
\begin{equation*}
    R(\lambda) := \{i < \lambda : \omega_2 \leq i \text{ and } i \text{ is regular}\} \text{.}
\end{equation*}
For any set $\mathrm{Sb}$, let $Nb_2(\lambda, \mathrm{Sb})$ denote the statement
\begin{align*}
    \text{`} & \text{there is a stationary-preserving forcing notion } \mathbb{P} \text{ such that} \\ 
    & \Vdash_{\mathbb{P}} ``cof(\alpha) = \omega" \text{ for all } \alpha \in R(\lambda) \setminus \mathrm{Sb} \text{,} \\
    & \Vdash_{\mathbb{P}} ``cof(\gamma) = \omega_1" \text{ for all } \gamma \in R(\lambda) \cap \mathrm{Sb} \text{, and} \\
    & \Vdash_{\mathbb{P}} ``cof(\gamma') = \omega_1" \text{ for all regular cardinals } \gamma' \text{ satisfying } \lambda \leq \gamma' \leq 2^{< \lambda} \text{, and} \\
    & \Vdash_{\mathbb{P}} ``cof(\beta) = \beta" \text{ for all regular cardinals } \beta \text{ satisfying } 2^{< \lambda} < \beta \text{'.}
\end{align*}
\end{defi}

Jensen first considered statements similar to $Nb_2$ in \cite{lforcing}. There, he showed that $Nb_2(\lambda, \mathrm{Sb})$ for every set $\mathrm{Sb}$ whenever $\lambda$ is strongly inaccessible. Recent results by De Bondt and Veli\v{c}kovi\'{c} (\cite{bendb}, as part of De Bondt's PhD dissertation) imply that $Nb_2(\lambda, \mathrm{Sb})$ for every regular $\lambda > \omega_2$ and every set $\mathrm{Sb}$. 

It would be nice to have the following strengthening of Theorem \ref{notion1}, so that modulo an assumption beyond $\mathsf{ZFC}$, we can improve on a facet of De Bondt's and Veli\v{c}kovi\'{c}'s results by finding smaller witnesses to $Nb_2$.

\begin{conj}\label{notion4}
Assume $\mathrm{NS}_{\omega_1}$ is precipitous. Then $Nb_2(\lambda^{\dagger}, \mathrm{Sb})$ holds for every regular cardinal $\lambda^{\dagger} > 2^{\omega_1}$ and every set $\mathrm{Sb}$, and is witnessed by a forcing notion of size $2^{< \lambda^{\dagger}}$. In other words, whenever $\lambda^{\dagger} > 2^{\omega_1}$ is a regular cardinal and $\mathrm{Sb}$ is a set, there is a stationary-preserving forcing notion $\mathbb{P}'$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\arabic*)]
    \item\label{cond1+} $\Vdash_{\mathbb{P}'} ``cof(\alpha) = \omega"$ for all $\alpha \in R(\lambda^{\dagger}) \setminus \mathrm{Sb}$,
    \item\label{cond2+} $\Vdash_{\mathbb{P}} ``cof(\gamma) = \omega_1"$ for all $\gamma \in R(\lambda^{\dagger}) \cap \mathrm{Sb}$,
    \item $\Vdash_{\mathbb{P}'} ``cof(\gamma') = \omega_1"$ for all regular cardinals $\gamma'$ satisfying $\lambda^{\dagger} \leq \gamma' \leq 2^{< \lambda^{\dagger}}$, and
    \item\label{cond4+} $\Vdash_{\mathbb{P}'} ``cof(\beta) = \beta"$ for all regular cardinals $\beta$ satisfying $2^{< \lambda^{\dagger}} < \beta$.
\end{enumerate}
\end{conj}

It might be tempting to adapt the proof of Theorem \ref{notion1} in an attempt to prove Conjecture \ref{notion4}. We first show that a reduction via the natural analogue of Lemma \ref{lem26} for our purposes is possible. 

\begin{lem}\label{lem26an}
If in $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$ there is a stationary-preserving forcing notion $\mathbb{P} = (P, \leq_P)$ fulfilling 
\begin{enumerate}[label=(\arabic*')]
    \item\label{4100'} $P \subset H(\lambda^{\dagger})$, 
    \item\label{4101'} $\Vdash_{\mathbb{P}} ``cof(\alpha) = \omega"$ for all $\alpha \in R(\lambda^{\dagger}) \setminus \mathrm{Sb}$,
    \item\label{4101''} $\Vdash_{\mathbb{P}} ``cof(\gamma) = \omega_1"$ for all $\gamma \in R(\lambda^{\dagger}) \cap \mathrm{Sb}$, and
    \item\label{4102'} $\Vdash_{\mathbb{P}} ``cof(\lambda^{\dagger}) = \omega_1"$,
\end{enumerate}
then for some $Col(\lambda^{\dagger}, \lambda^{\dagger})$-name $\dot{\mathbb{P}}$ for $\mathbb{P}$,
\begin{equation*}
    Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}}
\end{equation*} 
is a stationary-preserving forcing notion that contains a dense subset of size $2^{< \lambda^{\dagger}}$ and fulfils \ref{cond1+} to \ref{cond4+} of Conjecture \ref{notion4} in $V$.
\end{lem}

\begin{proof}
Working in $V$, set 
\begin{equation*}
    \mathbb{P}' := Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}} \text{.}
\end{equation*} 
Then $\mathbb{P}'$ is a stationary-preserving forcing notion fulfilling \ref{cond1+} and \ref{cond2+} of Conjecture \ref{notion4} because 
\begin{itemize}
    \item $Col(\lambda^{\dagger}, \lambda^{\dagger})$ is a stationary-preserving forcing notion forcing $\dot{\mathbb{P}}$ to be stationary-preserving, 
    \item $Col(\lambda^{\dagger}, \lambda^{\dagger})$ forces $cof(\alpha) = cof^{V}(\alpha)$ for all $ \alpha < \lambda^{\dagger}$, and
    \item \ref{4101'} and \ref{4101''} hold.
\end{itemize}
The rest of this proof is identical to that of Lemma \ref{lem26}.
\end{proof}

In order to prove Conjecture \ref{notion4}, it is therefore sufficient to exhibit in $W$ a stationary-preserving forcing notion $\mathbb{P} = (P, \leq_P)$ fulfilling \ref{4100'} to \ref{4102'} of Lemma \ref{lem26an}. The most straightforward way to find such a $\mathbb{P}$, considering the knowledge we so far possess, is perhaps to additively define it.

To this end, retroactively set $R := R(\lambda^{\dagger}) \setminus \mathrm{Sb}$. Also, set $R' := R(\lambda^{\dagger}) \cap \mathrm{Sb}$.

\begin{defi}\label{defl'}
The language $\mathcal{L}'$ requires the distinguished symbols $\dot{F}'_i$ for $i \in R'$. Now fix $\mathcal{L}'$ to be the smallest set closed under negation, that contains expressions (strings) of the type
\begin{enumerate}[label=(L\arabic*), leftmargin=40pt]
    \setcounter{enumi}{2}
    \item $\ulcorner \dot{F}'_i (\beta) = \alpha \urcorner$, for $i \in R'$, $\beta < \omega_1$ and $\alpha < i$.
\end{enumerate}
\end{defi}

\begin{defi}
For any pair $(i, \Sigma)$, define $F'_i(\Sigma)$ to be the set 
\begin{equation*}
    \{(\beta, \alpha) : \ulcorner \dot{F}'_i(\beta) = \alpha \urcorner \in \Sigma\} \text{.}
\end{equation*}
\end{defi}

Expand $\mathcal{L}$ to just include $\mathcal{L}'$ and expand $\Gamma_{base}$ to just include the singleton set containing
\begin{align*}
    \varphi_{new} := \ulcorner & \forall i \ \forall \beta_0 \ \forall \beta_1 \ \forall \alpha_0 \ \forall \alpha_1 \\
    & ((E(\ulcorner \dot{F}'_i (\beta_0) = \alpha_0 \urcorner) \wedge E(\ulcorner \dot{F}'_i (\beta_1) = \alpha_1 \urcorner) \wedge \beta_0 \in \beta_1) \\
    & \implies \alpha_0 \in \alpha_1) \urcorner \text{.}
\end{align*}
In other words, reassign
\begin{align*}
    \mathcal{L} := \ & \mathcal{L} \cup \mathcal{L}' \\
    \Gamma_{base} := \ & \Gamma_{base} \cup \{\varphi_{new}\} \text{.}
\end{align*}

In principle, $\varphi_{new}$ means to say that $F'_i(\Sigma)$ is strictly increasing as long as $\Sigma$ interprets the unary relation symbol $\ulcorner E \urcorner$ occurring in $\mathcal{L}^*_{\mathfrak{A}}$ (formulas).

Recall that the crux of the proof of Theorem \ref{notion1} lies in Lemma \ref{sideuni}, and the proof of Lemma \ref{sideuni} hinges on the ability to extend $j" (\bigcup g)$ to $j(\Gamma_{\lambda^{\dagger}})(j(\mathcal{L}), j(\mathfrak{A}))$-certify a particular member of $j([\mathcal{L}]^{< \omega})$, where
\begin{itemize}
    \item $j$ is a fixed elementary embedding of $W$ into some inner model of $W$ with critical point $\omega_1^W$,
    \item $j$ possesses the following property: $\bigcup j" A = j(\bigcup A)$ as long as $A$ is a set of ordinals with supremum being a regular cardinal of $W$ above $\omega^W_1$, 
    \item $g$ is a $\mathbb{P}_{\lambda}$-generic filter over $W$ for some $\lambda \in C$, and
    \item $\Gamma_{\lambda}$ can be considered definitionally as $\Gamma_{base} \cup \Gamma_{side1}(\Vec{\mathbb{P}}_{\lambda})$ whenever $\lambda \in C \cup \{\lambda^{\dagger}\}$.
\end{itemize}
Now notice that given our newly reassigned $\mathcal{L}$ and $\Gamma_{\lambda}$'s, no extension of $j" (\bigcup g)$ can help make progress towards $i \in R'$ having an uncountable cofinality in $W^{\mathbb{P}}$ if $F'_i(\bigcup g)$ has domain bounded in $\omega_1^W$ and range unbounded in $i$. To continue leveraging on the current technology behind Lemma \ref{sideuni}, we would have to find the non-trivial expansions of $\mathcal{L}$, $\Gamma_{base}$, $\Gamma_{side1}(\Vec{P})$ and $\Gamma_{side2}(P)$ that help guard against this scenario, as $i$ ranges over $R'$. Only then does an additively defined forcing notion $\mathbb{P}$ have any hope of satisfying \ref{4100'} to \ref{4102'} of Lemma \ref{lem26an}.

\begin{defi}
For each $i \in R'$, let $\varphi^i_{req}$ be the member of $\mathcal{L}^*_{\mathfrak{A}}$ defined by
\begin{equation*}
    \varphi^i_{req} := \ulcorner \exists \gamma \ \forall \beta \ \forall \alpha \ (E(\ulcorner \dot{F}'_i (\beta) = \alpha \urcorner) \implies \alpha \in \gamma) \urcorner \text{.}
\end{equation*}
What $\varphi^i_{req}$ means to say, is that the range of $F'_i(\Sigma)$ is bounded in $i$ as long as $\Sigma$ interprets the unary relation symbol $\ulcorner E \urcorner$ occurring in $\mathcal{L}^*_{\mathfrak{A}}$ (formulas).
\end{defi}

Given any $i \in R'$, $\varphi^i_{req}$ is a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_2$ sentence. Hence we cannot just add it to $\Gamma_{base}$ and run it through our current forcing framework while expecting $\bigcup g \models^{*}_{\mathfrak{A}} \varphi^i_{req}$ for every $\mathbb{P}_{\lambda}$-generic filter $g$ over $W$ whenever $\lambda \in C \cup \{\lambda^{\dagger}\}$. Nevertheless, under the current side-condition technology exemplified by the proofs of Lemmas \ref{sideuni} and \ref{statdone}, we would require 
\begin{enumerate}[label=(TC\arabic*), leftmargin=50pt]
    \item\label{tc1} $\{\lambda \in C : \ \Vdash_{\mathbb{P}_{\lambda}} \text{``} \bigcup \check{G} \models^{*}_{\mathfrak{A}} \varphi^i_{req} \text{''}\}$
    to be a sufficiently large subset of $\lambda^{\dagger}$, and yet also that
    \item\label{tc2} $\neg\varphi^i_{req}$ is derivable as a $(\mathfrak{A}, \mathbb{P}, \mathcal{L})$-universal $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentence in $\Gamma_{side2}(\mathbb{P})$, so that $\Vdash_{\mathbb{P}} \text{``} \bigcup \check{G} \models^{*}_{\mathfrak{A}} \neg\varphi^i_{req} \text{''}$,
\end{enumerate}
where $\check{G}$ stands for the context sensitive canonical name for the generic filter --- it appearing in the forcing language of a forcing notion $\mathbb{Q}$ indicates that it is standing in for the canonical $\mathbb{Q}$-name for the $\mathbb{Q}$-generic filter. Letting $\varphi_{sc}$ be the negation of the $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentence (\ref{c9}), one should spot a parallel between $\varphi^i_{req}$ and $\varphi_{sc}$. To elaborate, notice that for the original set of $\mathbb{P}_{\lambda}$'s (which include $\mathbb{P}$ as originally defined) in the proof of Theorem \ref{notion1},
\begin{enumerate}[label=(TC\arabic*'), leftmargin=50pt]
    \item\label{tc1'} $\{\lambda \in C : \ \Vdash_{\mathbb{P}_{\lambda}} \text{``} \bigcup \check{G} \models^{*}_{\mathfrak{A}} \varphi_{sc} \text{''}\}$
    is a club subset of $\lambda^{\dagger}$, and
    \item\label{tc2'} $\neg\varphi_{sc}$ is derivable as a $(\mathfrak{A}, \mathbb{P}, \mathcal{L})$-universal $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentence in $\Gamma_{side2}(\mathbb{P})$, so that $\Vdash_{\mathbb{P}} \text{``} \bigcup \check{G} \models^{*}_{\mathfrak{A}} \neg\varphi_{sc} \text{''}$.
\end{enumerate}
The parallel between \ref{tc1} and \ref{tc1'}, as well as the parallel between \ref{tc2} and \ref{tc2'}, is thus obvious. 

As a certain level of uniformity in the definition of the $\mathbb{P}_{\lambda}$'s, namely \ref{p1} and \ref{p2} in the proof of Theorem \ref{notion1}, is required for the proof of Lemma \ref{statdone} to go through, the tension between \ref{tc1} and \ref{tc2} immediately rules out any naive expansions of the $\Gamma_{\lambda}$'s. The hunch that any forcing notion $\mathbb{P}$ --- not to mention an additively defined one --- satisfying \ref{4100'} to \ref{4102'} of Lemma \ref{lem26an} has to have a deviously clever construction is also indirectly supported by the fact that the only known method of resolving the tension between \ref{tc1'} and \ref{tc2'} is a complicated inductive construction (that of the $\mathbb{P}_{\lambda}$'s in the proof of Theorem \ref{notion1}, as per the proof of Lemma 2.14 of \cite{schindler}) without spiritual precedence.

What about riding on the coattails of the Asper\'{o}-Schindler side condition method? This seems to be our best bet at the moment, but observe that in the proof of Theorem \ref{notion1}, we have the ability to bound the range of the function
\begin{equation*}
    \{(\delta, \lambda') : X_{\delta, \lambda'}(\bigcup g) \neq \emptyset\}
\end{equation*}
by choosing an appropriate $\lambda \in C$ and taking $g$ to be any $\mathbb{P}_{\lambda}$-generic filter over $W$. To take a leaf out of the same book, we can try to fulfil \ref{tc1} on a meta level by organising the inductive construction of a sequence of forcing notions that incorporates the ordering of a club subset of $i$. 

The thorny portion of the aforementioned approach is in making the construction along a selected club subset $C'$ of $i$ cohere with the construction along $C$. The construction along $C$, as seen in the proof of Theorem \ref{notion1}, works because the extension of the $j$-image $\Sigma$ of a $\mathbb{P}_{\lambda}$-generic filter over $W$, an extension so crucial to proving Lemma \ref{sideuni}, is for all practical purposes an end extension. This is due to the fact that, according to the proof of Lemma \ref{sideuni}, the initial side condition of $\Sigma$ is extended by adding a new elementary substructure that completely covers (i.e. includes) $\Sigma$. Such end extensions can be done easily via $j$ as they are bound by few constraints. However, once $C'$ comes into the picture, there is seemingly  no way around dealing with extensions that are more constrained and elaborate, not unless we give up \ref{p1} and \ref{p2}. Both options would require certain innovation and ingenuity, in no small part because the proof of Lemma \ref{statdone}, which relies heavily on \ref{p1} and \ref{p2}, is evidently a core component of the Asper\'{o}-Schindler technology.

All in all, the upshot of the preceding discussion in this subsection can be (informally) summarised thus: a proof of Conjecture \ref{notion4}, if it were provable, would most likely involve either
\begin{itemize}
    \item a non-trivial augmentation to the basic skeleton of the Asper\'{o}-Schindler method, or 
    \item a totally different --- and in all likelihood, entirely new --- approach.
\end{itemize}

\subsection{\texorpdfstring{$\mathbb{P}$}{P} always adds reals}

Recall that we are still working in the universe $W$, with the objects
\begin{equation*}
    \lambda^{\dagger}, \mathfrak{A}, \mathcal{L}, C, \{(Q_{\lambda}, A_{\lambda}) : \lambda \in C\}, \{\mathbb{P}_{\lambda} = (P_{\lambda}, \leq_{\lambda}) : \lambda \in C \cup \{\lambda^{\dagger}\}\}
\end{equation*}
as defined in the proof of Theorem \ref{notion1}. We shall prove the following.

\begin{thm}\label{thm747}
For every $\lambda \in C \cup \{\lambda^{\dagger}\}$, $\mathbb{P}_{\lambda}$ adds reals.
\end{thm}

\begin{proof}
Let us first define a few transformations on subsets of $\mathcal{L}$.

\begin{defi}
Whenever $\Sigma \subset \mathcal{L}$ and $n < \omega$, let
\begin{align*}
    \mathrm{Ret}(\Sigma, n) := 
    \begin{cases}
        \alpha & \text{if there is a unique } \alpha \text{ for which } \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in \Sigma \\
        0 & \text{otherwise.}
    \end{cases}
\end{align*}
\end{defi}
...
\begin{defi}
Whenever $\Sigma \subset \mathcal{L}$, $n < \omega$, $z < 2$ and $y < 2$, let
\begin{align*}
    \mathrm{RetS}(\Sigma, n, z, y) := 
    \begin{cases}
    \!\begin{aligned}[t]
    \begin{split}
        \{\ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) \urcorner, \\
        \ulcorner \neg \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) + 1 \urcorner\}
    \end{split}
    \end{aligned} & \text{if } z = 1 \text{ and } y = 0 \\
    \!\begin{aligned}[t]
    \begin{split}
        \{\ulcorner \neg \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) \urcorner, \\
        \ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) + 1 \urcorner\}
    \end{split}
    \end{aligned} & \text{if } z = 1 \text{ and } y = 1 \\
    \!\begin{aligned}
    \begin{split}
        \{\ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) \urcorner, \\
        \ulcorner \neg \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) - 1 \urcorner\} 
    \end{split}
    \end{aligned} & \text{if } z = 0 \text{ and } y = 0 \\
    \!\begin{aligned}[t]
    \begin{split}
        \{\ulcorner \neg \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) \urcorner, \\
        \ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) - 1 \urcorner\}
...
\begin{defi}
Whenever $\Sigma \subset \mathcal{L}$, $n < \omega$ and $z < 2$, let
\begin{align*}
    \mathrm{BC}(\Sigma, n, z) = 
    \begin{cases}
    \!\begin{aligned}[t]
    \begin{split}
        \Sigma \setminus \mathrm{RetS}(\Sigma, n, z, 0) \\
        \cup \ \mathrm{RetS}(\Sigma, n, z, 1)
    \end{split}
    \end{aligned} & \text{if } z = 1 \\
    \!\begin{aligned}[t]
    \begin{split}
        \Sigma \setminus \mathrm{RetS}(\Sigma, n, z, 0) \\
        \cup \ \mathrm{RetS}(\Sigma, n, z, 1)
    \end{split}
    \end{aligned} & \text{if } z = 0 \text{ and } \mathrm{Ret}(\Sigma, n) - 1 \neq \mathrm{Ret}(\Sigma, n) \\
    \Sigma & \text{otherwise.}
    \end{cases}
\end{align*}
\end{defi}

It should be clear that the functions $\mathrm{Ret}$, $\mathrm{RetS}$ and $\mathrm{BC}$ are absolute for outer models of $W$. The next lemma will do the heavy lifting in proving the theorem.

\begin{lem}\label{lem7411}
Let $\lambda \in C \cup \{\lambda^{\dagger}\}$. If in some outer model $W'$ of $W$ we have some $\Sigma$ that $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$, then for every $n < \omega$ and every $z < 2$, $\mathrm{BC}(\Sigma, n, z)$ $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$ in $W'$.
\end{lem}

\begin{proof}
By induction on $\lambda$. Assume the lemma is true for all $\lambda' \in C \cap \lambda$, and move to an outer model $W'$ of $W$ containing some $\Sigma$ that $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$. Define $F_{\omega_2} := F_{\omega_2^W}(\Sigma)$. If $X_{\delta^*, \lambda^*}(\Sigma) = \emptyset$ for all $\delta^* < \omega_1$ and all $\lambda^* \in C \cap \lambda$, then any increment or decrement to $F_{\omega_2}$ evaluated at any point in its domain, when reflected in $\Sigma$, would not affect its fulfillment of conditions \ref{c1} to \ref{c8}. In particular, the modified $\Sigma$ still vacuously fulfils conditions \ref{c3} to \ref{c8}.

Now assume $X_{\delta^*, \lambda^*}(\Sigma) \neq \emptyset$ for some $\delta^* < \omega_1$ and some $\lambda^* \in C \cap \lambda$. Fix some $n < \omega$ and $z < 2$, before labelling $\Sigma' := \mathrm{BC}(\Sigma, n, z)$. We can without loss of generality assume that $\Sigma' \neq \Sigma$ and that $z = 1$, because the case where $\Sigma' \neq \Sigma$ and $z = 0$ is symmetrical. It is easy to see that $\Sigma'$ fulfils conditions \ref{c1} to \ref{c7} since $\Sigma$ does, so we are only left to check that $\Sigma'$ fulfils condition \ref{c8}. Towards this end, let
\begin{enumerate}[label=(\arabic*)]
    \item\label{74111} $\delta < \omega_1^W$ and $\lambda' \in C \cap \lambda$ be such that $X_{\delta, \lambda'} := X_{\delta, \lambda'}(\Sigma') \neq \emptyset$, and
    \item\label{74112} $D$ be a dense subset of $\mathbb{P}_{\lambda'}$ definable over $(Q_{\lambda'}; \in, \mathbb{P}_{\lambda'}, A_{\lambda'})$ with parameters from $X_{\delta, \lambda'}$.
\end{enumerate}
We are going to show that
\begin{equation}\label{eq741}
    [\Sigma']^{< \omega} \cap X_{\delta, \lambda'} \cap D \neq \emptyset \text{.}
\end{equation}
If $\omega_2^W \geq \lambda'$, then $Q_{\lambda'} \cap \Sigma = Q_{\lambda'} \cap \Sigma'$, so (\ref{eq741}) is immediately implied by
\begin{equation*}
    [\Sigma]^{< \omega} \cap X_{\delta, \lambda'} \cap D \neq \emptyset \text{,}
\end{equation*}
which is in turn implied by the given fact that $\Sigma$ fulfils condition \ref{c8}. Now suppose $\omega_2^W < \lambda'$. Then $\omega_2^W$ is definable without parameters in $(Q_{\lambda'}; \in)$ and thus in $(X_{\delta, \lambda'}; \in)$. This means the set
\begin{equation*}
    D_n := \{p \in P_{\lambda'} : \exists \alpha < \omega_2^W \ (\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in p)\}
\end{equation*}
is a dense subset of $\mathbb{P}_{\lambda'}$ definable over $(Q_{\lambda'}; \in, \mathbb{P}_{\lambda'}, A_{\lambda'})$ with parameters from $X_{\delta, \lambda'}$. As $\Sigma$ fulfils condition \ref{c8}, there exists some $p_1 \in [\Sigma]^{< \omega} \cap X_{\delta, \lambda'} \cap D_n \text{,}$. By the fact that $\Sigma$ fulfils condition \ref{c1}, it must be the case that 
\begin{equation*}
    \ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma, n) \urcorner \in p_1 \text{,}
\end{equation*}
so also $\mathrm{Ret}(\Sigma, n) \in X_{\delta, \lambda'}$. As a consequence, we have that
\begin{equation}\label{eq742}
    \mathrm{BC}(A, n, z) \in X_{\delta, \lambda'} \text{ for each } A \in \mathcal{P}(\mathcal{L}) \cap X_{\delta, \lambda'} \text{.}
\end{equation}
We also note here the following easily verifiable rule: 
\begin{align}\label{eq742'}
\begin{split}
    \forall \Sigma_1 \ \forall \Sigma_2 \ (&(\text{``}\Sigma_2 \ \Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } \emptyset \text{''} \wedge \Sigma_1 \subset \Sigma_2) \\
    & \implies \mathrm{BC}(\Sigma_1, n, z) \subset \mathrm{BC}(\Sigma_2, n, z)) \text{.}
\end{split}
\end{align}

For convenience, denote $\mathrm{Ret}(\Sigma, n)$ by $\alpha$. Now define
\begin{align*}
    D' := \ & D \cap D_n \\
    D'' := \ & \{p \in D' : \{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner, \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1\urcorner\} \cap p = \emptyset\} \  \cup \\
    & \{\mathrm{BC}(p, n, z) : p \in D' \text{ and } \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in p\} \ \cup \\
    & \{\mathrm{BC}(p, n, z-1) : p \in D' \text{ and } \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1 \urcorner \in p\} \text{.}
\end{align*}
First, note that $D'' \subset D_n$. Next, due to the following facts:
\begin{itemize}
    \item that \ref{74112} holds,
    \item $D_n$ being definable over $(Q_{\lambda'}; \in, \mathbb{P}_{\lambda'}, A_{\lambda'})$ with parameters from $X_{\delta, \lambda'}$, and
    \item $\{\omega_2^W, \alpha\} \subset X_{\delta, \lambda'}$, 
\end{itemize}
we can conclude that both $D'$ and $D''$ are definable over $(Q_{\lambda'}; \in, \mathbb{P}_{\lambda'}, A_{\lambda'})$ with parameters from $X_{\delta, \lambda'}$. Moreover, since $D_n$ is dense open in $\mathbb{P}_{\lambda'}$, $D'$ is a dense subset of $\mathbb{P}_{\lambda'}$ as well.

\begin{claim}
$D''$ is a dense subset of $\mathbb{P}_{\lambda'}$.
\end{claim}

\begin{proof}
Choose an arbitrary $p \in P_{\lambda'}$. We wish to find $q \in D''$ with $q \leq_{\lambda'} p$. Proceed by cases as follows.
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    \item\label{7411c1} $p \in D_n$ and $\{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner, \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1\urcorner\} \cap p = \emptyset$. Find $r \in D'$ with $r \leq_{\lambda'} p$. Then we too have $\{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner, \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1\urcorner\} \cap r = \emptyset$, so $r \in D''$. We can thus set $q := r$.
    \item $p \in D_n$ and $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in p$. By the induction hypothesis specified at $\lambda'$, $\mathrm{BC}(p, n, z) \in P_{\lambda'}$. Find $r \in D'$ with $r \leq_{\lambda'} \mathrm{BC}(p, n, z)$. It must be the case that $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1 \urcorner \in r$, which yields $\mathrm{BC}(r, n, z-1) \in D''$. But $\mathrm{BC}(r, n, z-1) \leq_{\lambda'} p$, so we can set $q := \mathrm{BC}(r, n, z-1)$.
    \item $p \in D_n$ and $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1 \urcorner \in p$. By the induction hypothesis specified at $\lambda'$, $\mathrm{BC}(p, n, z-1) \in P_{\lambda'}$. Find $r \in D'$ with $r \leq_{\lambda'} \mathrm{BC}(p, n, z-1)$. It must be the case that $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in r$, which yields $\mathrm{BC}(r, n, z) \in D''$. But $\mathrm{BC}(r, n, z) \leq_{\lambda'} p$, so we can set $q := \mathrm{BC}(r, n, z)$.
    \item\label{7411c4} $p \not\in D_n$. Let $\Sigma^*$ $\Gamma_{\lambda'} (\mathcal{L} \cap Q_{\lambda'}, \mathfrak{A})$-certifies $p$ in some outer model $W^*$ of $W$. Through finitely many iterated applications of both
    \begin{itemize}
        \item $\mathrm{BC}(\cdot, n, z)$ on $\Sigma^*$, and
        \item the induction hypothesis specified at $\lambda'$,
    \end{itemize}
    we are able to obtain in $W^*$ some $\Sigma^{**} \subset \mathcal{L}$ for which $\mathrm{Ret}(\Sigma^{**}, n) \not\in \{\alpha, \alpha+1\}$ and $\Sigma^{**}$ $\Gamma_{\lambda'} (\mathcal{L} \cap Q_{\lambda'}, \mathfrak{A})$-certifies $p$. Setting 
    \begin{equation*}
        p' := \{\ulcorner \dot{F}_{\omega_2^W}(n) = \mathrm{Ret}(\Sigma^{**}, n) \urcorner\} \cup p \text{,}
    \end{equation*}
    we thus have $p' \in D_n$ and $\{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner, \ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1\urcorner\} \cap p' = \emptyset$, which brings us back to \hyperref[7411c1]{Case 1}. \qedhere
\end{enumerate}
\end{proof}

\begin{rem}\label{rem7412}
The particular manner in which \hyperref[7411c4]{Case 4} gets resolved in the proof of the preceding claim hinges on $\mathbb{P}_{\lambda'}$ having finite subsets of $\mathcal{L}$ as conditions. The same argument does not work in general; specifically, it fails when 
\begin{equation*}
    \{\ulcorner \neg \dot{F}_{\omega_2^W}(n) = \beta \urcorner : \alpha \leq \beta < \alpha + \omega\} \subset p \text{.}
\end{equation*}
\end{rem}

Now that $D''$ is a dense subset of $\mathbb{P}_{\lambda'}$ definable over $(Q_{\lambda'}; \in, \mathbb{P}_{\lambda'}, A_{\lambda'})$ with parameters from $X_{\delta, \lambda'}$, the fact that $\Sigma$ satisfies condition \ref{c8} means there exists
\begin{equation*}
    p_2 \in [\Sigma]^{< \omega} \cap X_{\delta, \lambda'} \cap D'' \text{.}
\end{equation*}
We can additionally deduce $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner \in p_2$ from $D''$ being a subset of $D_n$. Going by the definition of $D''$ alone, there must be some $p_3 \in D'$ such that 
\begin{enumerate}[label=(\arabic*)]
    \setcounter{enumi}{2}
    \item\label{74113} $\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha + 1 \urcorner \in p_3$, and
    \item\label{74114} $p_2 = \mathrm{BC}(p_3, n, z-1)$.
\end{enumerate}
Furthermore, \ref{74114} immediately gives us
\begin{enumerate}[label=(\arabic*)]
    \setcounter{enumi}{4}
    \item\label{74115} $\mathrm{BC}(p_2, n, z) \leq_{\lambda'} p_3$.
\end{enumerate}
That $p_2 \in [\Sigma]^{< \omega}$ implies $p_3 \in [\Sigma']^{< \omega}$ in the presence of (\ref{eq742'}) and \ref{74115}. On the other hand, that $p_2 \in X_{\delta, \lambda'}$ implies $\mathrm{BC}(p_2, n, z) \in X_{\delta, \lambda'}$ by (\ref{eq742}). Finally, the conjunction of 
\begin{itemize}
    \item $\mathrm{BC}(p_2, n, z) \in X_{\delta, \lambda'}$
    \item \ref{74115} being true, and
    \item $\mathrm{BC}(p_2, n, z) \setminus p_3$ being finite
\end{itemize}
allows the conclusion of $p_3 \in X_{\delta, \lambda'}$. We thus have (\ref{eq741}) from
\begin{equation*}
    p_3 \in [\Sigma']^{< \omega} \cap X_{\delta, \lambda'} \cap D' \subset [\Sigma']^{< \omega} \cap X_{\delta, \lambda'} \cap D. \qedhere
\end{equation*}
\end{proof}

Fix $\lambda \in C \cup \{\lambda^{\dagger}\}$. We shall use a parity argument to code in $W^{\mathbb{P}_{\lambda}}$ a real that is new to $W$. Let $g$ be any $\mathbb{P}_{\lambda}$-generic filter over $W$, so that $\bigcup g$ $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $\emptyset$. In $W[g]$, make the following definitions.
\begin{itemize}
    \item $F_{\omega_2} := F_{\omega_2^W}(\bigcup g)$. 
    \item $e$ is the unique function from $\omega$ into $2$ such that
    \begin{align*}
        e(n) := 
        \begin{cases}
            1 & \begin{aligned}[t]
            \begin{split}
               & \text{if there are } \alpha \text{ and } k \text{ satisfying} \\ & \omega \cdot \alpha + k = F_{\omega_2}(n) \text{ and } 1 \equiv k \ (\mathrm{mod} \ 2)
            \end{split}
            \end{aligned} \\
            0 & \text{otherwise}.
        \end{cases}
...
Let $\dot{e}$ be a $\mathbb{P}_{\lambda}$-name for $e$ in $W$. Clearly, $\Vdash_{\mathbb{P}_{\lambda}} \text{``} \dot{e} \in 2^{\omega} \text{''}$. It suffices to show that $\Vdash_{\mathbb{P}_{\lambda}} \text{``} \dot{e} \not\in \mathbf{V} \text{''}$. By way of contradiction, suppose otherwise, that $\dot{e}$ is not always a new real. Then for some $p \in P_{\lambda}$ and some $r \in (2^{\omega})^W$, 
\begin{equation}\label{eq744}
    p \Vdash_{\mathbb{P}_{\lambda}} \dot{e} = \check{r} \text{.}
\end{equation} 
Choose $n < \omega$ such that $p$ does not explicitly decide the value of $F_{\omega_2}(n)$, or in other words,
\begin{equation*}
    \{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha \urcorner, \ulcorner \neg \dot{F}_{\omega_2^W}(n) = \alpha \urcorner\} \cap p = \emptyset
\end{equation*}
for every $\alpha$. This is possible because $F_{\omega_2}$ would live in $W$ otherwise. Now in some outer model $W'$ of $W$, there is some $\Sigma$ that $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $p$. Setting $\alpha^* := \mathrm{Ret}(\Sigma, n)$, this means 
\begin{equation*}
    q_0 := p \cup \{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha^* \urcorner\} \in P_{\lambda} \text{.}
\end{equation*} 
On the other hand, $\mathrm{BC}(\Sigma, n, 1)$ $\Gamma_{\lambda} (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A})$-certifies $p$ as well by Lemma \ref{lem7411} and the definition of $\mathrm{BC}$, so also 
\begin{equation*}
    q_1 := p \cup \{\ulcorner \dot{F}_{\omega_2^W}(n) = \alpha^* + 1 \urcorner\} \in P_{\lambda} \text{.}
\end{equation*}
From how $\dot{e}(n)$ is defined, either
\begin{equation*}
    q_0 \Vdash_{\mathbb{P}_{\lambda}} \text{``}\dot{e}(\check{n}) \neq \check{r}(\check{n}) \text{''}
\end{equation*}
or 
\begin{equation*}
    q_1 \Vdash_{\mathbb{P}_{\lambda}} \text{``}\dot{e}(\check{n}) \neq \check{r}(\check{n}) \text{''}
\end{equation*}
must hold. But since $q_0, q_1 \leq_{\lambda} p$, (\ref{eq744}) tells us that
\begin{equation*}
    q_i \Vdash_{\mathbb{P}_{\lambda}} \text{``}\dot{e}(\check{n}) = \check{r}(\check{n})
\end{equation*}
for all $i < 2$, a clear contradiction.
\end{proof}

The proofs of both Theorem \ref{thm747} and Lemma \ref{lem7411} go through as they are, even with the definitions of all associated parameters swopped out for those in the proof of Theorem \ref{notion2}. Similarly, the proofs go through when the definitions of the said parameters are modified from the proof of Theorem \ref{notion2} according to the paragraph immediately before Theorem \ref{notion3}. As a result, mildly stronger versions of Theorems \ref{notion1}, \ref{notion2} and \ref{notion3} can be stated. In particular, we have the following.
...
Then there is a stationary-preserving forcing notion $\mathbb{P}$ of size $2^{< \lambda^{\dagger}}$ such that 
\begin{enumerate}[label=(\alph*)]
    \item $\mathbb{P}$ witnesses $Nb'_1(\lambda^{\dagger})$ (thus also $Nb_1(\lambda^{\dagger})$),
    \item in $V^{\mathbb{P}}$, there is a $\Diamond$-iteration $$\langle \bar{N}_i = (N_i; \in, I_i, a_i), \sigma_{ij} : i \leq j \leq \omega_1^V \rangle$$ satisfying 
    \begin{enumerate}[label=(\arabic*), leftmargin=40pt]
        \item $\bar{N}_0 \in D^* := (F^*) " ((F^*)^{-1}(D)^*) \subset \mathbb{P}_{max}$,
        \item $I_{\omega_1^V} = \mathrm{NS}_{\omega_1}^{V^{\mathbb{P}}} \cap N_{\omega_1^V}$, and
    ...
    \end{enumerate}
    \item $\mathbb{P}$ adds reals.
\end{enumerate}
\end{thm}

\subsection{Towards Greater Supports}

Work in the universe $W$ as defined in the proof of Theorem \ref{notion1} and let $\lambda^{\dagger}$, $\kappa$, $\mathfrak{A}$, $\mathcal{L}$, $C$ and $\{Q_{\lambda} : \lambda \in C\}$ be as in the same proof. Fix any infinite cardinal $\theta > \omega$. Should we then alter the definition of $\Gamma_{\lambda}$ and $P_{\lambda}$ for each $\lambda \in C \cup \{\lambda^{\dagger}\}$ as follows:
\begin{align*}
    \Gamma_{\lambda}^C(\theta) := \ & \text{the set } \Gamma_{\lambda} \text{ defined according to Definition \ref{defc},} \\
    & \text{but with } \mathbb{P}^C_{\lambda'}(\theta) \text{ in place of the parameter } \mathbb{P}_{\lambda'} \text{ for each } \lambda' \in \lambda \cap C \\
    P_{\lambda}^C(\theta) := \ & \{p \in [\mathcal{L}]^{< \theta} \cap Q_{\lambda} : \\
    & \ \Vdash_{Col(\omega, |H(\kappa)|)} \exists \Sigma \ (``\Sigma \ \Gamma^C_{\lambda}(\theta) (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } p")\} \\
    \mathbb{P}^C_{\lambda}(\theta) := \ & (P^C_{\lambda}(\theta), \supset) \text{,}
\end{align*}
would all subsequent lemmas in the proof still go through?

In an attempt to answer this question, let $\theta$ be a cardinal greater than $\omega$. Then the most obvious gap in the proof shows up in the semantic interpretation of members of $\Gamma^C_{\lambda}(\theta)$. More specifically, since there are many dense subsets of many of the $\mathbb{P}_{\lambda'}$'s which are definable without parameters and contain only infinite sets, \ref{c8} no longer means
\begin{equation}
    \label{eq47}
...
\end{enumerate}
as we ought to recall. In fact, if we cannot translate (\ref{eq47}) into a set of $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentences, there is then an inability to guarantee that the $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$-generic filter over $V$ is sufficiently generic over each non-empty $X_{\delta, \lambda'}$. This throws a wrench into the side condition method so crucial to the main results of this chapter. 

Thankfully, there is a simple way out when $\theta = \omega_1$, via a straightforward ``quasi-second-order'' augmentation of our language. Let $\mathcal{L}$ be as defined in the proof of Theorem \ref{notion1}, and retain all information in the said proof before the said definition. Pick a new distinguished symbol $\dot{G}$, rename $\mathcal{L}$ as $\mathcal{L}^o$, and re-declare
\begin{equation*}
    \mathcal{L} := \mathcal{L}^o \cup \{\ulcorner A \subset \dot{G} \urcorner : A \in [\mathcal{L}^o]^{< \omega_1}\} \cup \{\ulcorner \neg A \subset \dot{G} \urcorner : A \in [\mathcal{L}^o]^{< \omega_1}\} \text{.}
\end{equation*} 
As in the proof of Theorem \ref{notion1}, we want to define $\mathbb{P}_{\lambda}$ assuming $\mathbb{P}_{\lambda'} = (P_{\lambda'}, \leq_{\lambda'})$ has been defined for all $\lambda' \in \lambda \cap C$. First, given $\lambda' \in \lambda \cap C$, we call a condition $p$ of $\mathbb{P}_{\lambda'}$ (i.e. $p \in P_{\lambda'}$) \textit{good} iff whenever $$x = \ulcorner \neg A \subset \dot{G} \urcorner \in p$$ for some $A \in [\mathcal{L}^o]^{< \omega_1}$, we can find $x \in A$ for which $\neg x \in p$. Next, let 
\begin{equation*}
    P'_{\lambda'} := \{p \in P_{\lambda'} : p \text{ is good}\} \text{,}
\end{equation*}
and define $\mathrm{SP}_{\lambda'}$ to be the function with domain $P_{\lambda'}$ that takes $p \in P_{\lambda'}$ to the set
\begin{equation*}
    \bigcup \{A \in [\mathcal{L}^o]^{< \omega_1} : \ulcorner A \subset \dot{G} \urcorner \in p\} \cup (p \cap \mathcal{L}^o) \text{.}
\end{equation*}
Finally, to $\Gamma_{\lambda}$ in Definition \ref{defc}, we add the $\mathcal{L}^*_{\mathfrak{A}}$-$\Pi_2$ sentence 
\begin{enumerate}[label=(S\arabic*)$_{\lambda}$, leftmargin=40pt]
    \setcounter{enumi}{8}
    \item\label{s9n} $\ulcorner \forall A \in [\mathcal{L}^o]^{< \omega_1} \cap Q_{\lambda} \ (E(\ulcorner A \subset \dot{G} \urcorner) \iff \forall x \in A \ (E(x))) \urcorner$,
\end{enumerate}
and re-declare
\begin{enumerate}[label=(S\arabic*)$_{\lambda} \ :$, leftmargin=50pt]
    \setcounter{enumi}{7}
    \item\label{s8n}
    \!
    $\begin{aligned}[t]
        \ulcorner & \forall \delta \ \forall \lambda' \ \forall \phi \ \forall \bar{s} \ \forall x \ \exists p \in P_{\lambda'} \ \exists q \in P'_{\lambda'} \ \exists A \\
        & ((E(\ulcorner x \in \dot{X}_{\delta, \lambda'} \urcorner) \ \wedge \\
        & (\phi, \bar{s}) \in Df^{\lambda}(\lambda') \ \wedge \\
        & \forall n \in dom(\bar{s}) \ (E(\ulcorner \bar{s}(n) \in \dot{X}_{\delta, \lambda'} \urcorner))) \\
        & \implies ((\phi, \bar{s}, p) \in Ef_0^{\lambda}(\lambda') \wedge p \subset q \ \wedge \\
        & E(\ulcorner q \in \dot{X}_{\delta, \lambda'} \urcorner) \wedge \mathrm{SP}_{\lambda'}(q) = A \ \wedge \\
        & E(\ulcorner A \subset \dot{G} \urcorner))) \urcorner.
    \end{aligned}$
\end{enumerate}
With these modifications, the rest of the proof of Theorem \ref{notion1} goes through without much difficulty, once we note that
\begin{enumerate}[label=(CS\arabic*), leftmargin=40pt]
    \item\label{cs1} $P'_{\lambda}$ is dense in $\mathbb{P}_{\lambda}$ for all $\lambda \in C$, and
    \item\label{cs2} letting $j$ be the generic ultrapower map as in the proof of Lemma \ref{sideuni}, $j(A) = j" A$ must hold whenever $$A \in V \text{ and } V \models \text{``} |A| < \omega_1 \text{''.}$$
\end{enumerate}

We have described above what is spiritually a countable-support version of the original sequence $\{\mathbb{P}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ constructed in the proof of Theorem \ref{notion1}. To distinguish this new version from the original, call the former sequence $\{\mathbb{P}^{< \omega_1}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$.

\begin{prop}\label{prop741}
$\mathbb{P}^C_{\lambda}(\omega_1)$ and $\mathbb{P}^{< \omega_1}_{\lambda}$ are forcing equivalent for all $\lambda \in C \cup \{\lambda^{\dagger}\}$.
\end{prop}

\begin{proof}
By induction on $\lambda \in C \cup \{\lambda^{\dagger}\}$. Given \ref{s9n}, \ref{cs1} and the induction hypothesis, we can deduce that (\ref{eq47}) and the redeclared $\hyperref[s8n]{\text{(S8)}_{\lambda}}$ are equivalent through a routine unfolding of definitions. We shall show that the map $A \mapsto \{\ulcorner A \subset \dot{G} \urcorner\}$ is a dense weak embedding from $\mathbb{P}^C_{\lambda}(\omega_1)$ into $\mathbb{P}^{< \omega_1}_{\lambda}$. That it is a weak embedding is easy to see, so we need only show that its image is dense in $w(\mathbb{P}^{< \omega_1}_{\lambda})$. Towards that end, let $p \in P^{< \omega_1}_{\lambda}$. By \ref{cs1}, choose $q \leq_{\lambda} p$ such that $q$ is a good condition of $\mathbb{P}^{< \omega_1}_{\lambda}$. Now, setting 
\begin{equation*}
    A_q := \bigcup \{A \in [\mathcal{L}^o]^{< \omega_1} : \ulcorner A \subset \dot{G} \urcorner \in q\} \cup (q \cap \mathcal{L}^o) \text{,}
\end{equation*}
we have by \ref{s9n} that $\{\ulcorner A_q \subset \dot{G} \urcorner\}$ extends $q$, and thus $p$, in $w(\mathbb{P}^{< \omega_1}_{\lambda})$.
\end{proof}

By the modularisability of our forcing framework, one can easily verify that greater support analogues of the forcing notions witnessing the main theorems of Section \ref{ss43} exist. In particular, the following souped up version of Theorem \ref{notion3} can be proven with just a little additional effort.
...
Then there are 
\begin{enumerate}[label=(\arabic*)]
    \item $Col(\lambda^{\dagger}, \lambda^{\dagger})$-names $\dot{\mathfrak{A}}$, $\dot{\mathcal{L}}$, and $\dot{C}$, 
    \item a sequence of $Col(\lambda^{\dagger}, \lambda^{\dagger})$-names $\{\dot{Q}_{\lambda} : \lambda \in C\}$, and
    \item a formula $\varphi_{\Gamma}$ with a single free variable in the forcing language of $Col(\lambda^{\dagger}, \lambda^{\dagger})$ fulfilling
    \begin{equation*}
        \Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} \forall x \ (\text{``} \varphi_{\Gamma}(x) \subset \dot{\mathcal{L}}^*_{\dot{\mathfrak{A}}} \text{''})
    \end{equation*}
\end{enumerate}
such that for each infinite ordinal $\theta < \omega_2$, there are
\begin{enumerate}[label=(\arabic*)]
    \setcounter{enumi}{3}
    \item a sequence of $Col(\lambda^{\dagger}, \lambda^{\dagger})$-names $\{\dot{\Gamma}_{\lambda}(\theta) : \lambda \in \dot{C} \cup \{\lambda^{\dagger}\}\}$,
    \item a sequence of $Col(\lambda^{\dagger}, \lambda^{\dagger})$-names $\{\dot{P}_{\lambda}(\theta) : \lambda \in \dot{C} \cup \{\lambda^{\dagger}\}\}$,
    \item a sequence of $Col(\lambda^{\dagger}, \lambda^{\dagger})$-names $\{\dot{\mathbb{P}}_{\lambda}(\theta) : \lambda \in \dot{C} \cup \{\lambda^{\dagger}\}\}$, and
    \item a stationary-preserving forcing notion $\mathbb{P}(\theta)$ of size $2^{< \lambda^{\dagger}}$
\end{enumerate}
for which
\begin{enumerate}[label=(\alph*)]
    \item $\Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} \text{``} \dot{C} \text{ is a club subset of } \lambda^{\dagger} \text{''}$,
    \item $\Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} \text{``} \forall \lambda \in \dot{C} \cup \{\lambda^{\dagger}\} \ (\dot{\Gamma}_{\lambda}(\theta) = \varphi_{\Gamma}(\{\dot{\mathbb{P}}_{\lambda'}(\theta) : \lambda' \in C \cap \lambda\})) \text{''}$,
    \item \!
    $\begin{aligned}[t]
        \Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} \text{``} \forall \lambda \in \dot{C} \ \cup & \ \{\lambda^{\dagger}\} \\
        (\dot{P}_{\lambda}(\theta) = \ & \{p \in [\dot{\mathcal{L}}]^{< \theta} \cap \dot{Q}_{\lambda} : \\
        & \Vdash_{Col(\omega, |trcl(\dot{\mathfrak{A}})|)} \exists \Sigma \ (\text{``} \Sigma \ \dot{\Gamma}_{\lambda}(\theta) (\dot{\mathcal{L}} \cap \dot{Q}_{\lambda}, \dot{\mathfrak{A}}) \text{-certifies } p \text{''})\}) \text{'',}
    \end{aligned}$
    \item $\Vdash_{Col(\lambda^{\dagger}, \lambda^{\dagger})} \text{``} \forall \lambda \in \dot{C} \cup \{\lambda^{\dagger}\} \ (\dot{\mathbb{P}}_{\lambda}(\theta) = (\dot{P}_{\lambda}(\theta), \supset)) \text{''}$,
    \item $\mathbb{P}(\theta) = Col(\lambda^{\dagger}, \lambda^{\dagger}) * \dot{\mathbb{P}}_{\lambda^{\dagger}}(\theta)$,
    \item $\mathbb{P}(\theta)$ witnesses $Nb'_1(\lambda^{\dagger})$ (thus also $Nb_1(\lambda^{\dagger})$), and
    \item in $V^{\mathbb{P}(\theta)}$, there is a $\Diamond$-iteration $$\langle \bar{N}_i = (N_i; \in, I_i, a_i), \sigma_{ij} : i \leq j \leq \omega_1^V \rangle$$ satisfying 
    \begin{enumerate}[label=(\arabic*), leftmargin=40pt]
        \item $\bar{N}_0 \in D^* := (F^*) " ((F^*)^{-1}(D)^*) \subset \mathbb{P}_{max}$,
        \item $I_{\omega_1^V} = \mathrm{NS}_{\omega_1}^{V^{\mathbb{P}}} \cap N_{\omega_1^V}$, and
        \item $a_{\omega_1^V} = A$.
...
Now that we have shown how to augment the definition of $\{\mathbb{P}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ in the proof of Theorem \ref{notion1} to obtain a sequence of forcing notions pointwise forcing equivalent to $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$, what prevents us from adapting this augmentation process to ``supports'' of greater cardinalities? For one, \ref{cs2} no longer holds if we replace the sole instance of $\omega_1$ therein with a larger cardinal. It stands to reason that a more drastic overhaul of the specifications is needed for further extensions in this direction.

\subsection{Towards a Measurable Analogue}

Also natural is the question of whether we can swop out the precipitous ideal on $\omega_1$ for a measurable cardinal and still derive a close analogue of Theorem \ref{notion1}. If we were to allow the collapse of all cardinals strictly between $\omega$ and a measurable cardinal $\kappa$, it would be much more economical to reduce the hypotheses to that of Theorem \ref{notion1} by by first collapsing $\kappa$ to $\omega_1$ and obtaining that $\mathrm{NS}_{\omega_1}$ is precipitous. What is more interesting is an analogue that preserves both $\omega_1$ and $\kappa$. Before we highlight some of the difficulties in proving such an analogue, let us first recapitulate a core ingredient of the proof of Theorem \ref{notion1}.

\begin{defi}
\leavevmode
\begin{enumerate}[label=(\arabic*)]
    \item For any inner or outer model $W$ of $V$, we say $\pi$ is an \textit{$\omega_1$-elementary embedding of $W$} iff in some outer model of $W$,
    \begin{enumerate}[label=(\alph*)]
        \item $\pi$ is a definable function,
        \item $\pi$ is non-trivial elementary embedding of $W$ into some definable inner model $M$ (so that $W$ is also a definable inner model), and
        \item $\pi$ has critical point $\omega_1^W$.
    \end{enumerate}
    \item For any inner or outer model $W$ of $V$ and any set $S \in \mathcal{P}(\omega_1^W) \cap W$, we say $\pi$ is an \textit{$(\omega_1, S)$-elementary embedding of $W$} iff in some outer model of $W$,
    \begin{enumerate}[label=(\alph*)]
        \item $\pi$ is an $\omega_1$-elementary embedding of $W$, and
        \item $\omega_1^W \in \pi(S)$.
    \end{enumerate}
\end{enumerate}
\end{defi}

Preserving both $\omega_1$ and $\kappa$ would require substantial innovation, because
\begin{enumerate}[label=(\alph*)]
    \item the Asper\'{o}-Schindler side condition method is the only known way of preserving $\omega_1$ that plays well with the broader machinery of forcing with language fragments, and
    \item employing the Asper\'{o}-Schindler side condition method to preserve $\omega_1$ requires the existence of an $(\omega_1, S)$-elementary embedding of $V$ for each $S \in \mathcal{P}(\omega_1^V) \cap V$ fulfilling
    \begin{equation*}
        V \vDash \text{``} S \text{ is a stationary subset of } \omega_1 \text{''.}
    \end{equation*}
\end{enumerate}
yet
\begin{enumerate}[label=(\alph*)]
    \setcounter{enumi}{2}
    \item the existence of an $(\omega_1, S)$-elementary embedding of $V$ for even a single $S \in \mathcal{P}(\omega_1^V) \cap V$ is not known to follow from 
    \begin{equation*}
        V \vDash \text{``there is a measurable cardinal''.}
    \end{equation*}
\end{enumerate}
Subsection \ref{2cof} provides an indication of how finicky and difficult tweaks and augmentations to the Asper\'{o}-Schindler side condition method can be, when the objective is to force an ordinal strictly below the cardinality of the resultant forcing notion to take on some uncountable cofinality. The difficulties highlighted in the aforementioned subsection stem primarily from the fact that certain important higher-order truths are not preserved by any $(\omega_1, S)$-elementary embedding of $V$ for any $S \in \mathcal{P}(\omega_1^V) \cap V$. Even if one is so optimistic to believe the said difficulties can be overcome, that no $(\omega_1, S)$-elementary embedding of $V$ is guaranteed to exist surely is a bridge too far, for such embeddings form the heart and soul of the Asper\'{o}-Schindler method, and losing a handle on them would call for a complete reinvention of the side condition technology should side conditions be needed to preserve $\omega_1$. 

If not through side conditions, how else can we preserve $\omega_1$? The only category of known alternatives appears to be the class of methods leveraging on specific closure properties. This seems viable for our purposes at first glance, considering that the sequences of forcing notions $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ can naturally be endowed with ``countable supports''. In this subsection, let us briefly experiment with such sequences. 

\begin{defi}
Let $GNb_1(\kappa, \lambda)$ denote the statement
\begin{align*}
    \text{`} & \text{there is a forcing notion } \mathbb{P} = (P, \leq_{\mathbb{P}}) \text{ such that:} \\
    & P \subset H(\lambda), \\
    & \mathbb{P} \text{ adds no } \omega \text{-sequences of ordinals below } \omega_1 \text{,} \\
    & \mathbb{P} \text{ preserves stationary subsets of } \kappa \text{,} \\
    & \Vdash_{\mathbb{P}} ``cof(\alpha) = \omega_1" \text{ for all regular cardinals } \alpha \text{ satisfying } \omega_2 \leq \alpha < \lambda \text{ and } \alpha \neq \kappa \text{,} \\
    & \Vdash_{\mathbb{P}} ``cof(\lambda) = \kappa" \text{'.}
\end{align*}
\end{defi}

\begin{defi}
Let $GNb'_1(\kappa, \lambda)$ denote the statement
\begin{align*}
    \text{`} & \text{there is a forcing notion } \mathbb{P} \text{ such that:} \\
    & \mathbb{P} \text{ adds no } \omega \text{-sequences of ordinals below } \omega_1 \text{,} \\
    & \mathbb{P} \text{ preserves stationary subsets of } \kappa \text{,} \\
    & \Vdash_{\mathbb{P}} ``cof(\alpha) = \omega_1" \text{ for all regular cardinals } \alpha \text{ satisfying } \omega_2 \leq \alpha < \lambda \text{ and } \alpha \neq \kappa \text{,} \\
    & \Vdash_{\mathbb{P}} ``cof(\gamma) = \kappa" \text{ for all regular cardinals } \gamma \text{ satisfying } \lambda \leq \gamma \leq 2^{< \lambda} \text{, and} \\
    & \Vdash_{\mathbb{P}} ``cof(\beta) = \beta" \text{ for all regular cardinals } \beta \text{ satisfying } 2^{< \lambda} < \beta \text{'.}
\end{align*}
\end{defi}

\begin{ass}\label{thm744}
Let $\kappa$, $\lambda^{\dagger}$ be regular cardinals such that 
\begin{enumerate}[label=(\alph*)]
    \item $\omega_1 < \kappa < \lambda^{\dagger}$,
    \item $\Diamond_{\lambda^{\dagger}}$ holds, and
    \item for every stationary subset $S$ of $\kappa$, there is an outer model $W$ of $V$ wherein a definable elementary embedding $j$ exists of $V$ into a definable inner model $M$ of $W$ with the property
    \begin{equation*}
        crit(j) = \kappa \in j(S) \text{.}
    \end{equation*}
\end{enumerate}
Then there is a forcing notion of size $\lambda$ witnessing $GNb_1(\kappa, \lambda^{\dagger})$.
\end{ass}

If we take Assumption \ref{thm744} to be true, a strong analogue of Theorem \ref{notion1} can be proven.

\begin{thm}
Let $\kappa$, $\lambda^{\dagger}$ be regular cardinals such that $\omega_1 < \kappa < 2^{\kappa} < \lambda^{\dagger}$ and $\kappa$ is measurable. Given that Assumption \ref{thm744} holds, there is a forcing notion of size $2^{< \lambda}$ witnessing $GNb_1'(\kappa, \lambda^{\dagger})$.
\end{thm}

\begin{proof}
It is easy to see that in $V^{Col(\lambda^{\dagger}, \lambda^{\dagger})}$, 
\begin{enumerate}[label=(\arabic*)]
    \item $\lambda^{\dagger}$ remains regular,
    \item $\Diamond_{\lambda^{\dagger}}$ holds,
    \item $\kappa$ remains measurable, 
    \item $|H(\lambda^{\dagger})| = \lambda^{\dagger}$, and
    \item $H(\lambda^{\dagger}) = H(\lambda^{\dagger})^V$.
\end{enumerate}
As a result, the hypotheses of Assumption \ref{thm744} are fulfilled in all $Col(\lambda^{\dagger}, \lambda^{\dagger})$-generic extensions $V[h]$ of $V$. Furthermore, for every such $V[h]$, Lemma \ref{lem26} holds after we replace
\begin{itemize}
    \item the only instance of the clause ``stationary-preserving forcing notion $\mathbb{P} = (P, \leq_P)$ fulfilling'' with ``forcing notion $\mathbb{P} = (P, \leq_P)$ preserving stationary subsets of $\kappa$ and fulfilling'',
    \item the only instance of $\omega$ with $\omega_1$,
    \item the only instance of $\omega_1$ with $\kappa$, and
    \item the only instance of the clause ``$\omega_2 \leq \alpha < \lambda^{\dagger}$'' with ``$\omega_2 \leq \alpha < \lambda^{\dagger} \text{ and } \alpha \neq \kappa$''.
\end{itemize}
We are therefore justified in concluding the theorem.
\end{proof}

It may seem mighty tempting at first glance to prove Assumption \ref{thm744} using an adaptation of the proof of Theorem \ref{notion1} in tandem with certain closure properties of forcing notions in some sequence of the form $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$. We shall present an intuitive attempt at an argument in the indented passage below.

\begin{quote}
We leave it to the reader to check that, under the hypotheses of Assumption \ref{thm744}, the entire argument after Lemma \ref{lem26} in the proof of Theorem \ref{notion1} goes through without a hitch should we employ the substitutions of
\begin{enumerate}[label=(\arabic*)]
    \item all instances of $\omega$ originally required to ensure 
    \begin{equation*}
        \Vdash_{\mathbb{P}} \text{``} cof(\alpha) = \omega \text{''} \text{ for all regular cardinals } \alpha \text{ satisfying } \omega_2 \leq \alpha < \lambda^{\dagger}
    \end{equation*}
    with $\omega_1$,
    \item all instances of $\omega_1$ originally required to ensure
    \begin{equation*}
        \Vdash_{\mathbb{P}} \text{``} cof(\lambda^{\dagger}) = \omega_1 \text{''}
    \end{equation*}
    with $\kappa$, 
    \item all instances of $R$ with 
    \begin{equation*}
        R' := \{i < \lambda^{\dagger} : \omega_2 \leq i \neq \kappa \text{ and } i \text{ is regular} \} \text{,}
    \end{equation*}
\end{enumerate}
while
\begin{enumerate}[label=(\arabic*)]
    \setcounter{enumi}{3}
    \item modifying \ref{c2} so that it also says
    \begin{quote}
        ``each $F_i$ is a strictly increasing function'',
    \end{quote}
    \item\label{7444} modifying \ref{c7} so that it instead says
    \begin{quote}
        ``if $X_{\delta, \lambda'}$ is non-empty, then $(X_{\delta, \lambda'}; \in, c \cap X_{\delta, \lambda'}, \mathbb{P}_{\lambda'}, A_{\lambda'})$ is an elementary submodel of $(Q_{\lambda'}; \in, c \cap Q_{\lambda'}, \mathbb{P}_{\lambda'}, A_{\lambda'})$'',
    \end{quote}
    and
    \item\label{7445} using the aforementioned sequence of ``countable-support'' forcing notions $\{\mathbb{P}^C_{\lambda}(\omega_1) : \lambda \in C \cup \{\lambda^{\dagger}\}\}$ in place of the sequence $\{\mathbb{P}_{\lambda} : \lambda \in C \cup \{\lambda^{\dagger}\}\}$.
\end{enumerate}

What is left for us to prove is that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ adds no $\omega$-sequences of ordinals. We want to show that $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ exhibits some desirable closure property. Towards this end, let 
\begin{itemize}
    \item $p \in \mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$, 
    \item $\dot{f}$ be a $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$-name of function from $\omega$ into $\omega_1$, 
    \item and $\{A_n : n < \omega\}$ be a partition of $\omega$ into infinite subsets, such that
    \begin{equation*}
        n + 1 \subset \bigcup \{A_m : m \leq n\}
    \end{equation*}
    for every $n < \omega$.
\end{itemize}
Due to \ref{7444}, we may, by identifying $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ with a certain dense suborder of itself, assume that for any $\delta < \kappa$ and any $\lambda \in C \cup \{\lambda^{\dagger}\}$, $X_{\delta, \lambda}(q)$ is the Skolem hull of itself with respect to $c$ whenever $q$ is a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ and $X_{\delta, \lambda}(q) \neq \emptyset$. Now define a sequence $\{(p_n, B_n) : n < \omega\}$ such that
\begin{enumerate}[label=(\roman*)]
    \item\label{744i} $p_n$ is a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ for all $n < \omega$,
    \item $p_n \subset p_{n+1}$ for all $n < \omega$,
    \item\label{744iii} $B_n$ is a surjection from $\bigcup \{A_m : m \leq n\}$ onto 
    \begin{align*}
        E_n := \{(\delta, \lambda, D) \in \mathcal{P}(H(\lambda^{\dagger})): \ & \delta < \kappa \text{, } \lambda \in C \cup \{\lambda^{\dagger}\} \text{, } X_{\delta, \lambda}(p_n) \neq \emptyset \text{ and} \\ 
        & D \text{ is a dense subset of } \mathbb{P}^C_{\lambda}(\omega_1) \\ 
        & \text{definable over } (Q_{\lambda}; \in, \mathbb{P}_{\lambda}, A_{\lambda}) \\
        & \text{with parameters from } X_{\delta, \lambda}(p_n)\} \text{, and}
    \end{align*}
    \item\label{744iv} $B_n \subset B_{n+1}$ for all $n < \omega$, 
\end{enumerate}
as follows. Set $p_0 := p$ and choose $B_0$ to be any surjection from $A_n$ onto $E_0$. Assuming $(p_n, B_n)$ has been defined, we first extend $p_n$ to some condition $q_n$ of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ deciding the ordinal value of $\dot{f}(n)$, before extending $q_n$ to another condition $r_n$ of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ for which 
\begin{equation*}
    \exists \delta \ \exists \lambda \ \exists D \ (B_n(n) = (\delta, \lambda, D) \text{ and } [r_n]^{< \omega_1} \cap X_{\delta, \lambda}(r_n) \cap D \neq \emptyset) \text{.}
\end{equation*}
Such an $r_n$ can be found because of \ref{7445}. Assign $p_{n+1}$ to be $r_n$ and $B_{n+1}$ to be any set extending $B_n$ that satisfies \ref{744iii}. This construction has as a direct consequence the statement
\begin{align}\label{eq743}
\begin{split}
    \forall n < \omega \ \exists \beta \ \exists \delta \ \exists \lambda \ \exists D \ (& B_n(n) = (\delta, \lambda, D) \text{,} \\
    & p_{n+1} \Vdash_{\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)} \dot{f}(n) = \beta \text{ and} \\
    & [p_{n+1}]^{< \omega_1} \cap X_{\delta, \lambda}(p_{n+1}) \cap D \neq \emptyset) \text{.}
\end{split}
\end{align}
In conjunction with the (predefined target) properties \ref{744i} to \ref{744iv}, (\ref{eq743}) tells us that if
\begin{equation*}
    p^* := \bigcup \{p_n : n < \omega\}
\end{equation*}
is a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$, then it extends $p$ and forces $\dot{f}$ to take on a value in $V$. Since $p$ is an arbitrary condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$, the usual density argument would imply $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ adds no $\omega$-sequences of ordinals below $\omega_1$.
\end{quote}

At this point, there should be an inkling that $p^*$ being a condition of $\mathbb{P}^C_{\lambda^{\dagger}}(\omega_1)$ is difficult to show. In fact, setting $\lambda := \lambda^{\dagger}$ for the remainder of this subsection, we seem unable to guarantee its consistency with the fulfilment of just one of the 8 modified specifications among \ref{c1} to \ref{c8}. The reader should be able to convince themselves without too much hassle that $p^*$ is extendable to some $\Sigma$ satisfying 
\begin{itemize}
    \item either the modified versions of all these specifications less \ref{c5},
    \item or the modified versions of all these specifications less \ref{c8}.
\end{itemize}
However, even if we weaken the modified \ref{c5} to instead say
\begin{quote}
    ``$\mathrm{sup}(X_{\delta, \lambda'} \cap \delta) = \delta$ whenever $X_{\delta, \lambda'}$ is non-empty'',
\end{quote}
it appears hard to argue that a set $\Sigma$ both extending $p^*$ and satisfying \ref{c1} to \ref{c8} exists in some outer model of $V$. 

What we have uncovered in this subsection is therefore a tension between two of the original specifications in the proof of Theorem \ref{notion1}, a tension that appears robust enough to guard against the vast majority of closure properties from which one can conclude the preservation of $\omega_1$. Since the known techniques for preserving $\omega_1$ (beyond the most basic applications) rely either on side conditions or on closure properties, and the use of side conditions under our circumstances has been ruled out at the beginning of this subsection for being untenable given the current technology, all evidence points to the following:
\begin{quote}
    if a witness to Assumption \ref{thm744} can be uniformly constructed from $\kappa$ and $\lambda^{\dagger}$, then radically new ideas would have to be employed in the construction.
\end{quote}

\section{Open Questions}

Recall the definitions made in Section \ref{sect74}. In view of the discussion which just transpired there, our first two questions, regarding the nature of obstacles to our cause, are as pertinent as they are obvious. 

\begin{ques}
Does Assumption \ref{thm744} hold?
\end{ques}

\begin{ques}\label{oq1}
Let $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion1}. In $W$, can $\theta$, $C$ and
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}]
\end{gather*}
be chosen such that 
\begin{enumerate}[label=(\arabic*)]
    \item $\theta > \omega_1$ is a cardinal,
    \item \ref{4100} to \ref{4102} of Lemma \ref{lem26} are fulfilled with $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$ does not collapse $\omega_1^W$?
\end{enumerate}
\end{ques}

In the spirit of Jensen's results on the Namba extension problem and in light of Remark \ref{rem7412}, we are interested in the possibility --- under the right assumptions --- of suitably modifying the forcing notion $\mathbb{P} = \mathbb{P}_{\lambda^{\dagger}}$ constructed in the proof of Theorem \ref{notion1} so that it does not add reals. More generally, we can ask the same about the parametrised versions of said forcing notion. 

\begin{ques}\label{addsreals}
Let $V$, $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion1}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in $W$, can $\theta$, $C$ and
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}]
\end{gather*}
be chosen such that
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \ref{4100} to \ref{4102} of Lemma \ref{lem26} are fulfilled with $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}^C_{\lambda^{\dagger}}(\theta)$ does not add reals?
\end{enumerate}
\end{ques}

\begin{ques}\label{addsrealsadd}
Let $V$, $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion1}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in $W$, is there a forcing notion $\mathbb{P}_{\mathrm{new}}$ additively defined from the proof of Theorem \ref{notion1} such that
\begin{enumerate}[label=(\arabic*)]
    \item \ref{4100} to \ref{4102} of Lemma \ref{lem26} are fulfilled with $\mathbb{P}_{\mathrm{new}}$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}_{\mathrm{new}}$ does not add reals?
\end{enumerate}
\end{ques}

Comparing Theorem \ref{notion1} with Theorems \ref{notion2} and \ref{notion3} makes clear the existence of close relatives of Questions \ref{oq1} to \ref{addsrealsadd}, given the following definitions in the universe $W$, where $W$, $\lambda^{\dagger}$, $\kappa$, $C$ and $\{Q_{\lambda} : \lambda \in C\}$ are as defined in the proof of Theorem \ref{notion2}:
\begin{align*}
    \Gamma^{'C}_{\lambda}(\theta) := \ & \text{the set } \Gamma'_{\lambda} \text{ defined according to Definition \ref{def447},} \\
    & \text{but with } \mathbb{P}^{'C}_{\lambda'}(\theta) \text{ in place of the parameter } \mathbb{P}_{\lambda'} \text{ for each } \lambda' \in \lambda \cap C \\
    \Gamma^{''C}_{\lambda}(\theta) := \ & \text{the set } \Gamma'_{\lambda} \text{ modified according to what comes immediately before}
    \\
    & \text{the statement of Theorem \ref{notion3}, but with } \mathbb{P}^{''C}_{\lambda'}(\theta) \text{ in place of } \\
    & \text{the parameter } \mathbb{P}_{\lambda'} \text{ for each } \lambda' \in \lambda \cap C \\
    P^{'C}_{\lambda}(\theta) := \ & \{p \in [\mathcal{L}]^{< \theta} \cap Q_{\lambda} : \\
    & \ \Vdash_{Col(\omega, |H(\kappa)|)} \exists \Sigma \ (``\Sigma \ \Gamma^{'C}_{\lambda}(\theta) (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } p")\} \\
    P^{''C}_{\lambda}(\theta) := \ & \{p \in [\mathcal{L}]^{< \theta} \cap Q_{\lambda} : \\
    & \ \Vdash_{Col(\omega, |H(\kappa)|)} \exists \Sigma \ (``\Sigma \ \Gamma^{''C}_{\lambda}(\theta) (\mathcal{L} \cap Q_{\lambda}, \mathfrak{A}) \text{-certifies } p")\} \\
    \mathbb{P}^{'C}_{\lambda}(\theta) := \ & (P^{'C}_{\lambda}(\theta), \supset) \\
    \mathbb{P}^{''C}_{\lambda}(\theta) := \ & (P^{''C}_{\lambda}(\theta), \supset) \text{.}
\end{align*}

Indeed, one can easily see that the ``quasi-second-order'' augmentation described immediately before the statement of Question \ref{oq1} carries over without a hitch in the proofs of Theorems \ref{notion2} and \ref{notion3}, due in no small part to the much-emphasised and -elaborated modularisability of our forcing framework. In fact, more can be said. Let $\mathbb{P}^{* < \omega_1}_{\lambda^{\dagger}}$ and $\mathbb{P}^{** < \omega_1}_{\lambda^{\dagger}}$ be the results of the aforementioned augmentation on, respectively, the version of $\mathbb{P}_{\lambda^{\dagger}}$ constructed in the proof of Theorem \ref{notion2}, and the version of $\mathbb{P}_{\lambda^{\dagger}}$ to be constructed according to the guidelines found immediately before the statement of Theorem \ref{notion3} (in order to prove Theorem \ref{notion3}). One can then obtain the forcing equivalence of $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{* < \omega_1}_{\lambda^{\dagger}}$, as well as the forcing equivalence of $\mathbb{P}^{''C}_{\lambda^{\dagger}}(\omega_1)$ and $\mathbb{P}^{** < \omega_1}_{\lambda^{\dagger}}$, through essentially the same argument as the one in the proof of Proposition \ref{prop741}. This realisation specifically motivates the next two questions.

\begin{ques}\label{oq2}
Let $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion2}. In $W$, can $\theta$, $C$ and
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}]
\end{gather*}
be chosen such that 
\begin{enumerate}[label=(\arabic*)]
    \item $\theta > \omega_1$ is a cardinal,
    \item \ref{K1} to \ref{K5} of Fact \ref{fact436} are fulfilled with $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\theta)$ does not collapse $\omega_1^W$?
\end{enumerate}
\end{ques}

Before stating our subsequent questions, let us first state a stronger, $\Diamond$-iteration version of \ref{K3} of Fact \ref{fact436}.
\begin{enumerate}[label=(K\arabic*'), leftmargin=40pt]
    \setcounter{enumi}{2}
    \item\label{K3'} in $V^{\mathbb{P}}$ there is a $\Diamond$-iteration $$\langle \bar{N}_i = (N_i; \in, I_i, a_i), \sigma_{ij} : i \leq j \leq \omega_1^V \rangle$$ satisfying 
    \begin{enumerate}[label=(\arabic*), leftmargin=40pt]
        \item\label{k31'} $\bar{N}_0 \in D^* \subset \mathbb{P}_{max}$,
        \item\label{k32'} $I_{\omega_1^V} = \mathrm{NS}_{\omega_1}^{V^{\mathbb{P}}} \cap N_{\omega_1^V}$ and
        \item\label{k33'} $a_{\omega_1^V} = A$.
...
\begin{enumerate}[label=(\arabic*)]
    \item $\theta > \omega_1$ is a cardinal,
    \item \ref{K1}, \ref{K2}, \ref{K4}, \ref{K5} of Fact \ref{fact436}, together with \ref{K3'}, are fulfilled with $\mathbb{P}^{''C}_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}^{''C}_{\lambda^{\dagger}}(\theta)$ does not collapse $\omega_1^W$?
\end{enumerate}
\end{ques}

\begin{ques}\label{addsreals2}
Let $V$, $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion2}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in  $W$, can $\theta$, $C$ and 
\begin{gather*}
    (C \cup \{\lambda^{\dagger}\} \longrightarrow \mathcal{P}(H(\lambda^{\dagger}))) [\lambda \mapsto Q_{\lambda}]
\end{gather*}
be chosen such that
\begin{enumerate}[label=(\arabic*)]
    \item $\theta$ is an infinite cardinal,
    \item \ref{K1} to \ref{K5} of Fact \ref{fact436} are fulfilled with $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\theta)$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}^{'C}_{\lambda^{\dagger}}(\theta)$ does not add reals?
\end{enumerate}
\end{ques}

\begin{ques}
Let $V$, $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion2}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in  $W$, is there a forcing notion $\mathbb{P}'_{\mathrm{new}}$ additively defined from the proof of Theorem \ref{notion1} such that
\begin{enumerate}[label=(\arabic*)]
    \item \ref{K1} to \ref{K5} of Fact \ref{fact436} are fulfilled with $\mathbb{P}'_{\mathrm{new}}$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}'_{\mathrm{new}}$ does not add reals?
\end{enumerate}
\end{ques}
...
Let $V$, $W$, $\lambda^{\dagger}$ and $\kappa$ be as in the proof of Theorem \ref{notion2}. Additionally, assume $GCH$ holds in $V$ (and so also in $W$). Working in $W$, is there a forcing notion $\mathbb{P}''_{\mathrm{new}}$ additively defined from the proof of Theorem \ref{notion1} such that
\begin{enumerate}[label=(\arabic*)]
    \item \ref{K1}, \ref{K2}, \ref{K4}, \ref{K5} of Fact \ref{fact436}, together with \ref{K3'}, are fulfilled with $\mathbb{P}''_{\mathrm{new}}$ in place of $\mathbb{P}$, and
    \item $\mathbb{P}''_{\mathrm{new}}$ does not add reals?
\end{enumerate}
\end{ques}

In the event that the answer to Question \ref{addsrealsadd} (and thus to Question \ref{addsreals}) is negative, it makes sense to consider a more general question.
...
Let $\mathfrak{M}$ be a GSeqA and $Sq$ be a function from a limit ordinal $< \kappa(\mathfrak{M})$ into $S(\mathfrak{M})$. A state $s \in S(\mathfrak{M})$ is a \emph{weak simple limit of} $Sq$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$ such that, interpreting $\ulcorner \in \urcorner$ as $\in$ and $\ulcorner R^{\ulcorner X \urcorner} \urcorner$ as $R_{Sq}^{\ulcorner X \urcorner}$, 
\begin{align*}
    & \ulcorner X \urcorner^s (\alpha_1, \dots, \alpha_n) = \alpha_{n+1} \iff \\
    & (\kappa(\mathfrak{M}); \in, R_{Sq}^{\ulcorner X \urcorner}, dom(sq)) \models \phi[x_1 \mapsto \alpha_1, \dots, x_{n+1} \mapsto \alpha_{n+1}]
\end{align*}
whenever $(\alpha_1, \dots, \alpha_{n+1}) \in \kappa(\mathfrak{M})^{n+1}$. In this case, we say $\{\phi_{lim}^{\ulcorner X \urcorner} : \ulcorner X \urcorner \in \sigma(\mathfrak{M})\}$ \emph{witnesses} $s$ \emph{is a weak simple limit of} $Sq$.

A state $s \in S(\mathfrak{M})$ is a \emph{simple limit of} $Sq$ iff there is a set $Y$ such that for every $\beta < \alpha := dom(Sq)$, $Y$ witnesses $s$ is a weak simple limit of
\begin{equation*}
    Sq \restriction [\beta, \alpha) \circ \pi_{\beta, \alpha} \text{,}
\end{equation*}
where $\pi_{\beta, \alpha}$ denotes the canonical order-preserving bijection from the order-type of the interval $[\beta, \alpha)$ into $[\beta, \alpha)$. In this case, we say $Y$ \emph{witnesses} $s$ \emph{is a simple limit of} $Sq$.
\end{defi}
...
Let $\mathfrak{M}$ be a GSeqAP and $Sq$ be a function from a limit ordinal into $S(\mathfrak{M})$. A state $s \in S(\mathfrak{M})$ is a \emph{weak simple limit of} $Sq$ iff for each $n$-ary function symbol $\ulcorner X \urcorner \in \sigma(\mathfrak{M})$ there is a simple limit formula $\phi_{lim}^{\ulcorner X \urcorner}$ for $(\ulcorner X \urcorner, \mathfrak{M})$ such that, interpreting $\ulcorner \in \urcorner$ as $\in$ and $\ulcorner R^{\ulcorner X \urcorner} \urcorner$ as $R_{Sq}^{\ulcorner X \urcorner}$, 
\begin{align*}
    & \ulcorner X \urcorner^s (\alpha_1, \dots, \alpha_n) = \alpha_{n+1} \iff \\
    & (\lambda; \in, R_{Sq}^{\ulcorner X \urcorner}, dom(Sq)) \models \phi[x_1 \mapsto \alpha_1, \dots, x_{n+1} \mapsto \alpha_{n+1}]
\end{align*}
whenever $\lambda$ is a limit ordinal $\geq max\{\kappa(\mathfrak{M}), dom(Sq)\}$ and $(\alpha_1, \dots, \alpha_{n+1}) \in \kappa(\mathfrak{M})^{n+1}$. In this case, we say $\{\phi_{lim}^{\ulcorner X \urcorner} : \ulcorner X \urcorner \in \sigma(\mathfrak{M})\}$ \emph{witnesses} $s$ \emph{is a weak simple limit of} $Sq$.
...
It is instructive to note here that given an outer model $W$ of $U$ and a set $x \in W \setminus U$, it is not necessary for $U[x]$ to exist. According to \cite{ciesielski}, an example of such an $x$ for an arbitrary CTM $U$ is the set of Cohen reals over $U$ that generates Cohen's famous model of $\mathsf{ZFC} + \neg \mathsf{CH}$. 
...
There are formulas $\phi$, $\psi$ and $\psi'$ in the language of set theory with the following properties: 
\begin{enumerate}[label=(\arabic*)]
    \item $\phi$, $\psi$ and $\psi'$ have two, three and two free variables respectively,
    \item\label{9252} $\phi(\mathfrak{T}, -)$ defines a function from the class of all TCIs $\mathfrak{T}$ into the universe,
    \item\label{9253} $\psi(\mathfrak{T}, -, x)$ defines a partial function from the class
    \begin{equation*}
        \{(\mathfrak{T}, x) : \mathfrak{T} \text{ is a TCI and } \exists \mathcal{L} \ (\phi(\mathfrak{T}, \mathcal{L}) \text{ and } x \subset \mathcal{L})\}
    \end{equation*}
    onto the class
    \begin{equation*}
        \{\mathcal{M} : \exists \mathfrak{T} \ (\mathfrak{T} \text{ is a TCI and } \mathcal{M} \models^* \mathfrak{T})\} \text{,}
    \end{equation*}
    \item\label{9254} $\psi' = \ulcorner \exists x_1 \ \psi(x_0, x_1, x_2)  \urcorner$,
    \item\label{9255} $\phi$, $\psi$ and $\psi'$ are absolute for outer models of $V$,
    \item\label{3245} whenever $\mathfrak{T}$ is a TCI,
    \begin{equation*}
        \{\mathcal{M} : \exists x \ \psi(\mathfrak{T}, \mathcal{M}, x)\} = \{\mathcal{M} : \mathcal{M} \models^* \mathfrak{T}\} \text{,}
    \end{equation*}
    \item\label{3245'} whenever $\mathfrak{T}$ is a TCI, $\mathcal{M} \models^* \mathfrak{T}$ and $\phi(\mathfrak{T}, \mathcal{L})$, the set
    \begin{equation*}
        \{x \in \mathcal{P}(\mathcal{L}) : \psi(\mathfrak{T}, \mathcal{M}, x)\}
    \end{equation*}
    is absolute for outer models of $V$,
    ...
    \item\label{9258} for all TCIs $\mathfrak{T}$, $\mathfrak{T}$ is consistent iff $$\Vdash_{Col(\omega, \lambda)} \exists x \ \psi'(\mathfrak{T}, x) \text{,}$$ where $\lambda$ is any sufficiently large cardinal.
\end{enumerate}
\end{prop}

\begin{rem}\label{rem926}
One may view Proposition \ref{prop322} as a template for the set of requirements any sufficiently local means of defining a class of small extensions of $V$ through their generators should fulfil. More precisely, should we replace 
\begin{itemize}
    \item ``$\mathfrak{T}$ is a TCI'' with any other description $\varphi(\mathfrak{T})$ of a parameter set $\mathfrak{T}$, and
    \item ``$\mathcal{M} \models^* \mathfrak{T}$'' with any other sufficiently local means of defining the class of generators $\mathcal{M}$ of small extensions of $V$ from a parameter set $\mathfrak{T}$ satisfying $\varphi(\mathfrak{T})$
\end{itemize}
throughout (a thorough unfolding of) Proposition \ref{prop322}, we should still expect the said proposition to hold. This is because morally, 
\begin{itemize}
    \item localness ought to imply absoluteness, 
    \item any ``sufficiently local'' means ought to come with an \textit{a priori} bound to the amount of information needed for its purpose, and
    \item in light of Proposition \ref{prop37}, some form of coding (such as the one described in the proof of Lemma \ref{setcode}) ought to be built into definitions of classes of small extensions, 
\end{itemize}
all of which justify the properties of $\phi$, and thus of $\psi$ and $\psi'$, in our modified version of Proposition \ref{prop322}.

In any case where the aforementioned replacements behave as expected, we say that the associated means of defining a class of small extensions of $V$ through their generators \textit{fulfils the requirements suggested by Proposition \ref{prop322}}.
\end{rem}
...
\begin{align*}
    ...
    \mathcal{M}(\mathfrak{T}, x) := \ & \text{ the unique } \mathcal{M} \text{ for which } \psi(\mathfrak{T}, \mathcal{M}, x)
\end{align*}
whenever $\mathfrak{T}$ is a TCI and $\psi(\mathfrak{T}, \mathcal{M}, x)$ for some $\mathcal{M}$. This is possible by \ref{9252} and \ref{9253} of Proposition \ref{prop322}. Given a TCI $\mathfrak{T} = (T, \sigma, \dot{\mathcal{U}}, \vartheta)$ and sets $y, z$ with $\vartheta(\dot{\mathcal{U}}) = (y, z)$, we should think of $\mathcal{L}_{\mathfrak{T}}$ as containing all the possible atomic sentences over $\sigma$ that involve members of $y$ as parameters. Then by \ref{9253} of Proposition \ref{prop322}, for any model $\mathcal{M}$ of $\mathfrak{T}$, there is $x$ ($\subset \mathcal{L}_{\mathfrak{T}}$, by \ref{3246} of Proposition \ref{prop322}) such that $\mathcal{M} = \mathcal{M}(\mathfrak{T}, x)$. This set $x$ can be thought of generally as a code of $\mathcal{M}$, and in our current use case, as \textit{the} ``$\mathfrak{T}$-specific atomic diagram''  of $\mathcal{M}$. According to \ref{3245} of Proposition \ref{prop322}, for any model $\mathcal{M}$ of a TCI $\mathfrak{T}$ there is some $x$ for which $\psi(\mathfrak{T}, \mathcal{M}, x)$ holds. Moreover, we can always recover the same $\mathcal{M}(\mathfrak{T}, x)$ in any outer model of $V$ from $\mathfrak{T}$ and $x$ alone, due to \ref{9255} of Proposition \ref{prop322}. Finally, in the presence of what has been discussed in this paragraph so far, \ref{3245'} of Proposition \ref{prop322} implies that any set $x$ for which $\psi(\mathfrak{T}, \mathcal{M}, x)$ holds in any outer model of $V$ containing $\mathcal{M}$ necessarily generates the smallest outer model of $V$ containing $\mathcal{M}$. As a consequence, 
\begin{equation}\label{eq921}
    V[\mathcal{M}] = V[x] \text{ if } V \text{ is a CTM and } \psi(\mathfrak{T}, \mathcal{M}, x) \text{ for some TCI } \mathfrak{T} \in V \text{.}
\end{equation}

\begin{rem}\label{rem927}
Note that our argument for (\ref{eq921}) only leverages on the conditions enumerated in Proposition \ref{prop322}. Hence, (\ref{eq921}) holds for any means of defining a class of small extensions of $V$ through their generators, as long as it fulfils the requirements suggested by Proposition \ref{prop322} (see Remark \ref{rem926}).
\end{rem}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \end{gather*}
    and
    \item\label{92102} $X \leq^M Y$ to denote the statement
    \begin{align*}
...
Instead of specifying $\mathrm{Eval}^V(\mathfrak{T}') \subset \mathrm{Eval}^V(\mathfrak{T})$ and $\mathrm{Eval}^V(F(\mathfrak{T})) \subset \mathrm{Eval}^V(\mathfrak{T})$ in \ref{92101} and \ref{92102} of Definition \ref{def9210} respectively, we can respectively require every member of  $\mathrm{Eval}^V(\mathfrak{T}')$ and $\mathrm{Eval}^V(F(\mathfrak{T}))$ to extend some member of $\mathrm{Eval}^V(\mathfrak{T})$. The latter approach takes into account the ``computational power'' of member TCIs when talking about the complexity of local method definitions. To wit, we have the following definition.

\begin{defi}
Given local method definitions $X, Y$ of a CTM $V$, let
\begin{enumerate}[label=(\arabic*)]
    \item $X \leq^N_w Y$ denote the statement
    \begin{gather*}
        \text{``for each consistent } \mathfrak{T} \in X \text{ there is a consistent } \mathfrak{T}' \in Y \text{ such that} \\ 
        \text{whenever } W \in \mathrm{Eval}^V(\mathfrak{T}') \text{ there is } W' \in \mathrm{Eval}^V(\mathfrak{T}) \text{ satisfying } W' \subset W \text{'',}
    \end{gather*}
    and
    \item $X \leq^N Y$ denote the statement
    \begin{gather*}
        \text{``there is a function } F : X \longrightarrow Y \text{ definable in } V \text{ such that} \\
        \text{for each consistent } \mathfrak{T} \in X \text{, } F(\mathfrak{T}) \text{ is consistent and} \\ \text{whenever } W \in \mathrm{Eval}^V(F(\mathfrak{T})) \text{ there is } W' \in \mathrm{Eval}^V(\mathfrak{T}) \text{ satisfying } W' \subset W \text{''.}
    \end{gather*}
\end{enumerate}
\end{defi}

Obviously, $\leq^M_w \subset \leq^N_w$ and $\leq^M \subset \leq^N$ as partial orders on local method definitions. In fact, it can be easily verified that the identity map is both a dense embedding of $\leq^M_w$ into $\leq^N_w$ and a dense embedding of $\leq^M$ into $\leq^N$. Considering that the results in the remaining part of this thesis are positive with respect to the relation $\leq^M$, they also hold should we substitute $\leq^N$ for $\leq^M$ when stating them. 
...
\section{Beyond Local Method Definitions}
...
\end{itemize} 
As such, one might wonder if it is possible to generate small extensions of $V$ based locally on a set in $V$, in a way that differs substantively from set forcing. In this section, we will examine some arguments against this.

\subsection{Every Method Definition is \texorpdfstring{$\leq^M_w$-below $\mathsf{Fg}$}{weakly M-reducible in Fg}}

We start with a relatively simple argument, based on the relations $\leq^M$ and $\leq^M_w$.

The following fact can be extracted from the standard proof (see e.g. the lead-up to Corollary 15.42 of \cite{jech}) of Fact \ref{fact221}.

\begin{fact}\label{fact9413}
Let
\begin{enumerate}[label=(\alph*)]
    \item\label{9413a} $\varphi$ be a formula with one free variable in the language associated with $V$,
    \item $\mathbb{P}$ be a forcing notion, 
    \item $X$ be any set, and
    \item\label{9413d} $\dot{x}$ be a $\mathbb{P}$-name of a subset of $X$.
\end{enumerate}
Assume $\varphi$ is absolute for outer models of $V$, and that $\Vdash_{\mathbb{P}} \varphi(\dot{x})$. Then there exist $\mathbb{B}$ and $\dot{\bar{x}}$ such that
\begin{enumerate}[label=(\arabic*)]
    \item $\mathbb{B}$ is a complete subalgebra of $B(\mathbb{P})$, 
    \item $\dot{\bar{x}}$ is a $\mathbb{B}$-name of a subset of $X$, 
    \item $\Vdash_{\mathbb{B}} \varphi(\dot{\bar{x}})$, and
    \item $V[g] = V[\dot{\bar{x}}[g]]$ for every $\mathbb{B}$-generic filter $g$ over $V$.
\end{enumerate}
Moreover, distinguished witnesses $\mathbb{B}(\mathbb{P}, \dot{x})$ and $\dot{\bar{x}}(\mathbb{P})$, of $\mathbb{B}$ and $\dot{\bar{x}}$ respectively, can be chosen such that the function 
\begin{equation*}
    (\mathbb{P}, \dot{x}) \mapsto (\mathbb{B}(\mathbb{P}, \dot{x}), \dot{\bar{x}}(\mathbb{P}))
\end{equation*}
on the class of pairs $(\mathbb{P}, \dot{x})$ satisfying \ref{9413a} to \ref{9413d} in conjunction with any fixed $\varphi$ and $X$ is definable.
\end{fact}

\begin{thm}\label{thm9414}
Let $\mathfrak{T}$ be a TCI. Then $\{\mathfrak{T}\} \leq^M \mathsf{Fg}$.
\end{thm}

\begin{proof}
The theorem is trivially true if $\mathfrak{T}$ is inconsistent. So let us assume, without loss of generality, that $\mathfrak{T}$ is consistent. Setting $\lambda$ to be large enough, we have $$\Vdash_{Col(\omega, \lambda)} \exists x \ \psi'(\mathfrak{T}, x)$$ by \ref{9258} of Proposition \ref{prop322}. Then \ref{9254} and \ref{3246} of Proposition \ref{prop322} tell us for some set $\mathcal{L}$ there is a $Col(\omega, \lambda)$-name $\dot{x}$ for a subset of $\mathcal{L}$ for which $\Vdash_{Col(\omega, \lambda)} \psi'(\mathfrak{T}, \dot{x})$. Viewing $\psi'(\mathfrak{T}, x)$ as a formula with a single free variable $x$, we can apply Fact \ref{fact9413} with \ref{9255} of Proposition \ref{prop322} in mind, to obtain $\mathbb{B}$ and $\dot{\bar{x}}$ such that
\begin{enumerate}[label=(\arabic*)]
    \item $\mathbb{B}$ is a complete subalgebra of $B(Col(\omega, \lambda))$, 
    \item $\dot{\bar{x}}$ is a $\mathbb{B}$-name of a subset of $\mathcal{L}$, 
    \item\label{p94143} $\Vdash_{\mathbb{B}} \psi'(\mathfrak{T}, \dot{\bar{x}})$, and
    \item\label{p94144} $V[g] = V[\dot{\bar{x}}[g]]$ for every $\mathbb{B}$-generic filter $g$ over $V$.
\end{enumerate}
Now let $g$ be any $\mathbb{B}$-generic filter $g$ over $V$. Then in $V[g]$, there exists $\mathcal{M}$ satisfying $\psi(\mathfrak{T}, \mathcal{M}, \dot{\bar{x}}[g])$ by \ref{9254} of Proposition \ref{prop322} and \ref{p94143} of this proof. Fix one such $\mathcal{M}$. As a consequence of (\ref{eq921}) and \ref{p94144} of this proof, $V[g] = V[\mathcal{M}]$. Finally, \ref{3245} of Proposition \ref{prop322} gives us $\mathcal{M} \models^* \mathfrak{T}$.
\end{proof}

By carefully analysing the proof of Theorem \ref{thm9414}, we can come to a stronger conclusion. 

\begin{thm}\label{thm9415}
Let $\emptyset \neq X \subset \mathbf{M}_S(V)$ be defined by a means fulfilling the requirements suggested by Proposition \ref{prop322} (see Remark \ref{rem926}). Then there is a forcing notion $\mathbb{P} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset X$. 
\end{thm}

\begin{proof}
Since the proof of Theorem \ref{thm9414} only levarages on the conditions enumerated in Proposition \ref{prop322} as well as (\ref{eq921}), by Remark \ref{rem927}, an argument virtually identical to the one in the said proof can be used here.
\end{proof}

In practice, where forcing is used predominantly within the context of consistency proofs, Theorem \ref{thm9415} hints that there is no better tool than forcing to define a class of small extensions of $V$ through their generators, if we want this definition to be sufficiently local. 

What is a good benchmark for being sufficiently local? It might be illuminating to look at methods of generating small extensions other than set forcing. Currently known methods of this sort --- including the various types of amenable class forcing, as well as more esoteric ones (see e.g. \cite{stanley}) --- depend chiefly on decoding functions that unravel generators into subclasses of $V$ fulfilling certain conditions. These generators are always bounded subclasses of $V$, while these decoding functions, being the most complicated of the components that define the methods, are typically (if not always) $\Delta_1$-definable with parameters in $V \cup \{V\}$. This leads to applicable instances of these methods being $\Delta_1$-definable with parameters in $V \cup \{V\}$. Of course, the same definability condition applies to set forcing.

Now, being sufficiently local within a small extension $V[g]$ of $V$ necessarily excludes the use of $V$ itself as a parameter, unless $V$ is definable in $V[g]$. But if $g$ is a bounded subclass of $V$, then $V$ being definable in $V[g]$ must come from the fact that $V$ is definable in $V[g]$ with parameters in $V \cup \{g\}$. So, in any case, it makes sense to forbid the use of $V$ --- or any internally non-definable predicate thereof --- as a parameter in our push for localness. As it turns out, with this seemingly minor restriction imposed upon a property that virtually all known methods of generating small extensions possess, one can show that set forcing is in some sense the best such method for individual use cases.

\begin{defi}
Let 
\begin{enumerate}[label=(\alph*)]
    \item $n < \omega$,
    \item $\Vec{y} \in V^n$, and
    \item $\varphi$ be a $(n + 1)$-ary formula in the language of set theory.
\end{enumerate}
Define 
\begin{align*}
    U(\Vec{y}, \varphi) := \ & \{V[x] : \exists x' \in V \ (x \subset x') \text{ and } \varphi (\Vec{y}, x) \} \\
    = \ & \{V[x] \in \mathbf{M}_S(V) : \exists x' \in V \ ((V[x]; \in) \models \text{``} x \subset x' \text{''} \wedge \varphi (\Vec{y}, x)) \} \text{.}
\end{align*}
\end{defi}

\begin{cor}\label{cor9416}
Let 
\begin{enumerate}[label=(\alph*)]
    \item $n < \omega$,
    \item $\Vec{y} \in V^n$, and
    \item $\varphi$ be a $(n + 1)$-ary $\Delta_1$ formula in the L\'{e}vy hierarchy.
\end{enumerate}
If $U(\Vec{y}, \varphi) \neq \emptyset$, then there is a forcing notion $\mathbb{P} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset U(\Vec{y}, \varphi)$. 
\end{cor}

\begin{proof}
Since $U(\Vec{y}, \varphi) \neq \emptyset$, we can fix some $x, x'$ such that 
\begin{equation}\label{eq942}
    x' \in V \text{, } x \subset x' \text{ and } \varphi (\Vec{y}, x) \text{.}
\end{equation}
Notice that (\ref{eq942}) is provably equivalent over $\mathrm{ZFC}$ (actually over a much weaker set theory) to $\varphi'(y, x)$ for some $y \in V$ and some binary $\Delta_1$ formula $\varphi'$ in the L\'{e}vy hierarchy. Now $\varphi'$ naturally gives rise to a means fulfilling the requirements suggested by Proposition \ref{prop322}. To elaborate, we replace 
\begin{itemize}
    \item ``$\mathfrak{T}$ is a TCI'' with ``$\mathfrak{T} = y$'', and
    \item ``$\mathcal{M} \models^* \mathfrak{T}$'' with ``$\varphi'(\mathfrak{T}, \mathcal{M})$'',
\end{itemize}
throughout Proposition \ref{prop322}, so that the said proposition still holds should we then define
\begin{enumerate}[label=(\roman*)]
    \item\label{9416i} $\phi(\mathfrak{T}, z) := \mathfrak{T} = y \wedge z = x'$, and
    \item\label{9416ii} $\psi(\mathfrak{T}, z, x) := \mathfrak{T} = y \wedge z = x \wedge \varphi'(\mathfrak{T}, x)$.
\end{enumerate}
One way to see that condition \ref{9258} of our modified Proposition \ref{prop322} holds with definitions \ref{9416i} and \ref{9416ii}, is to invoke an argument similar to that in the proof of Lemma \ref{inout}. 

The long of and short of it is that the hypothesis of Theorem \ref{thm9415} is satisfied with
\begin{equation*}
    X := \{V[x] : x \subset x' \text{ and } \varphi(\Vec{y}, x)\} \text{,}
\end{equation*}
and since it is obvious that $X \subset U(\Vec{y}, \varphi)$, it suffices to apply Theorem \ref{thm9415} to yield a forcing notion $\mathbb{P} \in V$ with the property $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset X$.
\end{proof}

Returning to our original context of local method definitions, the following corollaries can be observed.

\begin{cor}\label{cor9417}
Let $X$ be a local method definition.
\begin{enumerate}[label=(\arabic*)]
    \item\label{94151} $X \leq^M_w \mathsf{Fg}$.
    \item\label{94152} If $X$ is a set, then $X \leq^M \mathsf{Fg}$.
\end{enumerate}
\end{cor}

\begin{proof}
Given Theorem \ref{thm9414}, apply Axiom of Choice outside $V$ for \ref{94151} and inside $V$ for \ref{94152}.
\end{proof}

\begin{cor}
\leavevmode 
\begin{enumerate}[label=(\arabic*)]
    \item\label{94161} Assume there is a definable (with parameters) well-ordering of $V$. Then $X \leq^M \mathsf{Fg}$ for every local method definition.
    \item\label{94162} If $\text{``}V = L\text{''}$ holds, then $X \leq^M \mathsf{Fg}$ for every local method definition.
\end{enumerate}
\end{cor}

\begin{proof}
Given Theorem \ref{thm9414}, \ref{94161} is immediate. Then \ref{94162} holds because of \ref{94161} and the fact that there is a definable well-ordering of $L$.
\end{proof}

\begin{rem}\label{rem9414}
Note that already $X \leq^N \mathsf{Fg}$ for every local method definition $X$ as a direct result of Lemma \ref{conalt}. Recalling Fact \ref{fact221}, this suggests that any small extension of $V$ not accessible by forcing cannot be triangulated in $\mathbf{M}_S(V)$ by a local method definition with a margin of error not obviously worse than that of forcing.
\end{rem}

\subsection{Difficulties in Concluding the \texorpdfstring{$\leq^M$-dominance}{M-reducibility Dominance} of Forcing}

From \ref{9252} and \ref{3246} of Proposition \ref{prop322}, as well as the last sentence of Fact \ref{fact9413}, one can infer that the main obstacle to proving the $\leq^M$-maximality of $\mathsf{Fg}$ in every CTM with our current tools is precisely the lack of canonical $Col(\omega, \lambda)$-names $\dot{x}$ that allow the proof of Theorem \ref{thm9414} to go through. In fact, this obstacle is intrinsic via the following two lemmas, as it is known that $\mathsf{ZFC}$ is consistent with the non-existence of a definable global choice function --- one can always force it, say, by adding proper class many Cohen reals.

\begin{lem}\label{lem9419}
Let $\lambda$ be an infinite cardinal, $1 \leq n < \omega$, and have $\Vdash^n_{Col(\omega, \lambda)}$ denote the restriction of $\Vdash_{Col(\omega, \lambda)}$ to $\Sigma_n$ sentences, that is,
\begin{equation*}
    \Vdash^n_{Col(\omega, \lambda)} \ := \{(p, \psi) : p \Vdash_{Col(\omega, \lambda)} \psi \text{ and } \psi \text{ is } \Sigma_n \text{ with parameters in } V^{Col(\omega, \lambda)}\} \text{.}
\end{equation*}
Define $\mathrm{Ex}_n(\lambda)$ to be $ran(\Vdash^n_{Col(\omega, \lambda)})$. If there is an ordinal definable class function $F$ on $\mathrm{Ex}_n(\lambda)$ such that $ran(F) \subset V^{Col(\omega, \lambda)}$ and 
\begin{equation}\label{eq943}
    \forall \psi \in \mathrm{Ex}_n(\lambda) \ \forall \phi \ (((\Vdash^n_{Col(\omega, \lambda)} \psi) \wedge \psi = \ulcorner \exists x \ \phi(x) \urcorner) \implies \Vdash^n_{Col(\omega, \lambda)} \phi(F(\psi))) \text{,}
\end{equation}
then there is an ordinal definable global choice function.
\end{lem}

\begin{proof}
Assume $F$ exists for some $\lambda$ and $n$ in accordance with the hypothesis of the lemma. Note that the forcing notion $Col(\omega, \lambda)$ and the proper classes $V^{Col(\omega, \lambda)}$, $\Vdash^n_{Col(\omega, \lambda)}$ and $\mathrm{Ex}_n(\lambda)$ are all definable from the parameter $\lambda$, which means (\ref{eq943}) is expressible as a single sentence with ordinal parameters in the language of set theory. 

Fix any definable well-ordering $<_{\lambda}$ of (the conditions of) $Col(\omega, \lambda)$. For example, the length-lexicographical ordering works as $<_{\lambda}$. For any non-empty set $X$, set $\psi_X := \ulcorner \exists x \ ( x \in \check{X}) \urcorner \in \mathrm{Ex}_n(\lambda)$, where $\check{X}$ is the canonical $Col(\omega, \lambda)$-name for $X$. Obviously, $\Vdash^n_{Col(\omega, \lambda)} \psi_X$, so by (\ref{eq943}), $\Vdash^n_{Col(\omega, \lambda)} F(\psi_X) \in \check{X}$. Let $p_X$ be the $<_{\lambda}$-least member of the (necessarily non-empty) set 
\begin{equation*}
    \{p \in Col(\omega, \lambda) : \exists x \ (p \Vdash^n_{Col(\omega, \lambda)} F(\psi_X) = \check{x})\} \text{.}
\end{equation*}
Then there is a unique $x$ such that $p_X \Vdash^n_{Col(\omega, \lambda)} F(\psi_X) = \check{x}$, and it must be the case that $x \in X$. We have thus given a definition of a global choice function based on $F$ and $\lambda$. Since $F$ is itself ordinal definable, we are done.
\end{proof}

Lemma \ref{lem9419} still holds should we replace $Col(\omega, \lambda)$ with any other forcing notion, as long as we relax the definability condition of the global choice function we obtain in the conclusion to allow for (non-ordinal) parameters. 

\begin{lem}\label{lem9420}
Let $\mathbb{P}$ be any forcing notion, $1 \leq n < \omega$, and have $\Vdash^n_{\mathbb{P}}$ denote the restriction of $\Vdash_{\mathbb{P}}$ to $\Sigma_n$ sentences, that is,
\begin{equation*}
    \Vdash^n_{\mathbb{P}} \ := \{(p, \psi) : p \Vdash_{\mathbb{P}} \psi \text{ and } \psi \text{ is } \Sigma_n \text{ with parameters in } V^{\mathbb{P}}\} \text{.}
\end{equation*}
Define $\mathrm{Ex}_n(\mathbb{P})$ to be $ran(\Vdash^n_{\mathbb{P}})$. If there is a definable (with parameters) class function $F$ on $\mathrm{Ex}_n(\mathbb{P})$ such that $ran(F) \subset V^{\mathbb{P}}$ and 
\begin{equation*}
    \forall \psi \in \mathrm{Ex}_n(\mathbb{P}) \ \forall \phi \ (((\Vdash^n_{\mathbb{P}} \psi) \wedge \psi = \ulcorner \exists x \ \phi(x) \urcorner) \implies \Vdash^n_{\mathbb{P}} \phi(F(\psi))) \text{,}
\end{equation*}
then there is a definable (with parameters) global choice function.
\end{lem}

\begin{proof}
Essentially the same as that of Lemma \ref{lem9419}: we need only add the forcing notion $\mathbb{P}$ and a well-ordering thereof to the list of parameters in the definition of our global choice function.
\end{proof}

More specifically, what stands between Corollary \ref{cor9417} and the collapse of the local method hierarchy above $\mathsf{\Sigma^M_1}$, if we were to follow an argument similar to that found in the proof of Theorem \ref{thm9414}, are the next two lemmas.

\begin{lem}\label{lem9421}
Let $\lambda$ be an infinite cardinal, $1 \leq n < \omega$, and $\Vdash^n_{Col(\omega, \lambda)}$ be as defined in Lemma \ref{lem9419}.
If for some $k < \omega$, there is an ordinal definable class function $F$ on $\mathsf{\Sigma^M_k}$ such that $ran(F) \subset V^{Col(\omega, \lambda)}$ and 
\begin{equation}\label{eq944}
    \forall \mathfrak{T} \in \mathsf{\Sigma^M_k} \ ((\Vdash^n_{Col(\omega, \lambda)} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}} \text{''})) \implies \Vdash^n_{Col(\omega, \lambda)} \text{``} F(\mathfrak{T}) \models^* \check{\mathfrak{T}} \text{''}) \text{,}
\end{equation}
then there is an ordinal definable global choice function.
\end{lem}

\begin{proof}
Assume $F$ exists for some $\lambda$, $n$ and $k$ in accordance with the hypothesis of the lemma. Recall that for any TCI $\mathfrak{T}$, $\text{``} \mathcal{M} \models^* \mathfrak{T} \text{''}$ is expressible as a $\Sigma_1$ sentence in the language of set theory with the parameters $\mathcal{M}$ and $\mathfrak{T}$, so 
\begin{equation*}
    \forall \mathfrak{T} \ (\mathfrak{T} \text{ is a TCI} \implies ((\Vdash^n_{Col(\omega, \lambda)} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}} \text{''})) \iff \mathfrak{T} \text{ is consistent})) \text{.}
\end{equation*}

As in the proof of Lemma \ref{lem9419}, fix a definable well-ordering $<_{\lambda}$ of (the conditions of) $Col(\omega, \lambda)$. Let $X$ be any non-empty set, and set
\begin{equation*}
    \mathfrak{T}_X := (\emptyset, \ulcorner c \urcorner, \dot{\mathcal{U}}, \{(\dot{\mathcal{U}}, (X, 1)), (\ulcorner c \urcorner, (X, 0))\}) \text{,}
\end{equation*}
where $\ulcorner c \urcorner$ is a constant symbol independent of $X$. Obviously, $\dot{\mathcal{U}}$ does not depend on $X$, while $\mathfrak{T}_X \in \mathsf{\Sigma^M_0} \subset \mathsf{\Sigma^M_k}$. Models of $\mathfrak{T}_X$ vary only by their interpretation of $\ulcorner c \urcorner$, and they are allowed to interpret $\ulcorner c \urcorner$ as any member of $X$. Since $X$ is non-empty, $\mathfrak{T}_X$ is consistent, and we have that 
\begin{equation*}
    \Vdash^n_{Col(\omega, \lambda)} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}}_X \text{''}) \text{.}
\end{equation*}
By (\ref{eq944}), $\Vdash^n_{Col(\omega, \lambda)} \text{``} F(\mathfrak{T}_X) \models^* \check{\mathfrak{T}}_X \text{''}$. Choose $p_X$ to be the $<_{\lambda}$-least member of the (necessarily non-empty) set 
\begin{equation*}
    \{p \in Col(\omega, \lambda) : \exists x \ (p \Vdash^n_{Col(\omega, \lambda)} \text{``} F(\mathfrak{T}_X) \text{ interprets } \ulcorner \check{c} \urcorner \text{ as } \check{x} \text{''})\} \text{.}
\end{equation*}
Then there is a unique $x$ such that $p_X \Vdash^n_{Col(\omega, \lambda)} \text{``} F(\mathfrak{T}_X) \text{ interprets } \ulcorner \check{c} \urcorner \text{ as } \check{x} \text{''}$, and it must be the case that $x \in X$. We are now done as we were in the proof of Lemma \ref{lem9419}, bearing in mind that the symbols $\ulcorner c \urcorner$ and $\dot{\mathcal{U}}$ are coded as members of $H(\omega)$, and are thus definable without parameters.
\end{proof}

We can proceed to conclude the next lemma by modifying the proof of Lemma \ref{lem9421}, the way we proceeded from Lemma \ref{lem9419} to Lemma \ref{lem9420}. The only additional ingredient we need is the realisation that for every forcing notion $\mathbb{P}$, 
\begin{equation*}
    \forall \mathfrak{T} \ (\mathfrak{T} \text{ is a TCI} \implies ((\Vdash_{\mathbb{P}} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}} \text{''})) \iff \mathfrak{T} \text{ is consistent})) \text{.}
\end{equation*}

\begin{lem}\label{lem9422}
Let $\mathbb{P}$ be any forcing notion, $1 \leq n < \omega$, and $\Vdash^n_{\mathbb{P}}$ be as defined in Lemma \ref{lem9420}.
If for some $k < \omega$, there is a definable (with parameters) class function $F$ on $\mathsf{\Sigma^M_k}$ such that $ran(F) \subset V^{\mathbb{P}}$ and 
\begin{equation*}
    \forall \mathfrak{T} \in \mathsf{\Sigma^M_k} \ ((\Vdash^n_{\mathbb{P}} \exists \mathcal{M} \ (\text{``} \mathcal{M} \models^* \check{\mathfrak{T}} \text{''})) \implies \Vdash^n_{\mathbb{P}} \text{``} F(\mathfrak{T}) \models^* \check{\mathfrak{T}} \text{''}) \text{,}
\end{equation*}
then there is a definable (with parameters) global choice function.
\end{lem}

\subsection{Enhanced Arguments}

Corollary \ref{cor9416} can be improved in different directions with the use of some basic coding. Before giving these improved versions, let us prove two theorems which are both helpful and independently interesting.
...
Define 
\begin{equation*}
    U'(\Vec{y}, \varphi) := \{W \in \mathbf{M}(V) : (W; \in) \models \exists x \ \varphi (\Vec{y}, x) \} \text{.}
\end{equation*}
\end{defi}

\begin{thm}\label{thm9419}
Let 
\begin{enumerate}[label=(\alph*)]
    \item $n < \omega$,
    \item $\Vec{y} \in V^n$, and
    \item $\varphi$ be a $(n + 1)$-ary $\Sigma_1$ formula in the L\'{e}vy hierarchy.
\end{enumerate}
If $U'(\Vec{y}, \varphi) \neq \emptyset$, then there is a TCI $\mathfrak{T} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}) \subset U'(\Vec{y}, \varphi)$. 
\end{thm}

\begin{proof}
Let $n$, $\Vec{y}$ and $\varphi$ be in accordance with the hypothesis, and assume $U'(\Vec{y}, \varphi) \neq \emptyset$. Then for some outer model $W$ of $V$, $(W; \in) \models \exists x \ \varphi (\Vec{y}, x)$. Since $\varphi$ is $\Sigma_1$, it can be written as $\ulcorner \exists \Vec{z} \ \phi(\Vec{y}, \Vec{z}, x) \urcorner$ for some formula $\phi$ that is $\Delta_0$ in the L\'{e}vy hierarchy. In $W$, there must be a witness $a$ for $\exists x \ \varphi (\Vec{y}, x)$ as well as witnesses $\Vec{b}$ for $\varphi$ itself, so let 
\begin{itemize}
    \item $S$ be a subset of some cardinal $\kappa$ that codes $\{\Vec{y}, a, \Vec{b}\}$ \textit{a la} Lemma \ref{setcode}, and
    \item $\lambda := \kappa^+$.
\end{itemize}
Then $L_{\lambda}[S] \models \phi(\Vec{y}, \Vec{b}, a)$ and $|L_{\lambda}[S]| = \lambda$ in $W$. 

In $V$, let $N := H(\lambda^+)$. We define a TCI $\mathfrak{T} = (T, \sigma, \dot{\mathcal{U}}, \vartheta) \in V$ with $\sigma$ containing exactly the binary relation symbols $\in$, $\ulcorner \in^* \urcorner$, $\ulcorner R^* \urcorner$ and $\ulcorner f_y^* \urcorner$ for each $y$ occurring in $\Vec{y}$, together with constant symbols $\ulcorner \lambda^* \urcorner$, $\ulcorner x^* \urcorner$, $\ulcorner y^* \urcorner$ for each $y$ occurring in $\Vec{y}$, and $\ulcorner z^* \urcorner$ for each $z$ occurring in $\Vec{z}$. Also, define
\begin{gather*}
    \vartheta (\dot{\mathcal{U}}) := (N, 1) \\
    \vartheta (\ulcorner \in \urcorner) := (\in \cap \ N, 1) \\
    \vartheta (\ulcorner \in^* \urcorner) := (\lambda \times \lambda, 0) \\
    \vartheta (\ulcorner R^* \urcorner) := (\lambda \times \lambda, 0) \\
    \vartheta (\ulcorner f_y^* \urcorner) := (\lambda \times trcl(\{y\}), 0) \text{ for each } y \text{ occurring in } \Vec{y} \\
    \vartheta (\ulcorner \lambda^* \urcorner) := (\{\lambda\}, 0) \\
    \vartheta (\ulcorner x^* \urcorner) := (\lambda, 0) \\
    \vartheta (\ulcorner y^* \urcorner) := (\lambda, 0) \text{ for each } y \text{ occurring in } \Vec{y} \\
    \vartheta (\ulcorner z^* \urcorner) := (\lambda, 0) \text{ for each } z \text{ occurring in } \Vec{z} \text{.}
\end{gather*}
Morally, for some transitive model $N'$ of set theory,
\begin{enumerate}[label=(\Alph*)]
    \item\label{9419A} $\ulcorner \in^* \urcorner$ is supposed to interpret a binary relation on $\lambda$ isomorphic to $\in \cap \ N'$ on $N'$, 
    \item\label{9419B} $\ulcorner R^* \urcorner$ is supposed to be a rank function on $N'$, and 
    \item\label{9419C} the $\ulcorner f_y^* \urcorner$s are supposed to guarantee that every $y$ occurring in $\Vec{y}$ is a member of $N'$.
\end{enumerate}
Meanwhile, 
\begin{enumerate}[label=(\Alph*)]
    \setcounter{enumi}{3}
    \item\label{9419D} the starred constant symbols are supposed to pick out parameters and witnesses in $N'$ corresponding to their unstarred names.
\end{enumerate}

Finally, we want $T$ to contain the following statements (the reader should check that they are expressible in our signature with our set constraints).
\begin{enumerate}[label=(\arabic*)]
    \item\label{94191} $dom(\in^*) = \lambda^*$.
    \item\label{94192} The extensionality of $\in^*$, i.e.
    \begin{equation*}
        \ulcorner \forall x \in \lambda^* \ \forall y \in \lambda^* \ (\forall z \ (z \in^* x \iff z \in^* y) \implies x = y) \urcorner \text{.}
    \end{equation*}
    \item\label{94193} $R^*$ behaves like the rank function, i.e. $R^*$ is a function from $\lambda^*$ onto itself, and for all $x \in \lambda^*$,
    \begin{equation*}
        R^*(x) = \bigcup \{R^*(y) + 1 : y \in^* x\} \text{.}
    \end{equation*}
    \item\label{94194} For each $y$ occurring in $\Vec{y}$, $f_y^*$ is a partial function from $\lambda^*$ onto $trcl(\{y\})$, such that for all members $z_1$ and $z_2$ of $dom(f_y^*)$, $z_1 \in^* z_2 \iff f_y^*(z_1) \in f_y^*(z_2)$.
    \item\label{94195} For each $y$ occurring in $\Vec{y}$, there is $x$ such that $z \in^* x \iff z \in dom(f_y^*)$ for all $z$, and furthermore, $y^*$ is equal to a $\in^*$-maximal member of $x$.
    \item\label{94196} $\phi^*$, where $\phi^*$ is obtained from $\phi(\Vec{y}, \Vec{z}, x)$ by a global substitution of $\ulcorner \in \urcorner$ with $\ulcorner \in^* \urcorner$, $\ulcorner x \urcorner$ with $\ulcorner x^* \urcorner$, $\ulcorner y \urcorner$ with $\ulcorner y^* \urcorner$ for every $y$ occurring in $\Vec{y}$, and $\ulcorner z \urcorner$ with $\ulcorner z^* \urcorner$ for every $z$ occurring in $\Vec{z}$.
\end{enumerate}
It is easy to verify that for some transitive model $N'$ of set theory containing $\Vec{y}$, \ref{94191} to \ref{94193} give us \ref{9419A} and \ref{9419B}, \ref{94194} and \ref{94195} give us \ref{9419C}, and \ref{94196} gives us \ref{9419D}. Moreover, this $N'$ is unique and 
\begin{equation}\label{eq943'}
    (N'; \in, \Vec{y}) \cong (\lambda; \in^*, \Vec{y^*}) \text{,}
\end{equation}
where $\Vec{y^*}$ is got from $\Vec{y}$ by replacing every instance of $y$ occurring in $\Vec{y}$ with the corresponding $y^*$, via the Mostowski collapse.

Now, $\mathfrak{T}$ is consistent due to the existence of $L_{\lambda}[S]$ in $W$. On the other hand, if $\mathcal{M} \models^* \mathfrak{T}$ in some outer model $W'$ of $V$, then there is some transitive model $N'$ of set theory containing $\Vec{y}$ and satisfying (\ref{eq943'}) by the absoluteness of the Mostowski collapse. This means $(N'; \in, \Vec{y}) \models \exists x \ \varphi (\Vec{y}, x)$, and because $\varphi$ is $\Sigma_1$, necessarily $(W'; \in) \models \exists x \ \varphi (\Vec{y}, x)$ as well. We have thus shown that $\mathrm{Eval}^V(\mathfrak{T}) \subset U'(\Vec{y}, \varphi)$.
\end{proof}
...
Define 
\begin{equation*}
    U''(\Vec{y}, \varphi) := \{W \in \mathbf{M}(V) : \exists x \in W \ (W = V[x] \wedge (W; \in) \models \varphi (\Vec{y}, x))\} \text{.}
\end{equation*}
\end{defi}

\begin{thm}\label{thm9420}
Let 
\begin{enumerate}[label=(\alph*)]
    \item $n < \omega$,
    \item $\Vec{y} \in V^n$, and
...
\begin{proof}
Let $n$, $\Vec{y}$ and $\varphi$ be in accordance with the hypothesis, and assume $U''(\Vec{y}, \varphi) \neq \emptyset$. Then for some outer model $W$ of $V$, there is $x \in W$ such that $W = V[x]$ and $(W; \in) \models \varphi(\Vec{y}, x)$. By Proposition \ref{prop35}, for some cardinal $\kappa$ in $W$, 
\begin{align}
\begin{split}\label{eq943''}
    \exists x' \ \exists x'' \ \exists \nu \ (\text{``} & x'' \text{ is a set of ordinals, } \nu \text{ is an isomorphism from the relation} \\
    & \text{coded by } x'' \text{ onto } trcl(\{x'\}) \text{, } \varphi(\Vec{y}, x') \text{ and } x'' \in V_{\kappa}[x'] \text{''})
\end{split}
\end{align}
holds in $W$. On the other hand, any outer model $W'$ of $V$ in which (\ref{eq943''}) holds with $a$ and $a'$ in place of $x'$ and $x''$ respectively, must also have as an inner model $V[a] = V[a'] \in U''(\Vec{y}, \varphi)$. As (\ref{eq943''}) is a $\Sigma_1$ formula in the L\'{e}vy hierarchy with parameters in $V$, we may hereon assume, without loss of generality, that $x$ is a set of ordinals with supremum $\alpha$. 

As in the proof of Theorem \ref{thm9419}, we write $\varphi(\Vec{y}, x)$ as $\ulcorner \exists \Vec{z} \ \phi(\Vec{y}, \Vec{z}, x) \urcorner$ for some formula $\phi$ that is $\Delta_0$ in the L\'{e}vy hierarchy, and find in $W$ witnesses $\Vec{b}$ for $\varphi$ so that $(W; \in) \models \phi(\Vec{y}, \Vec{b}, x)$. In $V$, choose a strong limit cardinal $\lambda > \alpha$ and a bijection $g$ from $\lambda$ onto $V_{\lambda}$, such that $\{\Vec{y}, \Vec{b}, x\} \in V_{\lambda}[x]$ and $(V_{\lambda}[x]; \in) \models \phi(\Vec{y}, \Vec{b}, x)$. Over the structure $(V_{\lambda}[x]; \in, g)$, one can define using a formula $\psi$ in two free variables, a canonical bijection $g'$ from $\lambda$ onto $V_{\lambda}[x]$ for which $g'(y) = z \iff \psi(y, z)$. 

\begin{rem}\label{rem9414'}
We can choose $\psi$ so that the definition of $g'$ goes through all formulas in a single free variable with parameters in $V_{\lambda} \cup \{g\}$, in a manner similar to how the canonical global well-ordering is coherently defined on every initial segment of $L$. As we shall see, this lets us simplify our proof a little.
\end{rem}

In $V$, we set $N := H(\lambda^+)$ and expand on the TCI $\mathfrak{T} = (T, \sigma, \dot{\mathcal{U}}, \vartheta)$ defined in the proof of Theorem \ref{thm9419}. Next, we do the following.
\begin{enumerate}[label=(\roman*)]
    \item Add a new constant symbol $\ulcorner g^* \urcorner$ to $\sigma$ for the interpretation of $g$.
    \item\label{9413ii} Add a binary relation symbol $\ulcorner V^* \urcorner$ to $\sigma$ for the interpretation of the copy of $(V_{\lambda}; \in)$ in the structure $(\lambda; \in^*)$.
    \item Carry out the assignment $\vartheta(\ulcorner g^* \urcorner) := (\{g\}, 0)$.
    \item\label{9413iv} Carry out the assignment $\vartheta(\ulcorner V^* \urcorner) := (V_{\lambda} \times \lambda, 0)$.
\end{enumerate}
(If $\psi$ were chosen as per Remark \ref{rem9414'}, we may omit steps \ref{9413ii} and \ref{9413iv}.)

For any sentence $\theta$ in the language of set theory with parameters occurring in or among $\Vec{y}$, $\Vec{b}$, $x$, and $g$, define $\theta^*$ to be the unique formula obtained from $\theta$ by a global substitution of
\begin{itemize}
    \item $\ulcorner \in \urcorner$ with $\ulcorner \in^* \urcorner$, 
    \item $\ulcorner x \urcorner$ with $\ulcorner x^* \urcorner$, 
    \item $\ulcorner g \urcorner$ with $\ulcorner g^* \urcorner$, 
    \item $\ulcorner y \urcorner$ with $\ulcorner y^* \urcorner$ for every $y$ occurring in $\Vec{y}$, 
    \item $\ulcorner z \urcorner$ with $\ulcorner z^* \urcorner$ for every $z$ occurring in $\Vec{z}$, 
    \item any existential quantification of the form $\ulcorner \exists z \urcorner$ with the corresponding bounded quantification $\ulcorner \exists z \in \lambda^* \urcorner$, and
    \item any universal quantification of the form $\ulcorner \forall z \urcorner$ with the corresponding bounded quantification $\ulcorner \forall z \in \lambda^* \urcorner$.
\end{itemize}
Finally, we add to $T$ the following statements (the reader should again check that they are expressible in our signature with our set constraints).
\begin{enumerate}[label=(\arabic*)]
    \item $\psi^*$ defines an order isomorphism $\pi$ under $\in$ from a subset of $\lambda$ onto $\lambda$.
    \item $dom(\pi)$ contains exactly all members $c$ of $\lambda$ that satisfy both
    \begin{equation*}
        \ulcorner \forall y \ \forall z \ ((y \in^* c \wedge z \in^* y) \implies z \in^* c) \urcorner
    \end{equation*}
    and
    \begin{equation*}
        \ulcorner \forall y \ \forall z \ ((y \in^* c \wedge z \in^* c) \implies (y \in^* z \vee z \in^* y \vee y = z)) \urcorner \text{.}
    \end{equation*}
    \item For each $c \in dom(\pi)$, 
    \begin{equation*}
        c \cap dom(\pi) = \{d \in \lambda : d \in^* c\} \text{.}
    \end{equation*}
    \item $x^*$ is a subset of $dom(\pi)$.
    \item\label{94135} $V^*$ is an isomorphism from $(V_{\lambda}; \in)$ onto $(ran(V^*); \in^*)$.
    \item For every $c \in \lambda$ and every $d \in (R^*)^{-1}(c+1) \setminus (R^*)^{-1}(c)$, there are $e \in \lambda$, $\Vec{a} \in ((R^*)^{-1}(c))^{< \omega}$, $\Vec{b} \in ((V^*)"(V_{c+1}))^{< \omega}$, and formulas $\chi$ and $\chi'$ such that
    \begin{itemize}
        \item $\chi'$ is a formula in the language of set theory of arity $dom(\Vec{a}) + dom(\Vec{b}) + 1$,
        \item $\{y \in \lambda : y \in^* e\} = ((R^*)^{-1}(c))$, 
        \item $\chi$ is the formal expression of the statement 
        \begin{equation*}
            \text{``} d = \{z \in e : (e; \in) \models \chi(\Vec{a}, \Vec{b}, z)\} \text{''}
        \end{equation*} 
        as a sentence in the language of set theory, and
        \item $\chi^*$ holds.
    \end{itemize}
    \item\label{94137} For every $c \in \lambda$, $\Vec{a} \in ((R^*)^{-1}(c))^{< \omega}$, $\Vec{b} \in ((V^*)"(V_{c+1}))^{< \omega}$ and every formula $\chi'$, there are $d \in (R^*)^{-1}(c+1) \setminus (R^*)^{-1}(c)$, $e \in \lambda$ and a formula $\chi$ such that if $\chi'$ is a formula in the language of set theory of arity $dom(\Vec{a}) + dom(\Vec{b}) + 1$, then
    \begin{itemize}
        \item $\{y \in \lambda : y \in^* e\} = ((R^*)^{-1}(c))$, 
        \item $\chi$ is the formal expression of the statement 
        \begin{equation*}
    ...
    \end{itemize}
\end{enumerate}
(If $\psi$ were chosen as per Remark \ref{rem9414'}, we may omit statements \ref{94135} to \ref{94137}.)

It is not difficult to verify that given (the appropriate reduct of) a model $(\lambda; \in^*, \Vec{y^*}, \Vec{z^*}, x^*)$ of our expanded $\mathfrak{T}$ in any outer model $W'$ of $V$, there are sets $\Vec{z}$ and $x'$ such that
\begin{enumerate}[label=(\Alph*)]
    \item\label{9420A} $\{\Vec{y}, \Vec{z}, x'\} \in V_{\lambda}[x']$.
    \item $(V_{\lambda}[x']; \in, \Vec{y}, \Vec{z}, x') \cong (\lambda; \in^*, \Vec{y^*}, \Vec{z^*}, x^*)$, where 
    \begin{itemize}
        \item $\Vec{y^*}$ is again got from $\Vec{y}$ by replacing every instance of $y$ occurring in $\Vec{y}$ with the corresponding $y^*$, and
        \item $\Vec{z^*}$ is got from $\Vec{z}$ by replacing every instance of $z$ occurring in $\Vec{z}$ with the corresponding $z^*$,
    \end{itemize}
    via the Mostowski collapse,
    \item\label{9420C} $(V_{\lambda}[x']; \in, \Vec{y}, \Vec{z}, x') \models \phi(\Vec{y}, \Vec{z}, x')$, 
    \item $x' \subset ORD^{V_{\lambda}[x']} = \lambda$, and
    \item\label{9420E} letting $\psi^{\dagger}$ denote the unique formula --- which happens to be $\Delta_0$ in the L\'{e}vy hierarchy --- obtained from $\psi$ by a global substitution of 
    \begin{itemize}
        \item any existential quantification of the form $\ulcorner \exists z \urcorner$ with the corresponding bounded quantification $\ulcorner \exists z \in V_{\lambda}[x'] \urcorner$, and
        \item any universal quantification of the form $\ulcorner \forall z \urcorner$ with the corresponding bounded quantification $\ulcorner \forall z \in V_{\lambda}[x'] \urcorner$,
    \end{itemize}
    $\psi^{\dagger}$ defines in $W'$ a bijection $\pi^{\dagger}$ from $\lambda$ onto $V_{\lambda}[x']$.
\end{enumerate}
Further, $\mathfrak{T}$ is consistent due to the existence of $(V_{\lambda}[x]; \in, g)$ in $W$. So let $\mathcal{M} \models^* \mathfrak{T}$ in some outer model of $V$. In $V[\mathcal{M}]$, there is a set of ordinals $x'$ and a list of parameters $\Vec{z}$ for which \ref{9420C} holds, so by the absoluteness of $\phi$, $(V[\mathcal{M}]; \in) \models \varphi(\Vec{y}, x')$. Trivially, $V[x'] \subset V[\mathcal{M}]$. On the flipside, we can accurately define $V_{\lambda}[x']$ in $V[x']$, which implies we can recover through $\psi^{\dagger}$ and its absoluteness, the same sets $\Vec{z}$ and $\pi^{\dagger}$ that make \ref{9420A} to \ref{9420E} true in $V[\mathcal{M}]$. But $\mathcal{M}$ is definable in an absolute sense from the structure $(V_{\lambda}[x']; \in, \Vec{y}, \Vec{z}, x')$ and the map $\psi^{\dagger}$, allowing us to conclude that $V[x'] = V[\mathcal{M}]$.
\end{proof}
...
\begin{enumerate}[label=(\arabic*)]
    \item\label{94210} If $U(\Vec{y}, \varphi) \neq \emptyset$, then there is a forcing notion $\mathbb{P} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset U(\Vec{y}, \varphi)$.
    \item\label{94211} If $U'(\Vec{y}, \varphi) \neq \emptyset$, then there is a forcing notion $\mathbb{P} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset U'(\Vec{y}, \varphi)$. 
    \item\label{94212} If $U''(\Vec{y}, \varphi) \neq \emptyset$, then there is a forcing notion $\mathbb{P} \in V$ such that $\mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset U''(\Vec{y}, \varphi)$. 
\end{enumerate}
\end{cor}

\begin{proof}
\ref{94211} is a direct consequence of Corollary \ref{cor9417} and Theorem \ref{thm9419}. Similarly, \ref{94212} is a direct consequence of Corollary \ref{cor9417} and Theorem \ref{thm9420}.

To see that \ref{94210} is true, assume $U(\Vec{y}, \varphi) \neq \emptyset$, or in other words, that there are $V[x] \in \mathbf{M}_S(V)$ and $x' \in V$ satisfying $(V[x]; \in) \models \text{``} x \subset x' \text{''} \wedge \varphi (\Vec{y}, x)$. Set
\begin{equation*}
    \varphi' := \ulcorner \text{``} x \subset x' \text{''} \wedge \varphi (\Vec{y}, x) \urcorner \text{.}
\end{equation*}
Now $\varphi'$ is a $(n+2)$-ary $\Sigma_1$ formula in the L\'{e}vy hierarchy with list of parameters $\Vec{y}' := \Vec{y}^{\frown}(x') \in V^{n+1}$. Moreover, $V[x] \in U''(\Vec{y}', \varphi') \subset U(\Vec{y}, \varphi)$, so an application of \ref{94212} yields the existence of a forcing notion $\mathbb{P} \in V$ for which
\begin{equation*}
    \mathrm{Eval}^V(\mathfrak{T}(\mathbb{P})) \subset U''(\Vec{y}', \varphi') \subset U(\Vec{y}, \varphi) \text{.} \qedhere
\end{equation*}
\end{proof}
...
Let us now turn our attention to the relation $\leq^M$ and the local method hierarchy. Naturally, one can also ask a variant of each of the remaining questions in this section, in which we swop out every (explicit or otherwise) instance of the relation $\leq^M$ for an instance of the relation $\leq^N$.

\begin{ques}\label{q843}
Is there a local method definition $X$ such that $X \not\leq^M \mathsf{Fg}$?
\end{ques}
...
\begin{rem}
By Remark \ref{rem9414}, the $\leq^N$-variant of Question \ref{q841} reduces to the question of whether $\mathsf{\Sigma^M_0} \equiv^N \mathsf{\Sigma^M_1}$ holds.
\end{rem}
...
\begin{rem}
By Remark \ref{rem9414}, the $\leq^N$-variant of Question \ref{q418} reduces to the $\leq^N$-variant of Question \ref{q417}.
\end{rem}
