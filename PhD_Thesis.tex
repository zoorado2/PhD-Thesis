...
A state $s \in S(\mathfrak{M})$ is a \emph{simple limit of} $Sq$ iff there is a set $\Gamma$ such that for every $\beta < \alpha := dom(Sq)$, $\Gamma$ witnesses $s$ is a weak simple limit of
\begin{equation*}
    Sq \restriction [\beta, \alpha) \circ \pi_{\beta, \alpha} \text{,}
\end{equation*}
where $\pi_{\beta, \alpha}$ denotes the canonical order-preserving bijection from the order-type of the interval $[\beta, \alpha)$ into $[\beta, \alpha)$. In this case, we say $\Gamma$ \emph{witnesses} $s$ \emph{is a simple limit of} $Sq$.
\end{defi}
